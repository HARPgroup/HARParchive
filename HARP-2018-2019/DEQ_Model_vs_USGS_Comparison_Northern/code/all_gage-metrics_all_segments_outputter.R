...
for (i in 1:num.gages) {
  ...
  segs.no <- as.numeric(length(RivSegStr))
  if (segs.no == 1) {
    all.errors.all.segments[all.errors.line.no,] <- all.metrics[1,2:num.metrics]
...
write.csv(all.errors.all.segments, paste0(container, "//spatial_analysis//user's_results//all.gage-metrics.pct.error.csv"))