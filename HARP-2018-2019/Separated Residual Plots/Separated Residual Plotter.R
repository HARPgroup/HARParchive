...
for (i in 2: num.metrics) {
  ...
  all.residuals.and.areas <- data.frame(matrix(NA, nrow = num.segs, ncol = 3))
  colnames(all.residuals.and.areas) <- c('area', 'residual', 'segID')
  
  for (j in 1: num.segs) {
    seg <- as.character(GageToSeg[j, 2])
    seg <- substr(seg, 1, 2)
    all.residuals.and.areas[j, 3] <- seg
  ...
  BS <- subset(all.residuals.and.areas, segID == 'BS')
  MN <- subset(all.residuals.and.areas, segID == 'MN')
  OD <- subset(all.residuals.and.areas, segID == 'OD')
  TU <- subset(all.residuals.and.areas, segID == 'TU')
  NR <- subset(all.residuals.and.areas, segID == 'NR')
  OR <- subset(all.residuals.and.areas, segID == 'OR')
  
  minres <- 1.1*min(all.residuals.and.areas$residual)
  maxres <- 1.1*max(all.residuals.and.areas$residual)
  

  #par(mfrow=c(2,3))
  plotname <- paste0(metricname, '.jpg')
  BSplot <- ggplot(data = BS, aes(x=area, y=residual)) + 
    geom_point() + 
    geom_smooth(method = lm) + 
    xlab('Drainage Area (Sq. Mile)') +
    ylab('Flow, Model-Gage (cfs)') +
    labs(title = paste0('BS: ',metricname))
  MNplot <- ggplot(data = MN, aes(x=area, y=residual)) + 
    geom_point() + 
    geom_smooth(method = lm) + 
    xlab('Drainage Area (Sq. Mile)') +
    ylab('Flow, Model-Gage (cfs)') +
    labs(title = paste0('MN: ',metricname))
  ODplot <- ggplot(data = OD, aes(x=area, y=residual)) + 
    geom_point() + 
    geom_smooth(method = lm) + 
    xlab('Drainage Area (Sq. Mile)') +
    ylab('Flow, Model-Gage (cfs)') +
    labs(title = paste0('OD: ',metricname))
  TUplot <- ggplot(data = TU, aes(x=area, y=residual)) + 
    geom_point() + 
    geom_smooth(method = lm) + 
    xlab('Drainage Area (Sq. Mile)') +
    ylab('Flow, Model-Gage (cfs)') +
    labs(title = paste0('TU: ',metricname))
  NRplot <- ggplot(data = NR, aes(x=area, y=residual)) + 
    geom_point() + 
    geom_smooth(method = lm) + 
    xlab('Drainage Area (Sq. Mile)') +
    ylab('Flow, Model-Gage (cfs)') +
    labs(title = paste0('NR: ',metricname))
  ORplot <- ggplot(data = OR, aes(x=area, y=residual)) + 
    geom_point() + 
    geom_smooth(method = lm) + 
    xlab('Drainage Area (Sq. Mile)') +
    ylab('Flow, Model-Gage (cfs)') +
    labs(title = paste0('OR: ',metricname))
  
  jpeg(paste0(metricname, '.jpg'))
  grid.arrange(BSplot, MNplot, ODplot, TUplot, NRplot, ORplot, ncol=3, nrow = 2)
  #plot(BS$area, BS$residual, main = paste0('Big Sandy:', metricname), xlab = 'Drainage Area', ylab = 'Residual (Model-Gage)', ylim = c(minres, maxres))
  #plot(MN$area, MN$residual, main = paste0('Meherrin:', metricname), xlab = 'Drainage Area', ylab = 'Residual (Model-Gage)', ylim = c(minres, maxres))
  #plot(OD$area, OD$residual, main = paste0('Dan:', metricname), xlab = 'Drainage Area', ylab = 'Residual (Model-Gage)', ylim = c(minres, maxres))
  #plot(TU$area, TU$residual, main = paste0('Tennessee:', metricname), xlab = 'Drainage Area', ylab = 'Residual (Model-Gage)', ylim = c(minres, maxres))
  #plot(NR$area, NR$residual, main = paste0('New:', metricname), xlab = 'Drainage Area', ylab = 'Residual (Model-Gage)', ylim = c(minres, maxres))
  #plot(OR$area, OR$residual, main = paste0('Roanoke:', metricname), xlab = 'Drainage Area', ylab = 'Residual (Model-Gage)', ylim = c(minres, maxres))
  dev.off()
}
