...
basepath='C:\\Users\\FujitsuT\\Downloads\\HARP\\GitHub\\hydro-tools';
...
GageToSeg <- read.csv(file=paste0(container,'\\data\\SPLITSEG_Gage_To_Segment_Northern.csv'))
...
c.areas <- matrix(0,nrow=num.segs, ncol = 2)
colnames(c.areas) <- c('num', 'area')
...
for (i in 1:num.segs) {
  ...
  # GETTING CONTRIBUTING DRAINAGE AREA FOR GAGE
  c.area <- readNWISsite(RivSegGage)
  c.area <- c.area$drain_area_va
  c.areas[i, 1] <- i
  c.areas[i, 2] <- c.area
...
# ORDERING BY AREA
c.areas <- c.areas[order(c.areas[,2], decreasing = TRUE),]
...
for (i in 1:num.segs) {
  #BEGIN BY DEFINING WATERSHED NUM BASED ON C.AREAS
  wshednum <- as.numeric(c.areas[i,1])
  c.namer <- paste0('c.watershedDF', wshednum)
  full.exist <- exists(c.namer)
  if (Metrics$DesiredMetric[wshednum] < -50) {
    if (full.exist == TRUE) {
      map <- map + geom_polygon(data = eval(parse(text = c.namer)), color='black', aes(fill='a'), lwd = 0.1, alpha = 1)
    }
  } else if (Metrics$DesiredMetric[wshednum] < -25) {
    if (full.exist == TRUE) {
      map <- map + geom_polygon(data = eval(parse(text = c.namer)), color='black', aes(fill='b'), lwd = 0.1, alpha = 1)
    }    
  } else if (Metrics$DesiredMetric[wshednum] < -5) {
    if (full.exist == TRUE) {
      map <- map + geom_polygon(data = eval(parse(text = c.namer)), color='black', aes(fill='c'), lwd = 0.1, alpha = 1)
    }    
  } else if (Metrics$DesiredMetric[wshednum] < 5) {
    if (full.exist == TRUE) {
      map <- map + geom_polygon(data = eval(parse(text = c.namer)), color='black', aes(fill='d'), lwd = 0.1, alpha = 1)
    }    
  } else if (Metrics$DesiredMetric[wshednum] < 25) {
    if (full.exist == TRUE) {
      map <- map + geom_polygon(data = eval(parse(text = c.namer)), color='black', aes(fill='e'), lwd = 0.1, alpha = 1)
    }    
  } else if (Metrics$DesiredMetric[wshednum] < 50) {
    if (full.exist == TRUE) {
      map <- map + geom_polygon(data = eval(parse(text = c.namer)), color='black', aes(fill='f'), lwd = 0.1, alpha = 1)
    }    
  } else {
    if (full.exist == TRUE) {
      map <- map + geom_polygon(data = eval(parse(text = c.namer)), color='black', aes(fill='g'), lwd = 0.1, alpha = 1)
    }    
  }
# }
# for (i in 1:num.segs) {
