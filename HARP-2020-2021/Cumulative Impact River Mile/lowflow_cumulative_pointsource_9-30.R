...
flow_and_intake <- function(AllSegList, riv_seg, runid, flow_metric) {
  ...
  while (i <= nrow(river)) {
    ...
    prop <- getProperty(inputs, site)
    wd_cumulative <- prop$propvalue
    intakecumulative <- append(intakecumulative, wd_cumulative)
  ...
  intakecumulative <- data.frame(intakecumulative)
  pointsource <- data.frame(pointsource)
  river_data <- cbind(segment, name, area, intake, intakecumulative, pointsource, flow, length, metric)
...
metric2 <- dat2$metric
intake2 <- dat2$intake
intakecumulative2 <- dat2$intakecumulative
pointsource2 <- dat2$pointsource
pct <- (dat1$intake/dat1$flow)*100
pct2 <- (dat2$intake/dat2$flow)*100

totaldat <- as.data.frame(cbind(dat1, flow2,intake2,intakecumulative2,pointsource2,metric2, pct, pct2))
...
#ps vs wd (cumulative)
ggplot(totaldat, aes(x=mile))+
  geom_line(aes(y = intakecumulative, colour = 'cumulative intake'))+
