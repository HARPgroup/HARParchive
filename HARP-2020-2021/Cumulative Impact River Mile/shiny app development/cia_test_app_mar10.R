...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    sidebarMenu(
      ...
      menuItem("Runid Table", tabName="runidtable",icon = icon('tint'))
...
server <- function(input, output) {
  ...
  CIA_data <- function(riv_seg, runid1, runid2, flow_metric){
    ...
    df <- data.frame(
      ...
      'runid' = c(paste0('runid_', runid1), paste0('runid_', runid2), '0.%20River%20Channel', 'local_channel'),
      'runlabel' = c('flow1', 'flow2', 'length', 'subcomp_da'),
      'metric' = c( flow_metric, flow_metric, 'length', 'drainage_area')
    )
  ...
  output$table <- DT :: renderDataTable({
    cia_data <- CIA_data(riv_seg = input$riv_seg, runid1 = input$runid1, runid2 = input$runid2, flow_metric = input$flow_metric)
    cia_data_table <- data.frame(cia_data$propname,  cia_data$mile, cia_data$flow1, cia_data$flow2)
    datatable(cia_data_table, colnames= c("Segment Name", "River Mile", "Q1", "Q2"))
  })
