...
flow_and_intake <- function(AllSegList, riv_seg, runid) {
  ...
  river_data <- cbind(segment, area, intake, cintake, flow, length, pct, metric)
  colnames(river_data) <- c("segment", "area", "intake", "cintake", "flow", "length", "pct", "metric")
...
cintake1 <- dat1$cintake...1.547
cintake2 <- dat2$cintake...1.547
metric1 <- dat1$metric / dat1$flow * 100
metric2 <- dat2$metric / dat2$flow * 100
pct1 <- (cintake1 / flow1) * 100
pct2 <- (cintake2 / flow2) * 100
...
## change these to be cintake or intake
totaldat <- as.data.frame(cbind(flow1, flow2, metric1, metric2,df))
...
ggplot(totaldat) +
  geom_point(aes(x = xaxis, y = metric1, colour = 'runid11 Flow')) +
  geom_point(aes(x = xaxis, y = metric2, colour = 'runid18 Flow')) +
  geom_line(aes(x = xaxis, y = metric1, colour = 'runid11 Flow')) +
  geom_line(aes(x = xaxis, y = metric2, colour = 'runid18 Flow')) +
  geom_bar(aes(x = xaxis, y = vals), stat = 'identity', fill = 'blue') +
