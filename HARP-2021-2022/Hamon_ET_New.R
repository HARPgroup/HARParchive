##### This script is an outline of calculating PET using the Hamon Method
### Where N = daylight hours = solar radiation (langley/hr) > 0 
## Last Update 7/22/21
## HARP Group
...
while(i<length(AllLandsegList)){
  ...
  dailyRAD <- sqldf("SELECT year, date, RAD, count(RAD) daylight_hrs
                  FROM dfRAD
                  WHERE rad > 0 
                  GROUP BY date")
  ...
  dailyMeanTMP <- sqldf("SELECT date, avg(temp) avg_temp
                     FROM dfTMP
                     GROUP BY date")
  # merge data into one data frame 
  dailyTOTAL <- merge(dailyRAD, dailyMeanTMP, by = "date")
  # Hamon Equation
  #N = daytime length
  N <- dailyTOTAL$daylight_hrs
  # e = saturation vapor pressur
  e = 6.108*exp((17.27*(dailyTOTAL$avg_temp)/((dailyTOTAL$avg_temp)+273.3)))
  # k = 1 = proportioanlity coefficient (always equal to 1)
  k <- 1
  # t = average daily temperature 
  t <- dailyTOTAL$avg_temp
  ET_H <- k*0.165*2.167*N*(e/(t+273.3))
  # add Hamon ET data to dataset
  # units in mm/day 
  dailyTOTAL$ET_H <- ET_H 
  # create and save HPET file as csv
  write.table(dailyTOTAL,paste0("/Users/katealbi/Desktop/HARP/",landseg,".HPET.csv"), 
