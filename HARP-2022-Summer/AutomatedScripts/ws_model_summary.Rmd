...
# rseg.data.3 <- data.table::fread('JA4_7280_7340_hydrd_wy_sub.csv')
...
# rseg.flow.3 <- zoo(as.numeric(as.character(rseg.data.3$Qout)), order.by = rseg.data.3$index)
# rseg.flow.3 <- window(rseg.flow.3, start = sdate, end = edate)
# mode(rseg.flow.3) <- 'numeric'

rseg.lowflow.1 <- IHA::group2(rseg.flow.1)
rseg.lowflow.2 <- IHA::group2(rseg.flow.2)
# rseg.lowflow.3 <- IHA::group2(rseg.flow.3)
...
``` {r scatter.2models, echo = FALSE, fig.cap=paste("The low flow scenarios and differences between:", title_param), fig.id = "scatter1", fig.cap.style = "Image Caption", fig.width = 6,fig.height = 8 }
...
``` {r scatter.3models, echo = FALSE}
# if (length(rseg.model.version) > 2) {
#   l90_diff.1v2 <- unlist(l90.data[2] - l90.data[3]) # has to unlist in order to plot with ggplot!
#   l90_diff.1v3 <- unlist(l90.data[2] - l90.data[4])
#   l90_diff.2v3 <- unlist(l90.data[3] - l90.data[4])
# 
  # l90_diff_pct <- 100.0 * (l90_diff) / l90.data[3]
  # 
  # l30_diff <- unlist(l30.data[2] - l30.data[3]) 
  # l30_diff_pct <- 100.0 * (l30_diff) / l30.data[3]
  # 
  # l7_diff <- unlist(l7.data[2] - l7.data[3]) 
  # l7_diff_pct <- 100.0 * (l7_diff) / l7.data[3]
  # 
  # l1_diff <- unlist(l1.data[2] - l1.data[3]) # has to unlist in order to plot with ggplot!
  # l1_diff_pct <- 100.0 * (l1_diff) / l1.data[3]
...
perc.diff.fig <- ggplot(stack(perc.diff.data), aes(x = ind, y = values)) +
  ...
  stat_summary(fun.data = fun_mean, geom="text", vjust=-0.7, aes(label=round(..y.., digits = 5)))
