...
params: 
  ...
  ftable.submod:  [ "value", "value"] #either p6 or p532, p6 will have /vadeq added automatically
  ftable.id: [ "value", "value"] #after /river/ and before /ftables in path
  seg: [ "value", "value"] #needed to pull LEN from ftable HYDR file?
...
```{r, echo = FALSE}
#Creating paths from input params to read in file containing length for ftables 
for (r in 1:length(rseg.model.version)) {
  ...
  length = as.numeric(lendf[rownum, 3])
  assign(paste0('length.',r),length*5280) #convert miles to ft
}
```
...
for (i in 1:length(ftable_raw$depth)) {
  if (!is.na(as.numeric(ftable_raw$depth[i]))) {
    ...
    ftable <- rbind(ftable, as.numeric(ftable_raw[row_i,]))
    colnames(ftable) <- c('depth','area','vol','disch')
  }
}
rownames(ftable) <- seq(1, length(ftable$depth)) #fix row numbers for clarity

#pull channel length:  -- now done using path created from params to pull from url
...
#    propcode='vahydro-1.0'), #can't be cbp-6.1 b/c it won't have a '0. River Channel'
#  TRUE)
...
#channel length coming from url believed to be in miles --- VERIFY!
#clength to be replaced by length.1, length.2, etc
#for (r in 1:length(rseg.model.version)) {
 # }

clength <- as.numeric(length.1) # -- temporary, for testing
...
persp(x, y, z, theta = 5, phi = 15, expand = .2, col = "#0099CC", axes=TRUE, nticks=8, ticktype="detailed", scale=TRUE, shade=0.65, border="003300", d=0.6, r=9, xlab="Width (ft)",
      ...
      main=paste(model.comparison[1,2], "Cross-section"), #model.comparison[1,i]
      sub=paste("Water is shown in channel at a depth of", ftable$depth[15],"ft \n Bankfull is approx.", ftable$depth[10], "ft"))->channel
  for(i in 1:15){
    lines (trans3d(x = c(-ftable$x[i], ftable$x[i]), y = seq(0, clength, by=10) , z = ftable$depth[i], pmat = channel), 
           col ="#33CCFF4F", pch=16) #bankfull
  }
#note: adding "_F" to the end of a color gives it transparency. 1 is more transparent; 9 is more opaque

persp(x, y, z, theta = 10, phi = 15, expand = .2, col = "#0099CC", axes=TRUE, nticks=8, ticktype="detailed", scale=TRUE, shade=0.65, border="003300", d=0.6, r=9, xlab="Width (ft)", ylab="Channel Length (ft)", zlab="Depth (ft)", main=paste(model.comparison[1,2], "Cross-section"), sub=paste("Bankfull Depth is approx.", ftable$depth[10], "ft"))

persp(x, y, z, theta = -5, phi = -15, expand = .2, col = "#0099CC", axes=TRUE, nticks=8, ticktype="detailed", scale=TRUE, shade=0.65, border="003300", d=0.6, r=9, xlab="Width (ft)", ylab="Channel Length (ft)", zlab="Depth (ft)", main=paste(model.comparison[1,2], "Cross-section"), sub=paste("Bankfull Depth is approx.", ftable$depth[10], "ft"))
...
``` {r scatter.2models, eval = FALSE, echo = FALSE, fig.cap=paste("The low flow scenarios and differences between:", title_param), fig.id = "scatter1", fig.cap.style = "Image Caption"}
...
``` {r percent, eval = FALSE, echo = FALSE, fig.cap=paste("The percent difference between the models:", title_param), fig.id = "percent", fig.cap.style = "Image Caption"}
