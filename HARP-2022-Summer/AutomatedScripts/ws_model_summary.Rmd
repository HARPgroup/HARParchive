...
trapezoid <- function(x, y){ #piecewise function utilizing R's understanding of TRUE==1 and FALSE==0
  (x <= 0-0.5*b) & (x >= 0-0.5*bf) * (slope*x) + 
  (x >= 0+0.5*b) & (x <= 0+0.5*bf) * (-slope*x) 
}

x <- seq(-bf, bf,length.out=50)
y <- seq(0, clength, length.out=50)
z <- outer(x, y, trapezoid)
 
# Create the plot
persp(x, y, z, theta = 0, phi = 30, expand = 0.2, col = "lightblue", border =NA)
