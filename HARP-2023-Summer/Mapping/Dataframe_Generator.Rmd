...
params:
  ...
  rivseg_metric: ["l90_Qout", "l30_Qout", "7q10", "Qout", "WA_90_mgd","l90_cc_Qout","l30_cc_Qout"] #l30_Qout, l90_Qout, 7q10
...
rsegs <- sqldf::sqldf(connection = ds$connection,
"SELECT hydroid, name, hydrocode AS rseg, hydrocode, dh_geofield AS geom
FROM dh_feature_fielded
WHERE ftype = 'vahydro' AND bundle = 'watershed'
")
...
if(featr_type == "facility"){ #get facility-level fiveyr_avg_mgy from foundatn_mp by summing all mp values for each facility:
  f_foundatn <- sqldf("
   SELECT facility_hydroid,CEDS_Facility_Id,facility,Use_Type,latitude,longitude,locality,facility_status,
    Total_Permit_Limit_MGY,gw_frac,wsp2040_mgy AS wsp_2040_mgy,wsp2020_2040_mgy AS wsp2020_2040_mgy,
    SUM(fiveyr_avg_mgy) as five_yr_avg  -- Creating an aggregated column for the fiveyr_avg_mgy
   FROM foundatn_mp
   GROUP BY Facility_hydroid,CEDS_Facility_Id,facility,Use_Type,
   locality,facility_status,Total_Permit_Limit_MGY,gw_frac,
   wsp_2040_mgy,wsp2020_2040_mgy
  ") # Not selecting any source related fields, as these are innacurate when grouped
  
  ## Updating Total_Permit_Limit_MGY from GPY to MGY
  ## Since this is a mixed number/character column, need to parse out the numbers, replace with those numbers divided by a million
  #### The numbers need to be first converted to numbers, then back to characters
  f_foundatn$Total_Permit_Limit_MGY[grepl("[0-9]",f_foundatn$Total_Permit_Limit_MGY)] <- 
    as.character(as.numeric(f_foundatn$Total_Permit_Limit_MGY[grepl("[0-9]",f_foundatn$Total_Permit_Limit_MGY)]) / 1000000)
...
## Adding an indicator for NC counties
featrs$Locality[featrs$Locality %in% c("Vance","Watauga")] <- paste0(featrs$Locality[featrs$Locality %in% c("Vance","Watauga")] , " (NC)")
...
featrs <- fn_sqldf_sf(
  ...
  left outer join featrs_riverseg_sm as b 
   ...
   GROUP BY a.featureid, a.pid, a.CEDS_Facility_Id, a.riverseg, b.riverseg  --Very small amount of duplicates created
 ", 
 geomback = 'featrs'
)
...
## Pulling in the tidal property
tidals <- sqldf::sqldf("
SELECT f.hydroid, f.hydrocode, f.name, t.propcode AS Tidal

FROM dh_properties t
LEFT JOIN dh_properties m
  ON m.pid = t.featureid
LEFT JOIN dh_feature f
  ON f.hydroid = m.featureid
  
WHERE t.propname = 'is_tidal'
", connection = ds$connection)

rsegs$tidal <- as.numeric(rsegs$hydroid %in% tidals$hydroid)
