...
params:
  ...
  baseline_runid_list: [ "runid_11", "runid_0" ]
  crs_default: 4326
...
for (j in 2:length(runid_list)) {
  for (k in 1:length(rivseg_metric)) {
    ...
    for (i in 1:length(baseline_runid_list)) {
      ...
      column1 = paste0(runid_list[i],"_",rivseg_metric[k])
      new_col = paste0("percentDiff_", rivseg_metric[k], "_", runid_list[i], "_", runid_list[j])
      message(paste("Adding %diff for", rivseg_metric[k], runid_list[i], runid_list[j]))
