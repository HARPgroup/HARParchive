...
params:
  ...
  runid_list: ["runid_11", "runid_13", "runid_17", "runid_0"]
...
foundatn_mp <- foundatn_mp[!(grepl('Abandoned',foundatn_mp$facility_status)),]
...
for (j in 2:length(runid_list)) {
  for (k in 1:length(rivseg_metric)) {
    ...
    if (rivseg_metric[k] == "Qout" & "runid_0" %in% runid_list) {
      column1 = paste0("runid_0_Qout")
      new_col = paste0("percentDiff_", rivseg_metric[k], "_runid_0_", runid_list[j])
            message(paste("Adding %diff for", rivseg_metric[k], "runid_0", runid_list[j]))
    } else {
