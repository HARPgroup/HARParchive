...
for (j in 2:length(runid_list)) {
  for (k in 1:length(rivseg_metric)) {
    ...
    if (rivseg_metric[k] == "Qout" & "runid_1000" %in% runid_list) {
      column1 = paste0("runid_1000_Qout")
      new_col = paste0("percentDiff_", rivseg_metric[k], "_runid_1000_", runid_list[j])
            message(paste("Adding %diff for", rivseg_metric[k], "runid_1000", runid_list[j]))
    } else {
