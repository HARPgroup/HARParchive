...
library(flextable)
library(geosphere)#Used one time
library(ggnewscale)#Used one time
library(ggrepel)
...
library(hydrotools)
library(stringr)
#The following packages may not be needed. They seem to only be used for 
#min, max, range, and merge, which are likely base wrappers. Kept for now.
library(rapportools)#Univar min,max,range wrappers
library(raster)#Difficult to tell if this is used.
library(sp)#Only uses merge, which may be base and raster also have
...
if (origin_type=="basin") { #finding upstream riversegs for basin maps 
  ...
  rsegs <- st_filter(rsegs, counties[counties$hydrocode==origin, ]) #filter basins by locality (if REST is working)
