...
source(paste0(HARParchive_location,"/HARP-2023-Summer/Mapping/Functions/fn_mapgen.R")) #load mapping function
...
for (k in 1:length(run_config$riverseg_metrics)){
  ...
  if (exists(data_set)) {
    ...
    digit <- run_config$riverseg_metrics[[k]]$digit
    ...
    #rounding
    numeric <- names(rseg_display[, sapply(rseg_display, is.numeric)])
    
    for (x in 1:length(numeric)) {
    rseg_display[,numeric[x]]<- round(rseg_display[,numeric[x]], digit)
    }
  ...
  for (x in 1:nrow(metric_names)){
    if (table_cols[i]==metric_names$read_metric_name[x])
    {
      names(rivTables[[k]])[names(rivTables[[k]]) == table_cols[i]] <- metric_names$new_metric_name[x]
    }
    else
    {
      next
  ...
  #Human Readable Names- edit in mapstyle config 
