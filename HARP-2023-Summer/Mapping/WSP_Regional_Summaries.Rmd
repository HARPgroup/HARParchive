...
include_gwma <- params$include_gwma
...
region_rsegs = rsegs # save these for later since rsegs is overwritten
...
This document does not provide current reported water use data. FORM 70-80-100, provided separately, contains information on individual water sources and water use.
...
table <- data.frame(
  ...
  VAHydro_ID = facils_table$CEDS_Facility_Id, 
...
} else if (origin_type == 'basin') {
  # this does not yet work, but it's a placeholder
  origin_shape = region_rsegs
  origin_shape$region = origin
}
...
When the groundwater fraction listed is 1.0, then the facility does not have surface water withdrawals, and does not have surface water modelling values.
