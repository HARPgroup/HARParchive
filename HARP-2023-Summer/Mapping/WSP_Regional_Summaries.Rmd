...
params: 
  ...
  origin: ["Roanoke_1"] #supply either a rivseg, locality, or region name ## JL7_7070_0001
  ...
  # default files for map's base-layers
...
source(paste0(github_location,"/HARParchive/HARP-2023-Summer/Mapping/Functions/fn_mapgen.R"),local = TRUE) #load mapping function
source(paste0(github_location,"/HARParchive/HARP-2023-Summer/Mapping/Functions/fn_tablegen.R"),local = TRUE) #load table function
source(paste0(github_location,"/HARParchive/HARP-2023-Summer/Mapping/Functions/fn_labelprep.R"),local = TRUE) #load labeling function
source(paste0(github_location,"/HARParchive/HARP-2023-Summer/Mapping/Functions/fn_nhd_labs.R"),local = TRUE) #load nhd label function
source(paste0(github_location,"/HARParchive/HARP-2023-Summer/Mapping/Config/mapstyle_config.R"),local = TRUE) #load general mapping aesthetics 
source(paste0(github_location,"/HARParchive/HARP-2023-Summer/Mapping/Config/rivsegmaps_config.R"),local = TRUE) #load rivseg-specific mapping aesthetics
...
regions_file <- paste0(github_location, '/HARParchive/HARP-2023-Summer/Mapping/DataFiles/regions_sf.csv')
counties_file <- paste0(github_location, '/HARParchive/HARP-2023-Summer/Mapping/DataFiles/counties_sf.csv')
cities_file <- paste0(github_location, '/HARParchive/HARP-2023-Summer/Mapping/DataFiles/cities.csv')
roads_file <- paste0(github_location, '/HARParchive/HARP-2023-Summer/Mapping/DataFiles/roads_sf.csv')

facils <- sf::st_read(featrs_file)
rsegs <- sf::st_read(rsegs_file)
counties <- sf::st_read(counties_file)
regions <- sf::st_read(regions_file)
...
## Framing Text:
Including how to use this doc per regulatory requirement.
Include note that water is limited for your locality due to what was withdrawn upstream, and what needs to be left for downstream users.
Ex. for Drought (reg section XXX), refer to summary document Drought section L90 maps.
...
if (show_map == TRUE) {
  for (i in 1:length(featrs_file_map_bubble_column)) { #execute for each metric in featrs_file_map_bubble_column param 
   ...
   if (i==1) { #data for table will be sorted by the first of featrs_file_map_bubble_column
    ...
    if (featrs_file_map_bubble_column[i]=="five_yr_avg"){
      ...
      #create title based on origin & type
    if (origin_type == "basin") {
      title <- (paste("Basin Upstream of", segs$name[segs$riverseg==origin] , origin, sep=" "))
    } else if (origin_type == "locality") {
      title <- paste0(origin, " Locality")
    } else if (origin_type == "region") {
      title <- paste0(origin, " Region")
    } 
    
    
      #create map title by combining label and title
      maptitle <- paste0(title, ", ", maplabel)
  
    
       
  
    #suppressWarnings(suppressMessages( 
      map <- fn_mapgen(mapnum = 1, style = styles[[map_style]], 
                  metric = featrs_file_map_bubble_column[i], segs = rsegs,
                  featr_type = featr_type, origin_type = origin_type, 
                  origin = origin, bbox = bbox,  
                  counties = counties, roads = roads, 
                  nhd = nhd, maplabs = maplabs, 
                  mp_layer = mp_layer, metric_unit = metric_unit, 
                  title = maptitle)
    #))
...
ft1 <- fn_tablegen(featr_type = "facility", table = table, 
                   ...
                   origin = origin, title = title ) #execute flextable function

```
###Narrative note: 
Narrative including note that facilities constructed after initial plan submission are not show in 2020 scenario
...
if (show_map == TRUE) {
  for (i in 1:length(rivseg_metric)){ #something going wrong in rsegs df within this loop 
    rseg_leg_title <- legend_titling(rivseg_metric[i], runid_list)
    
    rivmapmetric <- paste0(rivseg_metric[i]) #define title here for mapping so each map has diff metric title, pass into function
    ...
    rivmaptitle <- paste0(title, ", ", rivmaplabel)
  ...
  ####
    ...
    rsegs_nogeom <- st_drop_geometry(rsegs)
    pct_col <- as.data.frame(rsegs_nogeom[,paste0('percentDiff_', rivseg_metric[i])]) #identify pct diff for metric of interest
    colnames(pct_col) <- 'pct_diff'
    rsegs <- cbind(rsegs_nogeom, pct_col)
    rm(rsegs_nogeom)
    ##
    ...
    rivseg_classes <- fn_sqldf_sf( #sorting the data into bins based on their range 
  ...
  ## sqldf error here about raw vectors  
    ...
    ##Mapping function
    #suppressWarnings(suppressMessages(
      map_rivseg <- fn_mapgen(mapnum = 2, style = styles[[map_style]], 
                               metric = rivseg_metric[i], segs = rsegs_sf,
                               featr_type = featr_type, origin_type = origin_type, 
                               origin = origin, bbox = bbox,  
                               counties = counties, roads = roads, 
                               nhd = nhd, maplabs = maplabs, 
...
```{r Display Map 2, out.width = '100%', out.height='100%', echo=FALSE}
if (show_map == TRUE) {
...
for (k in 1:length(rivseg_metric)){ #loop thru metrics
  ...
  tmp_df <- as.numeric(rsegs[,paste0('percentDiff','_',rivseg_metric[k])]) #rsegs is a df not sf
  rivTables[[k]][,paste0('pct_diff','_',rivseg_metric[k])] <- round(tmp_df, digits = 2)
  
  #order by lowest % difference from rivTables[[k]]
  rivdf <- rivTables[[k]] #for sqldf
  rivdf <- sqldf(paste0("SELECT rivdf.* FROM rivdf ORDER BY ", 'pct_diff','_',rivseg_metric[k], " ASC")) 
  names(rivdf)[names(rivdf) == paste0('pct_diff','_',rivseg_metric[k])] <- '% Difference'
  ...
  rivtablemetric <- paste0(rivseg_metric[k]) #define title here for mapping so each map has diff metric title, pass into function
  ...
  if (rivseg_metric[k]=="l30_Qout"){
      ...
      rivtablelabel <- rivtablemetric
  }
  
  rivtabletitle <- paste0(title, ", ", rivtablelabel)
  
  ft2 <- fn_tablegen(featr_type = "riverseg", table = rivdf,
                     columns = "all", alignment = "left", 
                     metric = rivseg_metric[k], origin_type = origin_type,  
                     origin = origin , title = rivtabletitle) #flextable function
...
### Table: 
```{r View Table 2, out.width = '100%', out.height='100%', results='asis', echo=FALSE}
