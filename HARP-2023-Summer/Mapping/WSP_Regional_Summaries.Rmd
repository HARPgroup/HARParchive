...
if (show_map == TRUE) {
  ...
  for (i in 1:length(run_config$riverseg_metrics)){ #something going wrong in rsegs df within this loop 
     ...
     display_map <- run_config$riverseg_metrics[[k]]$show_map
     if (display_map == TRUE) {
...
for (k in 1:length(run_config$riverseg_metrics)){
  ...
  if (data_set == "rseg") {
    ...
    table_cols <- unlist(run_config$riverseg_metrics[[k]]$tables_cols_r)
    display_column <- run_config$riverseg_metrics[[k]]$column_name
  }
  if (data_set == "facil") {
    rseg_display <- facils_nogeom
    table_cols <- unlist(run_config$riverseg_metrics[[k]]$tables_cols_f)
    display_column <- NA
  }
