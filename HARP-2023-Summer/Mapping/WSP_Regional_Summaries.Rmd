...
params: 
  ...
  show_map: FALSE #either TRUE or FALSE
---
...
if (origin_type == "basin") {
  ...
  title_main <- paste0(origin, " Locality")
} else if (origin_type == "region") {
  title_main <- paste0(origin, " Region")
} 
...
ft1 <- fn_tablegen(featr_type = "facility", table = table, 
                   ...
                   origin = origin, tabletitle = title_main ) #execute flextable function
...
if (show_map == TRUE) {
  ...
  for (i in 1:length(run_config$riverseg_metrics)){ #something going wrong in rsegs df within this loop 
    ...
    rivmaptitle <- paste0(title_main, ", ", rivmaplabel)
...
for (k in 1:length(run_config$riverseg_metrics)){ #loop thru metrics
  ...
  rivtabletitle <- paste0(title_main, ", ", rivtablelabel)
  
  ft2 <- fn_tablegen(featr_type = "riverseg", table = rivdf,
                     columns = "all", alignment = "left", 
                     metric = display_metric, origin_type = origin_type,  
                     origin = origin , tabletitle = rivtabletitle) #flextable function
