...
source(paste0(github_location,"/HARParchive/HARP-2023-Summer/Mapping/Config/mapstyle_config.R")) #load general mapping aesthetics 
...
for (k in 1:length(run_config$riverseg_metrics)){
  ...
  if (exists(data_set)) {
    ...
    #table_featr_type definition should be replaced with styles
    if (data_set == 'facils_nogeom') {
      table_featr_type <- "facility"
    } else {
      table_featr_type <- "riverseg"
    }
    
    ft2 <- fn_tablegen(featr_type = table_featr_type, table = rivTables[[k]],
                       columns = "all", alignment = "left", 
                       metric = display_metric, origin_type = origin_type,  
                       origin = origin , tabletitle = rivtabletitle,
                       num=rivtablenum) #flextable function
