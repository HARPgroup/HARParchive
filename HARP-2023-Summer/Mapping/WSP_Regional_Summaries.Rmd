...
###Note: Facilities constructed or otherwise made known to DEQ after the initial plan submission are not show in 2020 scenario
...
potomac_int <- st_intersection(origin_shape,st_as_sfc(st_bbox(potomac)))
aquia_int <- st_intersection(origin_shape,st_as_sfc(st_bbox(aquia)))
pineypoint_int <- st_intersection(origin_shape,st_as_sfc(st_bbox(potomac)))

## First checks all of them, to determine if there is any intersection. If so, add section title
if (any(nrow(potomac_int) > 0, nrow(aquia_int) > 0, nrow(pineypoint_int) > 0) & show_map == TRUE) {
  
  cat('\n### Groundwater Critical Cell Maps \n')
  
}

## Then check the individual ones to add in maps
### Potomac
if (nrow(potomac_int) > 0 & show_map == TRUE) {
  ...
  knitr::include_graphics(paste0(export_path,origin,'_',aquifer,'.png'))
  
}
### Aquia
if (nrow(aquia_int) > 0 & show_map == TRUE) {
  ## If there is no intersection, then the length of the output is 0, so cant check the value
  #### So it only gets here if there IS intersection
...
## This line tells it to start formatting (in this case landscape)
...
### Modeling Summary Maps and Tables: 
