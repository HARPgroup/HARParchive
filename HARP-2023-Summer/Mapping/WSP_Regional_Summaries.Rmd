...
#source(paste0(HARParchive_location,"/HARP-2023-Summer/Mapping/Functions/fn_mapgen.R"))
...
if (show_map == TRUE) {
  rm(maplabs)
  maplabs <- fn_labelprep(data=list(counties, cities, roads, nhdlabs),
                          classes=c("county","city","road","nhd"))
}
```
  ...
  for (i in 1:length(featrs_file_map_bubble_column)) { #execute for each metric in featrs_file_map_bubble_column param 
   ...
   if (i==1) { #data for table will be sorted by the first of featrs_file_map_bubble_column
    ...
    map <- fn_mapgen(bbox, crs_default, metric_unit, mp_layer, featr_type, 
                     maptitle, mapnum=1, rseg_leg_title=NULL, map_server, map_layer, 
                     maplabs, nhd, roads, rsegs, map_style, styles)
...
#----Map 2----
  ...
  ## sqldf error here about raw vectors  
    ...
    map_rivseg <- fn_mapgen(bbox, crs_default, metric_unit, mp_layer, featr_type, 
                       maptitle = rivmaptitle, mapnum=2, rseg_leg_title, map_server, map_layer, 
                       maplabs, nhd, roads, rsegs=rsegs_sf, map_style, styles)
...
```{r Generate Errors Summary, echo=FALSE, eval=FALSE}
...
maperrors <- data.frame(envir_name=character(0),maptitle=character(0),errors=character(0))
for(i in 1:length(allmaps)){
  if(!is.null(allmaps[[i]][["errors"]])){
    maperrors <- rbind(maperrors,
                       data.frame(envir_name = names(allmaps[i]),
...
if(nrow(maperrors)!=0){
  tbl_maperrors <- mk_par(x = flextable(maperrors), j = c("envir_name","maptitle"),
                          i = ~ duplicated(maperrors$envir_name),
                          value = as_paragraph('')
                          )
  tbl_maperrors <- set_table_properties(ft_1, layout = "autofit")
}
