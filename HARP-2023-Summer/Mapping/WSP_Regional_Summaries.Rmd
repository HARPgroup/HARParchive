...
params: 
  ...
  origin: ["MiddleJames_2"] #supply either a rivseg, locality, or region name ## JL7_7070_0001
  origin_type: ["region"] #basin, locality, or region
  featr_type: ["facility"] #source or facility
  
  featrs_file: "C:/aa_HARP/Exports/MiddleJames_2_featrs_sf.csv"
  ...
  rsegs_file: "C:/aa_HARP/Exports/MiddleJames_2_rsegs_sf.csv"
  ...
  runid_list: ["runid_11", "runid_13", "runid_17"] #list of runids used for facil and rseg data
...
#----Map 2----
  ...
  for (i in 1:length(run_config$riverseg_metrics)){ #something going wrong in rsegs df within this loop 
    ...
    if (display_metric=="l30_Qout"){
      ...
      ##!!! Change this to just be run set label:
  ...
  ## sqldf error here about raw vectors  
    ...
    map_rivseg <- fn_mapgen(bbox, crs_default, metric_unit, mp_layer, featr_type, 
                       maptitle = rivmaptitle, mapnum=2, rseg_leg_title=display_title, 
...
# todo: set appropriate base paths for this narrative in model run_set
md_base_path = run_config$md_base_path
if (is.null(md_base_path)) {
...
for (k in 1:length(run_config$riverseg_metrics)){
  ...
  if (!is.null(run_config$riverseg_metrics[[k]]$intro_md_narrative)) {
    f_narrative = run_config$riverseg_metrics[[k]]$intro_md_narrative
    # this was a draft to allow region specific narrative
    #narrative_fname = paste0(md_base_path, "/", as.character( f_narrative), "_", origin, '.md' )
    # THis is for a generic, by section narrative.
    narrative_fname = paste0(md_base_path, "/", as.character( f_narrative))
    
    # atext <- try(paste(readLines(narrative_fname), collapse="\n"), silent=TRUE)
    # if (!class(atext) == "try-error") {
    #   message(paste("Could not download", narrative_fname))
    # } else {
    #   cat(paste(atext, "\n\n"))
    # }
    atext <- paste(readLines(narrative_fname), collapse="\n")
    cat(paste(atext, "\n\n"))
  }
