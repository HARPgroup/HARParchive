...
ggplot(RRwinter, aes(x=month, y=WmonthMean, color=as.factor(year))) + geom_point(size=2) + xlab("Winter Months")+ ylab("Winter Month Flow Mean(cfs)")+theme_minimal() + guides(color= guide_legend(title = "Years")) +geom_line()+ggtitle("Mean Flow for Winter(2000-Present)")+scale_x_continuous(breaks=seq(1,12,by=1))
...
boxplot(RRwinter$WmonthMean, ylab = 'Monthly magnitude (cfs)', main = 'Monthly Mean Winter', ylim= c(0,10000))
boxplot(RRspring$SmonthMean, ylab = 'Monthly magnitude (cfs)', main = 'Monthly Mean Spring', ylim= c(0,10000))
```
