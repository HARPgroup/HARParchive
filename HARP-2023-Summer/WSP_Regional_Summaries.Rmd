...
params: 
  ...
  origin: ["Culpeper"] #supply either a rivseg, locality, or region name ## JL7_7070_0001
...
ft1 <- fn_tablegen(featr_type = "facility", table = table, columns = "all", alignment = "left", origin_type, metric = rivseg_metric[i], origin, title=title ) #Execute flextable function, which saves the flextable as variable 'ft' for displaying later
...
for (i in 1:length(rivseg_metric)){
  ...
  ##Mapping function
  suppressWarnings(suppressMessages(
    fn_mapgen2(mapnum = 2,featr_type, origin_type, styles[[map_style]], metric = rivseg_metric[i], origin, bbox, rsegs, counties, roads, 
          nhd, maplabs, origin, origin, mp_layer, metric_unit, title=title) ))
...
#names(table) <- mgsub(names(table), pattern="_", replacement=" ")
 ...
 # rivTables[[k]][,paste0('Difference')] = round( st_drop_geometry(rsegs[,paste0('diff','_',rivseg_metric[k])]) ,digits= 2)
  ...
  ft2 <- fn_tablegen(type = "riverseg", table = rivdf, columns = "all", alignment = "left", origin_type,  metric = rivseg_metric[k], origin, title=title) #Flextable function
