...
if (is.null(run_config)) {
  ...
  for (j in rivseg_metric) {
    for (i in 2:length(runid_list)) {
      n = n + 1
      run_config$riverseg_metrics[[n]] = list(metric = j, column_name = paste(j,runid_list[1],runid_list[i],sep="_"))
    }
  }
} else {
  # we got a valid run_config so load riverseg_metrics AND generate rivseg_metric
  riverseg_metrics = run_config$riverseg_metrics
  rivseg_metric = c()
...
source(paste0(github_location,"/HARParchive/HARP-2023-Summer/fn_mapgen.R"),local = TRUE) #load mapping function
...
source(paste0(github_location,"/HARParchive/HARP-2023-Summer/mapstyle_config.R"),local = TRUE) #load general mapping aesthetics 
...
regions_file <- paste0(github_location, '/HARParchive/HARP-2023-Summer/regions_sf.csv')
counties_file <- paste0(github_location, '/HARParchive/HARP-2023-Summer/counties_sf.csv')
cities_file <- paste0(github_location, '/HARParchive/HARP-2023-Summer/cities.csv')
roads_file <- paste0(github_location, '/HARParchive/HARP-2023-Summer/roads_sf.csv')
...
if (show_map == TRUE) {
  for (i in 1:length(rivseg_metric)){ #something going wrong in rsegs df within this loop 
    display_metric <- riverseg_metrics[[i]]$metric
    display_column <- riverseg_metrics[[i]]$column_name
  ...
  ####
    ...
    pct_col <- as.data.frame(rsegs_nogeom[,display_column])  #identify pct diff for metric of interest
  ...
  ## sqldf error here about raw vectors  
    ...
    #suppressWarnings(suppressMessages(
      map_rivseg <- fn_mapgen(mapnum = 2, style = styles[[map_style]], 
                               metric = display_metric, segs = rsegs_sf,
                               featr_type = featr_type, origin_type = origin_type, 
                               origin = origin, bbox = bbox,  
                               counties = counties, roads = roads, 
                               nhd = nhd, maplabs = maplabs, 
  ...
  for (i in 1:length(rivseg_metric)) {
    rivmapfilename[i] <- paste(export_path, map_name, "_", display_metric, ".png", sep="")
...
for (k in 1:length(rivseg_metric)){ #loop thru metrics
  display_metric <- riverseg_metrics[[k]]$metric
  display_column <- riverseg_metrics[[k]]$column_name
  rivTables[[k]] <- data.frame(Name = rsegs$name) # add rivseg name
  rivTables[[k]][,paste0('Riverseg')] <- rsegs$riverseg # add rivseg number
  rivTables[[k]]$Metric <- display_metric #add metric

   for (i in 1:nrow(rsegs)) {
        for (j in 1:length(runid_list)){ #add actual metric column data
          tmp_df <- st_drop_geometry(rsegs[i,paste0(runid_list[j],'_',display_metric)])
  ...
  tmp_df <- as.numeric(rsegs[,display_column]) #rsegs is a df not sf
  rivTables[[k]][,paste0('pct_diff','_',display_metric)] <- round(tmp_df, digits = 2)
  
  #order by lowest % difference from rivTables[[k]]
  rivdf <- rivTables[[k]] #for sqldf
  rivdf <- sqldf(paste0("SELECT rivdf.* FROM rivdf ORDER BY ", 'pct_diff','_',display_metric, " ASC")) 
  names(rivdf)[names(rivdf) == paste0('pct_diff','_',display_metric)] <- '% Difference'
