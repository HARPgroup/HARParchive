...
#rivseg, locality, region based on desired location for planner, ensure syntax is correct
  rivseg: ["JL7_7070_0001"] #test rivsegs ["JL6_7320_7150","JL6_6890_6990","JL6_7430_7320","JA2_7410_7470", "JA4_7280_7340","JB3_6820_7053"] 
  locality: ["Stafford"]
  region: ["Chowan_1"]    
...
sqldf_sf <- function(statemt, geomback="NA"){
  ...
  if(geomback!="NA"){
    output <- merge(dfs[["result"]], .GlobalEnv[[geomback]], all.x=TRUE) #match the newly filtered obs. to their geometries
...
#note: bottom of this chunk also includes some rsegs geoprocessing necessary for pairing facilities with watersheds. Therefore, this chunk needs to stay above the rsegs filtering chunk.
if(type == "source"){
  facils <- with(facils, 
...
facils <- st_join(facils, rsegs[ ,c("riverseg",geoCol(rsegs)) ]) #pairs riverseg column from rsegs w/ facils based on geometry
```
...
```{r Filter Cities & Roads, echo=FALSE, message=FALSE, warning=FALSE, eval=FALSE}
#----Cities----
...
```{r Filter Rsegs by Boundary Type, echo=FALSE, message=FALSE, warning=FALSE, eval=FALSE}
sf_use_s2(FALSE) # switch off Spherical geometry ; some functions (eg. st_filter) give errors without this
if (map_type=="basin") { #finding upstream riversegs for basin maps 
  for(i in rivseg){
    if(i==rivseg[1]){
    basin <- fn_extract_basin(data.frame(rsegs), i) 
    }
    if(i!=rivseg[1]){
    basin <- rbind(basin, fn_extract_basin(data.frame(rsegs), i))
    }
  }
  rsegs <- st_as_sf( basin[!duplicated(basin$riverseg),] ,wkt=geoCol(basin), crs=4326) #convert back to sf & don't duplicate riversegs for overlapping basins
  rm(basin)
...
```{r Filter Sources/Facils by Basin or Boundary, echo=FALSE, message=FALSE, warning=FALSE, eval=FALSE}
...
# Pulling Feature Metrics Post-Filtering (Cleanup)
```{r Non-Modeled Facility Metrics, echo=FALSE, message=FALSE, warning=FALSE, eval=FALSE}
for (i in 1:nrow(facils)) {
  facils[i,metric_feat] <- RomProperty$new(ds,list(
                                featureid = facils$featureid[i],
                                propname = metric_feat),
                              TRUE)$propvalue #pull feature & directly assign metric propvalue to facility i
}
```

```{r Rseg Drought Metrics, echo=FALSE, message=FALSE, warning=FALSE, eval=FALSE}
...
```{r Calculate Rseg Metric % Diff, echo=FALSE, message=FALSE, warning=FALSE, eval=FALSE}
