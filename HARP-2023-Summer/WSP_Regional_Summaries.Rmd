...
params: 
  ...
  origin: ["JL7_7070_0001"] #supply either a rivseg, locality, or region name ## JL7_7070_0001
  origin_type: ["basin"] #basin, locality, or region
...
for (i in 1:length(featrs_file_table_column)){
  ...
  if (featrs_file_table_column[i] %in% colnames(facils)) {
  #adding requested columns to table
    ...
    tmp_df <- st_drop_geometry(facils[,featrs_file_table_column[i]])
    tmp_df <- as.data.frame(lapply(tmp_df,as.numeric))
    table[,featrs_file_table_column[i]] <- round(tmp_df, digits = 2)
  }
}
...
#names(table) <- mgsub(names(table), pattern="_", replacement=" ")
 ...
 # rivTables[[k]][,paste0('Difference')] = round( st_drop_geometry(rsegs[,paste0('diff','_',rivseg_metric[k])]) ,digits= 2)
  ...
  #rivdf <- sqldf(paste('SELECT *, ','pct_diff','_',rivseg_metric[k],' AS pct_diff 
  #       FROM rivdf' , sep="")) 
  
  rivdf <- sqldf(paste0("SELECT rivdf.* FROM rivdf ORDER BY ", 'pct_diff','_',rivseg_metric[k], " ASC")) #ordering by the metric of interest, descending 
