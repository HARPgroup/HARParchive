...
params: 
  facils_file: "C:/Users/nrf46657/Desktop/GitHub/plots/JU4_7330_7000_all_csvs/26_facils_sf.csv"
  facils_file_bubble_column: ["fiveyr_avg_mgy"]
  
  rsegs_file: "C:/Users/nrf46657/Desktop/GitHub/plots/JU4_7330_7000_all_csvs/28_rsegs_sf.csv"
  counties_file: "C:/Users/nrf46657/Desktop/GitHub/plots/JU4_7330_7000_all_csvs/11_counties_sf.csv"
  cities_file: "C:/Users/nrf46657/Desktop/GitHub/plots/JU4_7330_7000_all_csvs/22_cities.csv"
  roads_file: "C:/Users/nrf46657/Desktop/GitHub/plots/JU4_7330_7000_all_csvs/23_roads_sf.csv"
...
for (i in 1:length(facils_file_bubble_column)) { #execute for each metric in facils_file_bubble_column param 
  #legend_title <- paste0(facils_file_bubble_column[i])
  legend_title <- legend_titling(facils_file_bubble_column[i], runid_list_facilities)
  
  #add column of NAs when no metric col exists, when no facilities with the desired metric exist in the extent
  if ((facils_file_bubble_column[i] %in% colnames(facils)) == FALSE) {
    facils[facils_file_bubble_column[i]] <- NA
  ...
  "SELECT facils.*  
  FROM facils
  ORDER BY", ' ', facils_file_bubble_column[i], ' ', "DESC"), #ordering by the metric of interest, descending 
  "facils")
  facils$NUM <- seq(1, nrow(facils))
  #st_write(facils, paste0(export_path,"34_facils_sf.csv"), layer_options = "GEOMETRY=AS_WKT")
 if (i==1) { #data for table will be sorted by the first of facils_file_bubble_column
   #facils$within <- facils_within
 } 
  
  metric_unit <- substr(facils_file_bubble_column[i], nchar(facils_file_bubble_column[i]) - 2, nchar(facils_file_bubble_column[i]))  #get metric unit from last 3 characters of metric 
  ...
  mp_layer_sql <- paste('SELECT *, ',facils_file_bubble_column[i],' AS demand_metric
  ...
  names(mp_layer)[names(mp_layer) == 'demand_metric'] <- facils_file_bubble_column[i] #change column name back to the specific metric
...
##Mapping function
  ...
  suppressWarnings(suppressMessages( 
    fn_mapgen2(mapnum = 1, type, map_type, styles[[map_style]], metric = facils_file_bubble_column[i], rivseg, bbox, rsegs, counties, roads, 
...
# for (i in 1:length(facils_file_bubble_column)) {
#   mapfilename[i] <- paste(export_path, map_name, "_", facils_file_bubble_column[i], ".png", sep="")
...
for (i in 1:length(table_col)){
  ...
  #adding requested columns to table
    ...
    tmp_df <- st_drop_geometry(facils[,table_col[i]])
    tmp_df <- as.data.frame(lapply(tmp_df,as.numeric))
    table[,table_col[i]] <- round(tmp_df, digits = 2)
  }
}
...
# table <- st_read("C:/Users/nrf46657/Desktop/GitHub/plots/JU4_7330_7000_all_csvs/44_table.csv")
# table_sql <- sqldf_sf( #sorting the data into bins based on their range 
#       "SELECT field_1 AS '',
#               Number,
#               Facility,
#               HydroID,
#               Locality,     
#               River.Segment.Name AS 'River Segment Name',
#               River.Segment.ID AS 'River Segment ID',
#               Permitted.Capacity AS 'Permitted Capacity',
#               runid.11.wd.mgd AS 'runid 11 wd.mgd',
#               runid.13.wd.mgd AS 'runid 13 wd.mgd',
#               X5.yr.Avg.Use..mgy. AS '5-yr Avg Use (mgy)',
#               wsp2020.2040.mgy AS 'wsp2020 2040 mgy',
#       from table")
# table <- sqldf(table_sql)
  ...
  #write.csv(rivTables[[k]], paste0(export_path,"50_rivTables_",rivseg_metric[k],".csv"))
   ...
   for (i in 1:nrow(rsegs)) {
        for (j in 1:length(runid_list_riversegs)){ #add actual metric column data
          ...
          tmp_df <- st_drop_geometry(rsegs[i,paste0(runid_list_riversegs[j],'_',rivseg_metric[k])])
          tmp_df <- as.data.frame(lapply(tmp_df,as.numeric))
          rivTables[[k]][i,paste0(runid_list_riversegs[j])] <- round(tmp_df, digits = 2)
