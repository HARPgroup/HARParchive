...
##Mapping function
  ...
  #suppressWarnings(suppressMessages( 
    map <- fn_mapgen2(mapnum = 1, style = styles[[map_style]], 
...
####
  ...
  rsegs2 <- st_drop_geometry(rsegs)
  pct_col <- as.data.frame(rsegs2[,paste0('percentDiff_', rivseg_metric[i])]) #identify pct diff for metric of interest
  colnames(pct_col) <- 'pct_diff'
  rsegs <- cbind(rsegs2, pct_col)
  rm(rsegs2)
  ##
...
## sqldf error here about raw vectors  
  ...
  rsegs_sf <- st_as_sf(rsegs, wkt = 'WKT') #this function removes WKT col, so create new var to prevent loop error
  st_crs(rsegs_sf) <- crs_default

  ##Mapping function
  #suppressWarnings(suppressMessages(
    map_rivseg <- fn_mapgen2(mapnum = 2, style = styles[[map_style]], 
  ...
  rsegs$pct_diff <- NULL #remove column
