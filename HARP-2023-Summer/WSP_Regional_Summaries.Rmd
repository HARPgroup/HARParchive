...
params: 
  ...
  featrs_file: "C:/aa_HARP/Exports/JL7_7070_0001_mp_sf.csv"
  featrs_file_map_bubble_column: ["wd_mgd"]
  featrs_file_table_column: ["runid_11_wd_mgd","runid_13_wd_mgd","fiveyr_avg_mgy","wsp2020_2040_mgy"]

  rsegs_file: "C:/aa_HARP/Exports/JL7_7070_0001_rsegs_sf.csv"
...
for (i in 1:length(featrs_file_map_bubble_column)) { #execute for each metric in featrs_file_map_bubble_column param 
 ...
 if (i==1) { #data for table will be sorted by the first of featrs_file_map_bubble_column
  ...
  metric_unit <- substr(featrs_file_map_bubble_column[i], nchar(featrs_file_map_bubble_column[i]) - 2, nchar(featrs_file_map_bubble_column[i]))  #get metric unit from last 3 characters of metric 
...
##Mapping function
  ...
  suppressWarnings(suppressMessages( 
    map <- fn_mapgen2(mapnum = 1, style = styles[[map_style]], 
                metric = featrs_file_map_bubble_column[i], featr_type = featr_type, 
                origin_type = origin_type, origin = origin, 
                bbox = bbox, segs = rsegs, 
...
for (i in 1:length(rivseg_metric)){
  ...
  suppressWarnings(suppressMessages(
    map_rivseg <- fn_mapgen2(mapnum = 2, style = styles[[map_style]], 
                             metric = rivseg_metric[i], featr_type = featr_type, 
                             origin_type = origin_type, origin = origin, 
                             bbox = bbox, segs = rsegs, 
                             counties = counties, roads = roads, 
                             nhd = nhd, maplabs = maplabs, 
