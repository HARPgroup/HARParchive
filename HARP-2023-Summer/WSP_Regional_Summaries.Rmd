...
#determining which type/level of map is being created
  type: ["source"] #either 'source' or 'facility'
...
```{r (New) Get Permitted Capacity, echo=FALSE}
...
```{r Display Rivseg Map, out.width = '100%', out.height='100%', echo=FALSE}
...
for (k in 1:length(rivseg_metric)){
  ...
  for (i in 1:nrow(nacols)) {
    if (nacols$colSums.is.na.rivdf......nrow.rivdf.[i] == TRUE){ #if column is all na, then return true
      tablemessage2 = TRUE
    }
...
```{r View Rivseg Flextable, out.width = '100%', out.height='100%', results='asis', echo=FALSE}
