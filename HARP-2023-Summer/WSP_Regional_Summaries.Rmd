...
params: 
  ...
  featrs_file: "C:/HARP/Exports/JL7_7070_0001_featrs_sf1.csv"
  ...
  rsegs_file: "C:/HARP/Exports/JL7_7070_0001_rsegs_sf1.csv" 
...
library(pandoc)
#library(ggmap)
#register_google(key ="AIzaSyBvRzhfQk7nrOUtesvnHusWaOKcBhZ9DAM", write = TRUE)
...
#names(table) <- mgsub(names(table), pattern="_", replacement=" ")
 ...
 # rivTables[[k]][,paste0('Difference')] = round( st_drop_geometry(rsegs[,paste0('diff','_',rivseg_metric[k])]) ,digits= 2)
  ...
  tmp_df <- as.data.frame(lapply(tmp_df,as.numeric))
  rivTables[[k]][,paste0('pct_diff')] <- round(pct_diff, digits = 2) #possible error here?
  ...
  ft2 <- fn_tablegen(featr_type = "riverseg", table = rivdf,
                     columns = "all", alignment = "left", 
                     origin_type, metric = rivseg_metric[k], 
                     origin, title=title) #flextable function
