...
```{r Pull Non-Modeled Facility Metrics, echo=FALSE, message=FALSE, warning=FALSE, eval=FALSE}
...
```{r Pull Permitted Capacity, echo=FALSE, message=FALSE, warning=FALSE, eval=FALSE}
fac_model_data <- data.frame()
for (i in facils$hydroid ){
  ...
  fac_model_data <- rbind( fac_model_data ,  # [[ runid_list_facilities[1] ]]
                                                        read.csv(paste(site,"/model-summary-users-export-all-cols/",
                                                            gsub(" ","",toString( i )),"/",
                                                            runid_list_facilities[1],"/", #just use 1st runid b/c perm.cap doesn't change
                                                            gsub(" ","",toString( metric_mod )),"/", #user's model metric
                                                            gsub(" ","",toString( model_props )),sep=""))
                                                          )
}

statemt <- paste("SELECT a.*, b.permit_status, c.perm_cap_mgy
                  FROM facils as a
                  LEFT OUTER JOIN (
                  ...
                  LEFT OUTER JOIN (
                    SELECT facility_hydroid, model_prop_propcode as perm_cap_mgy
                    FROM fac_model_data
                    WHERE (model_prop_propname == 'vwp_max_mgy')
                    ) as c
                  ON (a.featureid = c.facility_hydroid)
                 ")


                  
test <- sqldf_sf(statemt, geomback="facils")
test2 <- test[!duplicated(test),] #remove duplicated rows 

test4 <- test3[test3$Facility == facils$Facility,]
what <- test[test$featureid == facils$featureid,]
hm <- unique(facils$hydroid)

#something wacky is going on here

```

```{r Pull Rseg Drought Metrics, echo=FALSE, message=FALSE, warning=FALSE, eval=FALSE}
