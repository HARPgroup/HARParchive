...
params: 
  ...
  origin: ["Giles"] #supply either a rivseg, locality, or region name ## JL7_7070_0001
  origin_type: ["locality"] #basin, locality, or region
  featr_type: ["source"] #source or facility
  
  featrs_file: "~/Desktop/HARPteam23/Giles_mp_sf.csv"
  featrs_file_map_bubble_column: ["runid_11_wd_mgd"] #runid & metric
  featrs_file_table_column: ["five_yr_avg","wsp2020_2040_mgy"]
...
if (show_map == TRUE) {
  ...
  class(facils[,featrs_file_table_column[1]]) <- "numeric"
...
for (i in 1:length(featrs_file_table_column)){
  ...
  #adding requested columns to table
    table[,featrs_file_table_column[i]] <- round(as.numeric(facils[,featrs_file_table_column[i]]), digits = 2)
  }
}
