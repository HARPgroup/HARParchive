...
params: 
  ...
  origin: ["Shenandoah_1"] #supply either a rivseg, locality, or region name ## JL7_7070_0001
  origin_type: ["region"] #basin, locality, or region
  featr_type: ["facility"] #source or facility
  
  featrs_file: "~/Desktop/HARPteam23/Shenandoah_1_featrs_sf.csv"
  featrs_file_map_bubble_column: ["runid_11_wd_mgd"] #runid & metric
  featrs_file_table_column: ["runid_11_wd_mgd","runid_13_wd_mgd","fiveyr_avg_mgy","wsp2020_2040_mgy"]

  rsegs_file: "~/Desktop/HARPteam23/Shenandoah_1_rsegs_sf.csv" 
...
if (show_map == TRUE) {
  for (i in 1:length(featrs_file_map_bubble_column)) { #execute for each metric in featrs_file_map_bubble_column param 
   ...
   if (i==1) { #data for table will be sorted by the first of featrs_file_map_bubble_column
    ...
    title1 <- paste0(featrs_file_map_bubble_column[i]) #define title here for mapping so each map has diff metric title, pass into function
    
    if (featrs_file_map_bubble_column[i]=="fiveyr_avg_mgy"){
...
table$`Permitted Capacity` <- facils_table$vwp_max_mgy #add permitted capacity to table 
...
for (k in 1:length(rivseg_metric)){ #loop thru metrics
  # metcol_name <- paste0(runid_list[j],'_',rivseg_metric[k])
  # Alt. 1:
  # rivTables[[k]] <- rsegs[,c('name','riverseg', metcol_name]
  # rivTables[[k]]$Metric <- rivseg_metric[k]
  # Alt 2:
  # rivTables[[k]] <- sqldf(
  #    paste0(
  #      "select name, riverseg, '",rivseg_metric[k],"' as Metric, ",
  #      metcol_name,
  #      " from rsegs"
  #   )
  #W )
  ...
  ft2 <- fn_tablegen(featr_type = "riverseg", table = rivdf,
                     ...
                     origin = origin , title=title) #flextable function
