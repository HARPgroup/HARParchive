...
df_storage <- data.frame(
  ...
  'runlabel' = c(paste0('SminL30mg_',runid, '_apx'), paste0('SminL90mg_', runid, '_apx'))
)
...
df_metrics <- data.frame(
  ...
  'runlabel' = c(paste0('l30_Qout_', runid), paste0('l90_Qout_', runid))
)
...
```{r Smin Calculations, message=FALSE, warning=FALSE}
...
for (i in 1:nrow(storage_data)) {
  ...
  storage_data$SminL30mg_approx[i] <- Smin_L30_approx_acf / 3.069
  storage_data$SminL90mg_approx[i] <- Smin_L90_approx_acf / 3.069
  ...
  storage_data$SminL30mg_nearexact[i] <- min(l30pd_df$Storage) / 3.069
  storage_data$SminL90mg_nearexact[i] <- min(l90pd_df$Storage) / 3.069
...
storage_byseg_30_apx <- sqldf(paste0("SELECT storage_data.riverseg, storage_data.SminL30mg_",runid,"_apx,
                       SUM(SminL30mg_",runid,"_apx) AS SminL30mg_",runid,"_apx_sum
                       FROM storage_data
                       GROUP BY riverseg
                       "))

#Sum qpprox SminL90 by riverseg
storage_byseg_90_apx <- sqldf(paste0("SELECT storage_data.riverseg, storage_data.SminL90mg_",runid,"_apx,
                       SUM(SminL90mg_",runid,"_apx) AS SminL90mg_",runid,"_apx_sum
                       FROM storage_data
                       GROUP BY riverseg
                       "))

#Join approx SminL30 and SminL90 summed storage
storage_byseg_apx <- sqldf(paste0("SELECT a.riverseg, a.SminL30mg_",runid,"_apx_sum, b.SminL90mg_",runid,"_apx_sum
...
storage_byseg_30_nex <- sqldf(paste0("SELECT storage_data.riverseg, storage_data.SminL30mg_nearexact,
                       SUM(SminL30mg_nearexact) AS SminL30mg_",runid,"_nex_sum
                       FROM storage_data
                       GROUP BY riverseg
                       "))

# #Sum near-exact SminL90 by riverseg
storage_byseg_90_nex <- sqldf(paste0("SELECT storage_data.riverseg, storage_data.SminL90mg_nearexact,
                       SUM(SminL90mg_nearexact) AS SminL90mg_",runid,"_nex_sum
                       FROM storage_data
                       GROUP BY riverseg
                       "))

#Join near-exact SminL30 and SminL90 summed storage
storage_byseg_nex <- sqldf(paste0("SELECT a.riverseg, a.SminL30mg_",runid,"_nex_sum, b.SminL90mg_",runid,"_nex_sum
...
storage_byseg <- sqldf(paste0("SELECT a.*,  b.SminL30mg_",runid,"_nex_sum, b.SminL90mg_",runid,"_nex_sum
...
```{r Join Storage, Metric, and Baseline Data}
...
metric_data <- sqldf(paste0('SELECT a.*, b.SminL30mg_', runid, '_apx_sum, b.SminL90mg_', runid, '_apx_sum, b.SminL30mg_', runid, '_nex_sum, b.SminL90mg_', runid, '_nex_sum
...
metric_data <- sqldf(paste0('SELECT a.*, b.l30_Qout_base, b.l90_Qout_base
                   FROM metric_data as a
...
for (i in 1:nrow(metric_data)) {
  ...
  if (nrow(ups_imp) > 0) {
    ...
    metric_data$SminL30mg_apx_total[i] <- sum(ups_df$SminL30mg_11_apx_sum) #sum storage available locally and upstream
    metric_data$SminL90mg_apx_total[i] <- sum(ups_df$SminL90mg_11_apx_sum) 
    metric_data$SminL30mg_nex_total[i] <- sum(ups_df$SminL30mg_11_nex_sum) 
    metric_data$SminL90mg_nex_total[i] <- sum(ups_df$SminL90mg_11_nex_sum)
...
## Perform unit conversions when plugging values into WA equation 
