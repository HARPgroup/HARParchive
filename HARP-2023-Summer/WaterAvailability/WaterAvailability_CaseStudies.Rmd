...
```{r Case Study Names, eval=FALSE}
#Get case study names for plot and map titling 
#get case study names for titling 
for (i in 1:length(outlet_list)) {
  #can we also display the impoundment-specific name from thee storage data?
  caseStudyName_rseg <- gsub('[0-9]+', '', caseStudies_df$propname[i])
  caseStudyName_rseg <- gsub('[[:punct:] ]+', ' ', caseStudyName_rseg)
  caseStudyName_rseg <- gsub('Impoundment', '', caseStudyName_rseg)
  
  caseStudyName_storage <- gsub('[0-9]+', '', storage_data[storage_data$riverseg == outlet_list[i], "propname"])
  caseStudyName_storage <- gsub('[[:punct:] ]+', ' ', caseStudyName_storage)
  caseStudyName_storage <- gsub('Impoundment', '', caseStudyName_storage)
  
  #remove 1 name if they're duplicated 
   if (length(caseStudyName_storage) != 0) { 
     if (caseStudyName_storage == caseStudyName_rseg) {
      caseStudyName_storage <- character()
      }
    }
   
  
  if (length(caseStudyName_storage) != 0) {
    caseStudyName <- paste0(caseStudyName_storage, " / ", caseStudyName_rseg)
  } else {
    caseStudyName <- caseStudyName_rseg
  }
  
  caseStudyName <- trimws(caseStudyName, which = "both")#remove leading space 
  
  assign(paste0('caseStudyName', i), caseStudyName)
}
...
cat(knitr::knit_print(ftable_WA)) #view flextable in knitted doc
```
...
```{r View Case Study Maps and Tables, out.width = '100%', out.height='100%', results='asis', echo=FALSE, eval=FALSE}
for (i in 1:length(outlet_list)){
  #L30 map
  fig_L30 = paste(paste0("![](",WAmapfilename30[i],")"),"\n")
  cat(fig_L30)
  
  #L30 table
  cat(knitr::knit_print(get(paste0('ftable_cswsL30_', i))))
  cat("\n\n\\pagebreak\n")
  
  #L90 map
  fig_L90 = paste(paste0("![](",WAmapfilename90[i],")"),"\n")
  cat(fig_L90)
  
  #L90 table
  cat(knitr::knit_print(get(paste0('ftable_cswsL90_', i))))
  cat("\n\n\\pagebreak\n")
...
rivseg_pct_vect <- c(0,10,50,100,500,1000,10000) #last value should be higher than any value expected, since classification is done using <=
