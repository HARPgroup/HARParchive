...
params: 
  outlet_list: ["PL3_5360_5250", "JU4_7000_7300", "JA5_7480_0001", "YP3_6670_6720", "OR3_7740_8271", "PU4_3970_3890"]
  demand_scenario: ["runid_11"] 
  baseline_scenario: ["runid_0"] 
  mif_coef: [0.9]
  Smin_analysis: TRUE #to compare Smin calculation methods: approximate and near-exact (adds tables 5 & 6)
  write_Qavailable: FALSE #to export Qavailable metrics to VAhydro
  write_outputs: ["local", "github"]
  crs_default: 4326
...
knitr::opts_chunk$set(echo = FALSE, message = FALSE, warning = FALSE, fig.cap = TRUE, out.width='100%', out.height = '100%') #set chunk options
...
if (Smin_analysis == TRUE) { #Only needed for Smin analysis 
...
```{r Smin Calculations}
##Only needed for Smin analysis 
if (Smin_analysis == TRUE) {
  ...
  for (i in 1:nrow(storage_data)) {
    ...
    if (storage_data$imp_off[i] == 0) {
      ...
      dat$Storage <- dat[,storagecol] #Storage col now represents usable storage
      ## All Storage is now in units of mg 
      ...
      #Near-Exact Smin 
        ...
        #Storage column in units of million gallons (set to usable storage (mg) variable earlier in script)
      storage_data$SminL30mg_nex[i] <- round(min(l30pd_df$Storage), digits = 3)
      storage_data$SminL90mg_nex[i] <- round(min(l90pd_df$Storage), digits = 3)
      
      #Approximate Smin 
        #Lowest storage within the low flow years 
        #Storage column in units of million gallons (set to usable storage (mg) variable earlier in script)
      storage_data$SminL30mg_apx[i] <- round(min(l30yr_df$Storage), digits = 3)
      storage_data$SminL90mg_apx[i] <- round(min(l90yr_df$Storage), digits = 3)
...
} else { #if Smins aren't calculated, use VAhydro metrics 
  storage_data$SminL30mg_apx <- storage_data$SminL30mg_vah
  storage_data$SminL90mg_apx <- storage_data$SminL90mg_vah
}
```
...
if (Smin_analysis == TRUE) { #Only needed for Smin analysis 
  ...
  storage_byseg <- storage_byseg_apx
}
```
...
if (Smin_analysis == TRUE) { #Only needed for Smin analysis 
  ...
  metric_data <- sqldf('SELECT a.*, b.SminL30mg_apx_local, b.SminL90mg_apx_local
                     FROM metric_data as a
                     LEFT OUTER JOIN storage_byseg as b
                     ON (a.riverseg = b.riverseg)')
}
...
for (i in 1:nrow(metric_data)) {
  ...
  if (nrow(ups_imp) > 0) {
    ...
    metric_data$SminL30mg_apx_ups[i] <- round(sum(ups_df$SminL30mg_apx_local) - metric_data$SminL30mg_apx_local[i], digits = 3)
    metric_data$SminL90mg_apx_ups[i] <- round(sum(ups_df$SminL90mg_apx_local) - metric_data$SminL90mg_apx_local[i], digits = 3)
  }
...
metric_data$WA_mgd_30_apx = round((metric_data$Qavailable_30_cfs / 1.547) + 
  (metric_data$SminL30mg_apx_total / 30), digits = 3)

metric_data$WA_mgd_90_apx = round((metric_data$Qavailable_90_cfs / 1.547) + 
...
```{r Case Studies Dataframe}
## Case Studies Dataframe
caseStudies_df <- data.frame()

for (i in 1:length(outlet_list)) {
  caseStudyRow <- metric_data[grep(outlet_list[i], metric_data$riverseg), ] #get row from metric data for case study
  caseStudies_df <- rbind(caseStudies_df, caseStudyRow)
}


#Remove duplicate rows
caseStudies_df <- caseStudies_df[!duplicated(caseStudies_df), ]
```

```{r Case Study Watersheds Dataframes}
#Create dataframes for case study watersheds (upstream from each of outlet_list)
for (i in 1:length(outlet_list)) {
  ups_data <- fn_extract_basin(metric_data, outlet_list[i])
  if (outlet_list[i] == "OR3_7740_8271") { #order Carvins / Upper Roanoke Wshed by cumulative withdrawal due to old naming conventions 
    ups_data <- sqldf( #ordering the data using sqldf 
     "SELECT ups_data.*  
     FROM ups_data
     ORDER BY wd_cumulative_mgd ASC") 
  }
  ups_data$NUM <- seq(1, nrow(ups_data)) #number rows
  assign(paste0('caseStudy_wshed_df_', i), ups_data)
}
...
table_WA <- data.frame(Propname = df_WA$propname,
                     ...
                     wd_cumulative_mgd = round(df_WA$wd_cumulative_mgd, digits = 2),
                     ps_cumulative_mgd = round(df_WA$ps_cumulative_mgd, digits = 2),
                     l30_Qout_mgd = round(df_WA$l30_Qout_dem / 1.547, digits = 2),
                     l30_Qout_base_mgd = round(df_WA$l30_Qout_base / 1.547, digits = 2),
                     l30_Qout_mif_mgd = round(df_WA$l30_Qout_mif / 1.547, digits = 2),
                     Qavailable_30_mgd = round(df_WA$Qavailable_30_cfs / 1.547, digits = 2),
                     l90_Qout_mgd = round(df_WA$l90_Qout_dem / 1.547, digits = 2),
                     l90_Qout_base_mgd = round(df_WA$l90_Qout_base / 1.547, digits = 2),
                     l90_Qout_mif_mgd = round(df_WA$l90_Qout_mif / 1.547, digits = 2),
                     Qavailable_90_mgd = round(df_WA$Qavailable_90_cfs / 1.547, digits = 2),
...
#Table for flextable with human readable column names 
table_WAhr <- table_WA

#Human readable column names for flextable 
names(table_WAhr)[names(table_WAhr) == "wd_cumulative_mgd"] <- "Cumulative Withdrawal (mgd)"
names(table_WAhr)[names(table_WAhr) == "ps_cumulative_mgd"] <- "Cumulative Point Source (mgd)"
names(table_WAhr)[names(table_WAhr) == "l30_Qout_mgd"] <- "Lowest 30-Day Demand Flow (mgd)"
names(table_WAhr)[names(table_WAhr) == "l30_Qout_base_mgd"] <- "Lowest 30-Day Base Flow (mgd)"
names(table_WAhr)[names(table_WAhr) == "l30_Qout_mif_mgd"] <- "30-Day Min. Instream Flow (mgd)"
names(table_WAhr)[names(table_WAhr) == "Qavailable_30_mgd"] <- "30-Day Available Flow (mgd)"
names(table_WAhr)[names(table_WAhr) == "l90_Qout_mgd"] <- "Lowest 90-Day Demand Flow (mgd)"
names(table_WAhr)[names(table_WAhr) == "l90_Qout_base_mgd"] <- "Lowest 90-Day Base Flow (mgd)"
names(table_WAhr)[names(table_WAhr) == "l90_Qout_mif_mgd"] <- "90-Day Min. Instream Flow (mgd)"
names(table_WAhr)[names(table_WAhr) == "Qavailable_90_mgd"] <- "90-Day Available Flow (mgd)"
names(table_WAhr)[names(table_WAhr) == "SminL30mg_apx_total"] <- "30-Day Total Storage (mg)"
names(table_WAhr)[names(table_WAhr) == "SminL90mg_apx_total"] <- "90-Day Total Storage (mg)"
names(table_WAhr)[names(table_WAhr) == "WA_mgd_30_apx"] <- "30-Day Water Availability (mgd)"
names(table_WAhr)[names(table_WAhr) == "WA_mgd_90_apx"] <- "90-Day Water Availability (mgd)"

ftable_WA <- flextable(table_WAhr) #create flextable
ftable_WA <- add_header_lines(ftable_WA, values= title_table_WA) #add title 
ftable_WA #view flextable in console
```
...
```{r Table 2}
title_table_caseStudies <- paste0('TABLE 2: CASE STUDIES. Demand: ', demand_scenario, ', Baseline: ', baseline_scenario, ', MIF: ', mif_coef)

table_caseStudies <- data.frame(Propname = caseStudies_df$propname,
                      Riverseg = caseStudies_df$riverseg,
                      wd_cumulative_mgd = round(caseStudies_df$wd_cumulative_mgd, digits = 2),
                      ps_cumulative_mgd = round(caseStudies_df$ps_cumulative_mgd, digits = 2),
                      l30_Qout_mgd = round(caseStudies_df$l30_Qout_dem / 1.547, digits = 2),
                      l30_Qout_base_mgd = round(caseStudies_df$l30_Qout_base / 1.547, digits = 2),
                      l30_Qout_mif_mgd = round(caseStudies_df$l30_Qout_mif / 1.547, digits = 2),
                      Qavailable_30_mgd = round(caseStudies_df$Qavailable_30_cfs / 1.547, digits = 2),
                      l90_Qout_mgd = round(caseStudies_df$l90_Qout_dem / 1.547, digits = 2),
                      l90_Qout_base_mgd = round(caseStudies_df$l90_Qout_base / 1.547, digits = 2),
                      l90_Qout_mif_mgd = round(caseStudies_df$l90_Qout_mif / 1.547, digits = 2),
                      Qavailable_90_mgd = round(caseStudies_df$Qavailable_90_cfs / 1.547, digits = 2),
                      SminL30mg_apx_total = caseStudies_df$SminL30mg_apx_total,
                      SminL90mg_apx_total = caseStudies_df$SminL90mg_apx_total,
                      WA_mgd_30_apx = caseStudies_df$WA_mgd_30_apx,
                      WA_mgd_90_apx = caseStudies_df$WA_mgd_90_apx)

table_caseStudiesHr <- table_caseStudies #table for flextable with human readable column names 

#Human readable column names for flextable 
names(table_caseStudiesHr)[names(table_caseStudiesHr) == "wd_cumulative_mgd"] <- "Cumulative Withdrawal (mgd)"
names(table_caseStudiesHr)[names(table_caseStudiesHr) == "ps_cumulative_mgd"] <- "Cumulative Point Source (mgd)"
names(table_caseStudiesHr)[names(table_caseStudiesHr) == "l30_Qout_mgd"] <- "Lowest 30-Day Demand Flow (mgd)"
names(table_caseStudiesHr)[names(table_caseStudiesHr) == "l30_Qout_base_mgd"] <- "Lowest 30-Day Base Flow (mgd)"
names(table_caseStudiesHr)[names(table_caseStudiesHr) == "l30_Qout_mif_mgd"] <- "30-Day Min. Instream Flow (mgd)"
names(table_caseStudiesHr)[names(table_caseStudiesHr) == "Qavailable_30_mgd"] <- "30-Day Available Flow (mgd)"
names(table_caseStudiesHr)[names(table_caseStudiesHr) == "l90_Qout_mgd"] <- "Lowest 90-Day Demand Flow (mgd)"
names(table_caseStudiesHr)[names(table_caseStudiesHr) == "l90_Qout_base_mgd"] <- "Lowest 90-Day Base Flow (mgd)"
names(table_caseStudiesHr)[names(table_caseStudiesHr) == "l90_Qout_mif_mgd"] <- "90-Day Min. Instream Flow (mgd)"
names(table_caseStudiesHr)[names(table_caseStudiesHr) == "Qavailable_90_mgd"] <- "90-Day Available Flow (mgd)"
names(table_caseStudiesHr)[names(table_caseStudiesHr) == "SminL30mg_apx_total"] <- "30-Day Total Storage (mg)"
names(table_caseStudiesHr)[names(table_caseStudiesHr) == "SminL90mg_apx_total"] <- "90-Day Total Storage (mg)"
names(table_caseStudiesHr)[names(table_caseStudiesHr) == "WA_mgd_30_apx"] <- "30-Day Water Availability (mgd)"
names(table_caseStudiesHr)[names(table_caseStudiesHr) == "WA_mgd_90_apx"] <- "90-Day Water Availability (mgd)"


ftable_caseStudies <- flextable(table_caseStudiesHr) #create flextable
ftable_caseStudies <- add_header_lines(ftable_caseStudies, values= title_table_caseStudies) #add title 

ftable_caseStudies
```

# Table 3: Case Study Watersheds

```{r Table 3}
#separate flextables for each case study with case study rseg in header 
for (i in 1:length(outlet_list)) { 
  title_table_caseStudyWshed <- paste0('TABLE 3: CASE STUDY WATERSHEDS. Demand: ', 
                                       demand_scenario, ', Baseline: ', baseline_scenario, 
                                       ', MIF: ', mif_coef, ', Case Study: ', outlet_list)
  csws_df <- get(paste0('caseStudy_wshed_df_', i))
  csws_df <- csws_df[!duplicated(csws_df), ]#remove duplicate rows
  table_caseStudyWshed <-   data.frame(Num = csws_df$NUM,
                      Propname = csws_df$propname,
                      Riverseg = csws_df$riverseg,
                      wd_cumulative_mgd = round(csws_df$wd_cumulative_mgd, digits = 3),
                      ps_cumulative_mgd = round(csws_df$ps_cumulative_mgd, digits = 3),
                      l30_Qout_mgd = round(csws_df$l30_Qout_dem / 1.547, digits = 3),
                      l30_Qout_base_mgd = round(csws_df$l30_Qout_base / 1.547, digits = 3),
                      l30_Qout_mif_mgd = round(csws_df$l30_Qout_mif / 1.547, digits = 3),
                      Qavailable_30_mgd = round(csws_df$Qavailable_30_cfs / 1.547, digits = 2),
                      l90_Qout_mgd = round(csws_df$l90_Qout_dem / 1.547, digits = 3),
                      l90_Qout_base_mgd = round(csws_df$l90_Qout_base / 1.547, digits = 3),
                      l90_Qout_mif_mgd = round(csws_df$l90_Qout_mif / 1.547, digits = 3),
                      Qavailable_90_mgd = round(csws_df$Qavailable_90_cfs / 1.547, digits = 2),
  ...
  table_caseStudyWshedHr <- table_caseStudyWshed #table for flextable with human readable column names 

  #Human readable column names for flextable 
  names(table_caseStudyWshedHr)[names(table_caseStudyWshedHr) == "wd_cumulative_mgd"] <- "Cumulative Withdrawal (mgd)"
  names(table_caseStudyWshedHr)[names(table_caseStudyWshedHr) == "ps_cumulative_mgd"] <- "Cumulative Point Source (mgd)"
  names(table_caseStudyWshedHr)[names(table_caseStudyWshedHr) == "l30_Qout_mgd"] <- "Lowest 30-Day Demand Flow (mgd)"
  names(table_caseStudyWshedHr)[names(table_caseStudyWshedHr) == "l30_Qout_base_mgd"] <- "Lowest 30-Day Base Flow (mgd)"
  names(table_caseStudyWshedHr)[names(table_caseStudyWshedHr) == "l30_Qout_mif_mgd"] <- "30-Day Min. Instream Flow (mgd)"
  names(table_caseStudyWshedHr)[names(table_caseStudyWshedHr) == "Qavailable_30_mgd"] <- "30-Day Available Flow (mgd)"
  names(table_caseStudyWshedHr)[names(table_caseStudyWshedHr) == "l90_Qout_mgd"] <- "Lowest 90-Day Demand Flow (mgd)"
  names(table_caseStudyWshedHr)[names(table_caseStudyWshedHr) == "l90_Qout_base_mgd"] <- "Lowest 90-Day Base Flow (mgd)"
  names(table_caseStudyWshedHr)[names(table_caseStudyWshedHr) == "l90_Qout_mif_mgd"] <- "90-Day Min. Instream Flow (mgd)"
  names(table_caseStudyWshedHr)[names(table_caseStudyWshedHr) == "Qavailable_90_mgd"] <- "90-Day Available Flow (mgd)"
  names(table_caseStudyWshedHr)[names(table_caseStudyWshedHr) == "SminL30mg_total"] <- "30-Day Total Storage (mg)"
  names(table_caseStudyWshedHr)[names(table_caseStudyWshedHr) == "SminL30mg_local"] <- "30-Day Local Storage (mg)"
  names(table_caseStudyWshedHr)[names(table_caseStudyWshedHr) == "SminL30mg_upstream"] <- "30-Day Upstream Storage (mg)"
  names(table_caseStudyWshedHr)[names(table_caseStudyWshedHr) == "SminL90mg_total"] <- "90-Day Total Storage (mg)"
  names(table_caseStudyWshedHr)[names(table_caseStudyWshedHr) == "SminL90mg_local"] <- "90-Day Local Storage (mg)"
  names(table_caseStudyWshedHr)[names(table_caseStudyWshedHr) == "SminL90mg_upstream"] <- "90-Day Upstream Storage (mg)"
  names(table_caseStudyWshedHr)[names(table_caseStudyWshedHr) == "WA_mgd_30"] <- "30-Day Water Availability (mgd)"
  names(table_caseStudyWshedHr)[names(table_caseStudyWshedHr) == "WA_mgd_90"] <- "90-Day Water Availability (mgd)"
  
  ftable_csws <- flextable(table_caseStudyWshedHr)
  rownum_cs <- which(csws_df$riverseg == outlet_list[i]) #highlight case study row
  ...
  assign(paste0('ftable_caseStudyWshed_', outlet_list[i]), ftable_csws)
  print(get(paste0('ftable_caseStudyWshed_', outlet_list[i])))
  
  if ("local" %in% write_outputs) {
    write.csv(table_caseStudyWshed, paste0(export_path, 
                                           'CaseStudies_table3.', i, '_CaseStudyWshed_', 
                                           outlet_list[i], '_', 
                                           demand_scenario, '_', baseline_scenario, 
                                           '_', mif_coef, '.csv')) #write csv locally
    
    flextable::save_as_image(ftable_csws, path = paste0(export_path, 
                                                      'CaseStudies_table3.', i, '_CaseStudyWsheds_',
                                                      outlet_list[i], '_',
                                                      demand_scenario, '_', baseline_scenario,
  ...
  if ("github" %in% write_outputs) {
    write.csv(table_caseStudyWshed, paste0(github_location, 
                                           '/HARParchive/HARP-2023-Summer/WaterAvailability/CaseStudiesTables/CSVs/CaseStudies_table3.', i, 
                                           '_CaseStudyWshed_', outlet_list[i], '_', demand_scenario, '_', 
                                           baseline_scenario, '_', mif_coef, '.csv')) #write csv to GitHub
  
    flextable::save_as_image(ftable_csws, path = paste0(github_location, 
                                           '/HARParchive/HARP-2023-Summer/WaterAvailability/CaseStudiesTables/flextables/CaseStudies_table3.', i, 
                                           '_CaseStudyWshed_', outlet_list[i], '_', demand_scenario, '_', 
                                           baseline_scenario, '_', mif_coef, '.png')) #save image of flextable locally
    }
}

```

# Table 4: L30/L90 Specific Case Study Watershed Tables 

```{r Table 4: L30/L90 Case Study Watershed Tables}
#to be displayed with WA maps 

for (i in 1:length(outlet_list)) { 
  csws_df <- get(paste0('caseStudy_wshed_df_', i))
  
  #L30 Case Study Watershed Tables
  table_cswsL30 <-   data.frame(Num = csws_df$NUM,
                      Propname = csws_df$propname,
                      Riverseg = csws_df$riverseg,
                      wd_cumulative_mgd = round(csws_df$wd_cumulative_mgd, digits = 3),
                      ps_cumulative_mgd = round(csws_df$ps_cumulative_mgd, digits = 3),
                      ...
                      l30_Qout_base_mgd = round((csws_df$l30_Qout_base / 1.547), digits = 3),
                      l30_Qout_mif_mgd = round((csws_df$l30_Qout_mif/ 1.547), digits = 3),
                      Qavailable_30_mgd = round(csws_df$Qavailable_30_cfs / 1.547, digits = 3),
                      SminL30mg_local = csws_df$SminL30mg_apx_local,
                      SminL30mg_upstream = csws_df$SminL30mg_apx_ups,
                      SminL30mg_total = csws_df$SminL30mg_apx_total,
                      WA_mgd_30 = csws_df$WA_mgd_30_apx)

  table_cswsL30Hr <- table_cswsL30 #table for flextable with human readable column names 
  
  #Human readable column names 
  names(table_cswsL30Hr)[names(table_cswsL30Hr) == "Num"] <- "#"
  names(table_cswsL30Hr)[names(table_cswsL30Hr) == "wd_cumulative_mgd"] <- "Cumulative Withdrawal (mgd)"
  names(table_cswsL30Hr)[names(table_cswsL30Hr) == "ps_cumulative_mgd"] <- "Cumulative Point Source (mgd)"
  names(table_cswsL30Hr)[names(table_cswsL30Hr) == "l30_Qout_mgd"] <- "Lowest 30-Day Demand Flow (mgd)"
  names(table_cswsL30Hr)[names(table_cswsL30Hr) == "l30_Qout_base_mgd"] <- "Lowest 30-Day Base Flow (mgd)"
  names(table_cswsL30Hr)[names(table_cswsL30Hr) == "l30_Qout_mif_mgd"] <- "Min. Instream Flow (mgd)"
  names(table_cswsL30Hr)[names(table_cswsL30Hr) == "Qavailable_30_mgd"] <- "Available Flow (mgd)"
  names(table_cswsL30Hr)[names(table_cswsL30Hr) == "SminL30mg_local"] <- "Local Storage (mg)"
  names(table_cswsL30Hr)[names(table_cswsL30Hr) == "SminL30mg_upstream"] <- "Upstream Storage (mg)"
  names(table_cswsL30Hr)[names(table_cswsL30Hr) == "SminL30mg_total"] <- "Total Storage (mg)"
  names(table_cswsL30Hr)[names(table_cswsL30Hr) == "WA_mgd_30"] <- "Water Availability (mgd)"
  
  ftable_cswsL30 <- flextable(table_cswsL30Hr)
  rownum_csL30 <- which(csws_df$riverseg == outlet_list[i]) #get rownum for case study
  ftable_cswsL30 <- flextable::bg(ftable_cswsL30, i = rownum_csL30 , bg = "yellow") #highlight case study row
  colnum_csL30 <- which(colnames(table_cswsL30Hr) == "Water Availability (mgd)") #get col number for WA
  ftable_cswsL30 <- flextable::bg(ftable_cswsL30, j = colnum_csL30 , bg = "yellow") #highlight WA column 
  assign(paste0('ftable_cswsL30_', i), ftable_cswsL30)
  
  #L90 Case Study Watershed Tables
  table_cswsL90 <-   data.frame(Num = csws_df$NUM,
                      Propname = csws_df$propname,
                      Riverseg = csws_df$riverseg,
                      wd_cumulative_mgd = round(csws_df$wd_cumulative_mgd, digits = 3),
                      ps_cumulative_mgd = round(csws_df$ps_cumulative_mgd, digits = 3),
                      ...
                      l90_Qout_base_mgd = round((csws_df$l90_Qout_base / 1.547), digits = 3),
                      l90_Qout_mif_mgd = round((csws_df$l90_Qout_mif / 1.547), digits = 3),
                      Qavailable_90_mgd = round(csws_df$Qavailable_90_cfs / 1.547, digits = 3),
                      SminL90mg_local = csws_df$SminL90mg_apx_local,
                      SminL90mg_upstream = csws_df$SminL90mg_apx_ups,
                      SminL90mg_total = csws_df$SminL90mg_apx_total,
                      WA_mgd_90 = csws_df$WA_mgd_90_apx)
  
  table_cswsL90Hr <- table_cswsL90 #table for flextable with human readable column names 
  
  #Human readable column names 
  names(table_cswsL90Hr)[names(table_cswsL90Hr) == "Num"] <- "#"
  names(table_cswsL90Hr)[names(table_cswsL90Hr) == "wd_cumulative_mgd"] <- "Cumulative Withdrawal (mgd)"
  names(table_cswsL90Hr)[names(table_cswsL90Hr) == "ps_cumulative_mgd"] <- "Cumulative Point Source (mgd)"
  names(table_cswsL90Hr)[names(table_cswsL90Hr) == "l90_Qout_mgd"] <- "Lowest 90-Day Demand Flow (mgd)"
  names(table_cswsL90Hr)[names(table_cswsL90Hr) == "l90_Qout_base_mgd"] <- "Lowest 90-Day Base Flow (mgd)"
  names(table_cswsL90Hr)[names(table_cswsL90Hr) == "l90_Qout_mif_mgd"] <- "Min. Instream Flow (mgd)"
  names(table_cswsL90Hr)[names(table_cswsL90Hr) == "Qavailable_90_mgd"] <- "Available Flow (mgd)"
  names(table_cswsL90Hr)[names(table_cswsL90Hr) == "SminL90mg_local"] <- "Local Storage (mg)"
  names(table_cswsL90Hr)[names(table_cswsL90Hr) == "SminL90mg_upstream"] <- "Upstream Storage (mg)"
  names(table_cswsL90Hr)[names(table_cswsL90Hr) == "SminL90mg_total"] <- "Total Storage (mg)"
  names(table_cswsL90Hr)[names(table_cswsL90Hr) == "WA_mgd_90"] <- "Water Availability (mgd)"
  
  ftable_cswsL90 <- flextable(table_cswsL90Hr)
  rownum_csL90 <- which(csws_df$riverseg == outlet_list[i]) #get rownum for case study
  ftable_cswsL90 <- flextable::bg(ftable_cswsL90, i = rownum_csL90 , bg = "yellow") #highlight case study row
  colnum_csL90 <- which(colnames(table_cswsL90Hr) == "Water Availability (mgd)") #get col number for WA
  ftable_cswsL90 <- flextable::bg(ftable_cswsL90, j = colnum_csL90 , bg = "yellow") #highlight WA column 
  assign(paste0('ftable_cswsL90_', i), ftable_cswsL90)
  
  #save both tables 
  
  if ("local" %in% write_outputs) {
    write.csv(table_cswsL30, paste0(export_path, 
                                           'CaseStudies_table4.', i, '_CaseStudyWshed_L30_', 
                                           outlet_list[i], '_', 
                                           demand_scenario, '_', baseline_scenario, 
                                           '_', mif_coef, '.csv')) #write table locally
    write.csv(table_cswsL90, paste0(export_path, 
                                           'CaseStudies_table4.', i, '_CaseStudyWshed_L90_', 
                                           outlet_list[i], '_', 
                                           demand_scenario, '_', baseline_scenario, 
                                           '_', mif_coef, '.csv')) #write table locally
    
    flextable::save_as_image(ftable_cswsL30, path = paste0(export_path, 
                                                      'CaseStudies_table4.', i, '_CaseStudyWshed_L30_',
                                                      outlet_list[i], '_',
                                                      demand_scenario, '_', baseline_scenario,
                                                      '_', mif_coef, '.png')) #save image of flextable locally
  
    flextable::save_as_image(ftable_cswsL90, path = paste0(export_path, 
                                                      'CaseStudies_table4.', i, '_CaseStudyWshed_L90_',
                                                      outlet_list[i], '_',
                                                      demand_scenario, '_', baseline_scenario,
                                                      '_', mif_coef, '.png')) #save image of flextable locally
  ...
  if ("github" %in% write_outputs) {
    write.csv(table_cswsL30, paste0(github_location, 
                               '/HARParchive/HARP-2023-Summer/WaterAvailability/CaseStudiesTables/CSVs/CaseStudies_table4.', i, '_CaseStudyWshed_L30_', 
                               outlet_list[i], '_', demand_scenario, '_', baseline_scenario, '_', mif_coef, '.csv')) #write table to GitHub
    
    write.csv(table_cswsL90, paste0(github_location, 
                               '/HARParchive/HARP-2023-Summer/WaterAvailability/CaseStudiesTables/CSVs/CaseStudies_table4.', i, '_CaseStudyWshed_L90_', 
                               outlet_list[i], '_', demand_scenario, '_', baseline_scenario, '_', mif_coef, '.csv')) #write table to GitHub
    
    flextable::save_as_image(ftable_cswsL30, path = paste0(github_location, 
                               '/HARParchive/HARP-2023-Summer/WaterAvailability/CaseStudiesTables/flextables/CaseStudies_table4.', i, '_CaseStudyWshed_L30_', 
                               outlet_list[i], '_', demand_scenario, '_', baseline_scenario, '_', mif_coef, '.png')) #save image of flextable to Github
    
    flextable::save_as_image(ftable_cswsL90, path = paste0(github_location, 
                               '/HARParchive/HARP-2023-Summer/WaterAvailability/CaseStudiesTables/flextables/CaseStudies_table4.', i, '_CaseStudyWshed_L90_', 
                               outlet_list[i], '_', demand_scenario, '_', baseline_scenario, '_', mif_coef, '.png')) #save image of flextable to Github
...
```{r Table 5}
if (Smin_analysis == TRUE) { #Only needed for Smin analysis  
  title_table_Storage <- paste0('TABLE 5: SMIN METHOD COMPARISON. Demand: ', demand_scenario, ', Baseline: ', baseline_scenario, ', MIF: ', mif_coef)
  ...
  #Table for flextable with human readable column names 
  table_StorageHr <- table_Storage
  
  #Human readable column names for flextable 
  names(table_StorageHr)[names(table_StorageHr) == "SminL30mg_apx"] <- "30-Day Storage (approx, mg)"
  names(table_StorageHr)[names(table_StorageHr) == "SminL30mg_nex"] <- "30-Day Storage (near-exact, mg)"
  names(table_StorageHr)[names(table_StorageHr) == "SminL90mg_apx"] <- "90-Day Storage (approx, mg)"
  names(table_StorageHr)[names(table_StorageHr) == "SminL90mg_nex"] <- "90-Day Storage (near-exact, mg)"
  names(table_StorageHr)[names(table_StorageHr) == "min_in_pd30"] <- "Storage Methods Equal for 30-Day?"
  names(table_StorageHr)[names(table_StorageHr) == "min_in_pd90"] <- "Storage Methods Equal for 90-Day?"
  names(table_StorageHr)[names(table_StorageHr) == "outside_pd30"] <- "Days Separating 30-Day Storage Methods"
  names(table_StorageHr)[names(table_StorageHr) == "outside_pd90"] <- "Days Separating 90-Day Storage Methods"
  names(table_StorageHr)[names(table_StorageHr) == "pctDiff30"] <- "% Difference of Storage Methods (30-day)"
  names(table_StorageHr)[names(table_StorageHr) == "pctDiff90"] <- "% Difference of Storage Methods (90-day)"
  names(table_StorageHr)[names(table_StorageHr) == "apx_lessthan_nex_30"] <- "Is Approx. Method Lowest? (30-day)"
  names(table_StorageHr)[names(table_StorageHr) == "apx_lessthan_nex_90"] <- "Is Approx. Method Lowest? (90-day)"
  
  ftable_Storage <- flextable(table_StorageHr) #create flextable
  ...
  #quantify occurences of matching Smin method results
  equal_Smins <- sum(storage_data$outside_sum == 0)
  Smin_message <- paste0("Smin methods resulted in matching values ", equal_Smins, " out of ", nrow(storage_data), " occurences")
  
  ftable_Storage <- flextable::bg(ftable_Storage, i = rownums , bg = "yellow") #add row highlight
  ftable_Storage <- flextable::add_footer_lines(ftable_Storage, values = Smin_message) #add footer
...
## This has been incorporated into table 5 so temporarliy disabled the chunk
...
#Table for flextable with human readable column names 
table_SminHr <- table_Smin

#Human readable column names for flextable 
names(table_SminHr)[names(table_SminHr) == "SminL30mg_apx"] <- "30-Day Storage (approx, mg)"
names(table_SminHr)[names(table_SminHr) == "SminL30mg_nex"] <- "30-Day Storage (near-exact, mg)"
names(table_SminHr)[names(table_SminHr) == "SminL90mg_apx"] <- "90-Day Storage (approx, mg)"
names(table_SminHr)[names(table_SminHr) == "SminL90mg_nex"] <- "90-Day Storage (near-exact, mg)"
names(table_SminHr)[names(table_SminHr) == "apx_lessthan_nex_30"] <- "Is Approx. Method Lowest? (30-day)"
names(table_SminHr)[names(table_SminHr) == "apx_lessthan_nex_90"] <- "Is Approx. Method Lowest? (90-day)"
names(table_SminHr)[names(table_SminHr) == "pctDiff30"] <- "% Difference of Storage Methods (30-day)"
names(table_SminHr)[names(table_SminHr) == "pctDiff90"] <- "% Difference of Storage Methods (90-day)"

ftable_Smin <- flextable(table_SminHr) #create flextable
...
# Table 6: Smins that Need to be Updated

```{r Table 6}
if (Smin_analysis == TRUE) { #Only needed for Smin analysis  
  title_table_update <- paste0('TABLE 6: SMINS NEEDING UPDATE. Demand: ', demand_scenario, ', Baseline: ', baseline_scenario, ', MIF: ', mif_coef)
  ...
  #Table for flextable with human readable column names 
  table_updateHr <- table_update
  
  #Human readable column names for flextable 
  names(table_updateHr)[names(table_updateHr) == "SminL30mg_vah"] <- "30-Day Storage (VAhydro, mg)"
  names(table_updateHr)[names(table_updateHr) == "SminL30mg_apx"] <- "30-Day Storage (calculated, mg)"
  names(table_updateHr)[names(table_updateHr) == "SminL90mg_vah"] <- "90-Day Storage (VAhydro, mg)"
  names(table_updateHr)[names(table_updateHr) == "SminL90mg_apx"] <- "90-Day Storage (calculated, mg)"
  
  ftable_update <- flextable(table_updateHr) #create flextable
...
```{r Create Q available Plots for L30/L90}
plot30_filenameL <- character()
plot90_filenameL <- character()
plot30_filenameG <- character()
plot90_filenameG <- character()

for (i in 1:length(outlet_list)) {
 
  #L30 graph
  plot30 <- ggplot(data = get(paste0('caseStudy_wshed_df_',i)), aes(x= reorder(riverseg, NUM))) +
    geom_bar(aes(y=l30_Qout_dem, fill = 'blue'), stat = 'identity') +
    geom_bar(aes(y=Qavailable_30_cfs, fill = 'green'), stat = 'identity') +
    theme(axis.text.x = element_text(size = 25, angle = 45, vjust = 1.0, hjust=1)) + #rotate x-axis labels
    theme(axis.text.y = element_text(size = 25)) + #enlarge y axis labels
    theme(axis.title.x=element_blank()) +
    theme(axis.title.y=element_text(size = 20)) +
    theme(legend.text = element_text(size = 35)) +
    ylab('cfs') + 
    scale_fill_manual(name = NULL, 
                      label = c("Demand Flow", "Available Flow"), 
                      values = c("blue","green")) +
    theme(legend.position="bottom") +
    theme(
      plot.title = element_markdown(size = 60),
      plot.subtitle = element_markdown(size = 40)) +
    labs(title = 'Available vs Demand Flow (30-day)',
       subtitle = paste0("For ", outlet_list[i], " <br> Available Flow = Demand Flow - Min. Instream Flow"))
    
  assign(paste0('plot30_', i), plot30, envir = globalenv())
  
  if ("local" %in% write_outputs) { #save image of plot locally
    plot30_filenameL[i] <- paste0(export_path, 'WAcaseStudies_Plot_', i, '_', outlet_list[i], '_QvsQavail_L30.png')
    ggsave( 
      filename = plot30_filenameL[i],
      plot = plot30,
      width = 25,
      height = 20)
  }
    
  if ("github" %in% write_outputs) { #save image of plot to GitHub
    plot30_filenameG[i] <- paste0(github_location, '/HARParchive/HARP-2023-Summer/WaterAvailability/CaseStudiesPlots/WAcaseStudies_Plot_',
                                  i, '_', outlet_list[i], '_QvsQavail_L30.png')
    ggsave( 
      filename = plot30_filenameG[i],
      plot = plot30,
      width = 25,
      height = 20)
  }
  ...
  #L90 graph
  plot90 <- ggplot(data = get(paste0('caseStudy_wshed_df_',i)), aes(x= reorder(riverseg, NUM))) +
    geom_bar(aes(y=l90_Qout_dem, fill = 'blue'), stat = 'identity') +
    geom_bar(aes(y=Qavailable_90_cfs, fill = 'green'), stat = 'identity') +
    theme(axis.text.x = element_text(size = 25, angle = 45, vjust = 1.0, hjust=1)) + #rotate x-axis labels
    theme(axis.text.y = element_text(size = 25)) + #enlarge y axis labels
    theme(axis.title.x=element_blank()) +
    theme(axis.title.y=element_text(size = 20)) +
    ...
    theme(
      plot.title = element_markdown(size = 60),
      plot.subtitle = element_markdown(size = 40)) +
    labs(title = 'Available vs Demand Flow (90-day)',
       subtitle = paste0("For ", outlet_list[i], " <br> Available Flow = Demand Flow - Min. Instream Flow"))
    
    assign(paste0('plot90_', i), plot90, envir = globalenv())
  
  if ("local" %in% write_outputs) { #save image of plot locally
    plot90_filenameL[i] <- paste0(export_path, 'WAcaseStudies_Plot_', i, '_', outlet_list[i], '_QvsQavail_L90.png')
    ggsave( 
      filename = plot90_filenameL[i],
      plot = plot90,
      width = 25,
      height = 20)
  }
  ...
  if ("github" %in% write_outputs) { #save image of plot to GitHub
    plot90_filenameG[i] <- paste0(github_location, '/HARParchive/HARP-2023-Summer/WaterAvailability/CaseStudiesPlots/WAcaseStudies_Plot_',
                                  i, '_', outlet_list[i], '_QvsQavail_L90.png')
    ggsave( 
      filename = plot90_filenameG[i],
      plot = plot90,
      width = 25,
      height = 20)
  }
...
```{r Binning for Mapping Water Availability}
#custom bins based on range of WA values
div <- c(-100,0,10,50,100,500,1000) # 7 divisions used in fn_mapgen()
bin_labs <- c("< 0", 
              "0 - 10", 
              "10 - 50",
              "50 - 100",
...
```{r Mapping Water Availability for L30 & L90}
#create spatial object of all rivsersegs to be trimmed
rsegs_all_sf <- sf::st_as_sf(rsegs_geom_data, wkt='geom', crs =  crs_default) #convert to sf

#map case study watersheds
WAmapfilename30L <- character()
WAmapfilename90L <- character()
WAmapfilename30G <- character()
WAmapfilename90G <- character()
for (i in 1:length(outlet_list)) {

  rsegs_df <- get(paste0('caseStudy_wshed_df_', i))
  
   #Binning each time we map based on metric of interest 
  entity_classes <- sqldf( #sorting the data into bins based on their range 
    ...
    on (a.WA_mgd_30_apx >= b.val) 
    group by a.NUM" , "rsegs_df")
    
  rsegs_df <- sqldf( #joining the bins with the metric data 
    "select a.*, b.bin 
    from rsegs_df as a 
    left outer join entity_classes as b 
    on a.NUM = b.NUM " , "rsegs_df")
  
  rsegs_df$bin[is.na(rsegs_df$bin)] <- "X" # replacing NA values in bin column
  ...
  rsegs_sf <- sf::st_as_sf(rsegs_df, wkt=fn_geoCol(rsegs_df), crs=4326) #convert to sf
  rsegs_sf <- fn_centroid_coords(rsegs_sf) #add centroid coordinates for labeling 
  
  #create bbox around rsegs of interest 
  sf_use_s2(FALSE) # switch off Spherical geometry
  bbox_sfc <- st_buffer(st_as_sfc(st_bbox(rsegs_sf)), .02) #slightly past basin
  ...
  #Get nhd data to map flowlines & waterbodies
  nhd  <- memo_plot_nhdplus(bbox = bbox, actually_plot = FALSE) 
  nhd$flowline <- fn_centroid_coords(nhd$flowline, geom_col = "geometry", crs = crs_default) #add centroid coordinates for labeling streams & rivers
  flowlines_large <- nhd$flowline[nhd$flowline$StreamOrde > 3, ]
  flowline_labs <- data.frame(name = flowlines_large$gnis_name,
                              geometry = flowlines_large$geometry,
                              lng = flowlines_large$lng,
                              lat = flowlines_large$lat)
  flowline_labs <- flowline_labs[!duplicated(flowline_labs$name), ]
  
  ## L30 map of Water Availability
  map30 <- ggplot(rsegs_bbox) + geom_sf() + #map all segments within bbox as background
    geom_sf(data = rsegs_sf, mapping = aes(fill = as.factor(bin)), color = "black", lwd = 1) +
    scale_fill_manual(values = c("firebrick2","darkorange",
                                 "white", "cadetblue1", "deepskyblue", "dodgerblue3", "blue"),
                      breaks = seq(1:7),
                      labels = bin_labs,
                      limits = as.factor(seq(1:7)),
                      name = "Water Availability (mgd)") +
    geom_sf(data = nhd$flowline, #map nhd flowlines
    ...
    geom_sf(data = rbind(nhd$off_network_wtbd, nhd$network_wtbd), #map nhd waterbodies
            inherit.aes=FALSE, fill= "dodgerblue", size=1) +
    geom_sf(data = rsegs_sf[rsegs_sf$riverseg == outlet_list[i],] ,
            fill = NA, color = "red", lwd = 2) + #highlight outline case study segment
    geom_text_repel(data = rsegs_sf, aes(x=lng, y=lat, label = NUM), size = 9) + #label segments 
    geom_text_repel(data = flowline_labs,
                    aes(x=lng, y=lat, label = name), size = 6)  + #label large rivers 
    ggtitle(paste0('Water Availability (30-Day) for ', outlet_list[i])) +
    theme(plot.title = element_text(size = 40),
          plot.subtitle = element_text(size = 20),
          legend.text = element_text(size = 25), 
          legend.title = element_text(size = 30)) + 
    coord_sf(xlim = bbox_points$long, ylim = bbox_points$lat, expand = FALSE) #set viewing window to bounding box 
  #plot(map30) #view map, helpful during testing to find errors 
  
  assign(paste0('map30', i), map30, envir = globalenv())
  
  if ("local" %in% write_outputs) { #save image of L30 map locally
    WAmapfilename30L[i] <- paste(export_path, "WAcaseStudiesMapL30_", i, "_", outlet_list[i], ".png", sep="")
    ggsave(
      filename = WAmapfilename30L[i],
      plot = map30,
      width = 25,
      height = 20)
  }
    
  if ("github" %in% write_outputs) { #save image of L30 map to GitHub
    WAmapfilename30G[i] <- paste0(github_location, '/HARParchive/HARP-2023-Summer/WaterAvailability/CaseStudiesMaps/WAcaseStudiesMapL30_', 
                                  i, "_", outlet_list[i], '.png')
    ggsave( 
      filename = WAmapfilename30G[i],
      plot = map30,
      width = 25,
      height = 20)
  }

  ## L90 map of Water Availability
  rsegs_df$bin <- NULL #remove bin col from L30 map 
  rsegs_df$geom <- NULL
  ...
  map90 <- ggplot(rsegs_bbox) + geom_sf() + #map all segments within bbox as background
    geom_sf(data = rsegs_sf, mapping = aes(fill = as.factor(bin)), color = "black", lwd = 1) +
    scale_fill_manual(values = c("firebrick2","darkorange",
                                 "white", "cadetblue1", "deepskyblue", "dodgerblue3", "blue"),
                      breaks = seq(1:7),
    ...
    geom_text_repel(data = flowline_labs,
                    aes(x=lng, y=lat, label = name), size = 6)  + #label large rivers 
    ggtitle(paste0('Water Availability (90-day) for ', outlet_list[i])) +
    theme(plot.title = element_text(size = 40),
          plot.subtitle = element_text(size = 20),
          legend.text = element_text(size = 25), 
          legend.title = element_text(size = 30)) + 
    coord_sf(xlim = bbox_points$long, ylim = bbox_points$lat, expand = FALSE) #set viewing window to bounding box 
  #plot(map90) #view map, helpful during testing to find errors 
  
  assign(paste0('map90', i), map90, envir = globalenv())
  
   if ("local" %in% write_outputs) { #save image of L90 map locally
    WAmapfilename90L[i] <- paste(export_path, "WAcaseStudiesMapL90_", i, "_", outlet_list[i], ".png", sep="")
    ggsave(
      filename = WAmapfilename90L[i],
      plot = map90,
      width = 25,
      height = 20)
  }
    
  if ("github" %in% write_outputs) { #save image of L90 map to GitHub
    WAmapfilename90G[i] <- paste0(github_location, '/HARParchive/HARP-2023-Summer/WaterAvailability/CaseStudiesMaps/WAcaseStudiesMapL90_', 
                                  i, "_", outlet_list[i], '.png')
    ggsave( 
      filename = WAmapfilename90G[i],
      plot = map90,
      width = 25,
      height = 20)
  }
...
if ("local" %in% write_outputs) {
  ...
  write.csv(table_caseStudies, paste0(export_path, 'CaseStudies_table2_CaseStudies_', 
                             demand_scenario, '_', baseline_scenario, '_', mif_coef, '.csv')) 
  
  #tables 3 and 4 exported when created because of loop structure 
  if (Smin_analysis == TRUE) { #Only needed for Smin analysis   
    write.csv(table_Storage, paste0(export_path, 'CaseStudies_table5_SminComparison_', 
                               demand_scenario, '_', baseline_scenario, '_', mif_coef, '.csv')) 
    
    write.csv(table_update, paste0(export_path, 'CaseStudies_table6_SminUpdate_', 
                               demand_scenario, '_', baseline_scenario, '_', mif_coef, '.csv')) 
...
if ("github" %in% write_outputs) {
  ...
  write.csv(table_caseStudies, paste0(github_location, '/HARParchive/HARP-2023-Summer/WaterAvailability/CaseStudiesTables/CSVs/CaseStudies_table2_CaseStudies_', 
                           demand_scenario, '_', baseline_scenario, '_', mif_coef, '.csv')) 
  
  #tables 3 and 4 exported when created because of loop structure 
  if (Smin_analysis == TRUE) { #Only needed for Smin analysis  
    write.csv(table_Storage, paste0(github_location, '/HARParchive/HARP-2023-Summer/WaterAvailability/CaseStudiesTables/CSVs/CaseStudies_table5_SminComparison_', 
                             demand_scenario, '_', baseline_scenario, '_', mif_coef, '.csv')) 
    
    write.csv(table_update, paste0(github_location, '/HARParchive/HARP-2023-Summer/WaterAvailability/CaseStudiesTables/CSVs/CaseStudies_table6_SminUpdate_', 
                             demand_scenario, '_', baseline_scenario, '_', mif_coef, '.csv')) 
   }
}
...
if ("local" %in% write_outputs) {
  #save flextable images locally 
  flextable::save_as_image(ftable_WA, path = paste0(export_path, 'CaseStudies_table1_WaterAvailabilityEqn_', 
                                                    demand_scenario, '_', baseline_scenario, '_', mif_coef, '.png')) 
  
  flextable::save_as_image(ftable_caseStudies, path = paste0(export_path, 'CaseStudies_table2_CaseStudies_', 
                                                    demand_scenario, '_', baseline_scenario, '_', mif_coef, '.png')) 
  
  #tables 3 and 4 exported when created because of loop structure 
  if (Smin_analysis == TRUE) { #Only needed for Smin analysis  
    flextable::save_as_image(ftable_Storage, path = paste0(export_path, 'CaseStudies_table5_SminComparison_', 
                                                      demand_scenario, '_', baseline_scenario, '_', mif_coef, '.png')) 
    
    flextable::save_as_image(ftable_update, path = paste0(export_path, 'CaseStudies_table6_SminUpdate_', 
                                                      demand_scenario, '_', baseline_scenario, '_', mif_coef, '.png')) 
  }
}

if ("github" %in% write_outputs) { 
  #save flextable images to Github 
  flextable::save_as_image(ftable_WA, path = paste0(github_location, 
                                                    '/HARParchive/HARP-2023-Summer/WaterAvailability/CaseStudiesTables/flextables/CaseStudies_table1_WaterAvailabilityEqn_',
                                                    demand_scenario, '_', baseline_scenario, '_', mif_coef, '.png')) 
  
  flextable::save_as_image(ftable_caseStudies, path = paste0(github_location, 
                                                    '/HARParchive/HARP-2023-Summer/WaterAvailability/CaseStudiesTables/flextables/CaseStudies_table2_CaseStudies_',
                                                    demand_scenario, '_', baseline_scenario, '_', mif_coef, '.png'))
  #tables 3 and 4 exported when created because of loop structure 
  if (Smin_analysis == TRUE) { #Only needed for Smin analysis  
    flextable::save_as_image(ftable_Storage, path = paste0(github_location, 
                                                    '/HARParchive/HARP-2023-Summer/WaterAvailability/CaseStudiesTables/flextables/CaseStudies_table5_SminComparison_',
                                                    demand_scenario, '_', baseline_scenario, '_', mif_coef, '.png')) 
  
    flextable::save_as_image(ftable_update, path = paste0(github_location, 
                                                    '/HARParchive/HARP-2023-Summer/WaterAvailability/CaseStudiesTables/flextables/CaseStudies_table6_SminUpdate_',
                                                    demand_scenario, '_', baseline_scenario, '_', mif_coef, '.png'))
