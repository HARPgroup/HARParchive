...
for (i in 1:nrow(storage_data)) {
  ...
  dat <- fn_get_runfile(elid, runid_dem, site= omsite,  cached = TRUE) #get timeseries data (read in as zoo)
...
df_WA <- sqldf("SELECT a.* FROM metric_data as a WHERE Sminl30mg_apx_local IS NOT 0")
...
table_WA <- data.frame(Propname = df_WA$propname,
                     ...
                     SminL30mg_apx = df_WA$SminL30mg_apx_ups,
                     SminL30mg_nex = df_WA$SminL30mg_nex_ups,
                     SminL90mg_apx = df_WA$SminL90mg_apx_ups,
                     SminL90mg_nex = df_WA$SminL90mg_nex_ups,
...
#add % differences
storage_data$pctDiff30 <- 100.0 * (storage_data$SminL30mg_apx - storage_data$SminL30mg_nex) / storage_data$SminL30mg_nex
storage_data$pctDiff90 <- 100.0 * (storage_data$SminL90mg_apx - storage_data$SminL90mg_nex) / storage_data$SminL90mg_nex
...
write.csv(table_WA, paste0(export_path, 'WAcaseStudies_WAtable1_', 
                           demand_scenario, '_', baseline_scenario, '_', mif_coef, '.csv'), col.names = TRUE) 

write.csv(table_Smin, paste0(export_path, 'WAcaseStudies_SminTable2_', 
                           demand_scenario, '_', baseline_scenario, '_', mif_coef, '.csv'), col.names = TRUE) 

write.csv(table_Storage, paste0(export_path, 'WAcaseStudies_StorageTable3_', 
                           demand_scenario, '_', baseline_scenario, '_', mif_coef, '.csv'), col.names = TRUE) 

write.csv(table_imp, paste0(export_path, 'WAcaseStudies_impTable4_', 
                           demand_scenario, '_', baseline_scenario, '_', mif_coef, '.csv'), col.names = TRUE) 

write.csv(table_ups, paste0(export_path, 'WAcaseStudies_upsTable5_', 
                           demand_scenario, '_', baseline_scenario, '_', mif_coef, '.csv'), col.names = TRUE) 
