...
for (i in 1:length(caseStudy_list)) { 
  ...
  colnum_csL30 <- which(colnames(table_cswsL30) == "WA_mgd_30") #get col number for WA
  ...
  colnum_csL90 <- which(colnames(table_cswsL90) == "WA_mgd_90") #get col number for WA
...
for (i in 1:nrow(caseStudies_df)) {
  ...
  map30 <- ggplot(rsegs_bbox) + geom_sf() + #map all segments within bbox as background
    geom_sf(data = rsegs_sf, mapping = aes(fill = as.factor(bin))) +
