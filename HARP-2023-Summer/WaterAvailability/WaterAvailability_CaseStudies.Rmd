...
for (i in 1:nrow(caseStudies_df)) {
  ...
  plot30 <- ggplot(data = get(paste0('ups_ds_df_',i)), aes(x=riverseg)) +
    ...
    # ggtitle(paste0('Qdemand vs Qavailable (L30) for ', caseStudiesMetrics_df$riverseg[i], ': ', caseStudyName),
...
#add bins to case study watershed dataframes based on 30-day WA
for (i in 1:nrow(caseStudies_df)) {
  csws_df <- get(paste0('ups_ds_df_', i))
  ...
  entity_classes <- sqldf( #sorting the data into bins based on their range 
    ...
    group by a.NUM" , "csws_df")
    
  csws_df <- sqldf( #joining the bins with the metric data 
    "select a.*, b.bin 
    from csws_df as a 
    left outer join entity_classes as b 
    on a.NUM = b.NUM " , "csws_df")
  
  csws_df$bin[is.na(csws_df$bin)] <- "X" # replacing NA values in bin column
  
  assign(paste0('ups_ds_df_', i), csws_df)
}
```
...
for (i in 1:nrow(caseStudies_df)) {
  ...
  map <- ggplot(rsegs_bbox) + geom_sf() + #map all segments within bbox as background
    ...
    geom_sf(data = rsegs_df[rsegs_df$riverseg == caseStudy_list[i],] ,
            fill = NA, color = "red", lwd = 2) + #highlight outline case study segment
    geom_text_repel(data = rsegs_df, aes(x=lng, y=lat, label = NUM), size = 8) + #label segments 
  ...
  #save image of map
  WAmapfilename[i] <- paste(export_path, "WAcaseStudiesMap_", i, "_", caseStudies_df$riverseg[i], ".png", sep="")
  ggsave(
...
```{r View Maps, eval=FALSE}
#Get map PNGs and assign unique names for viewing
for (i in 1:nrow(caseStudies_df)) {
  map_img <- readPNG(WAmapfilename[i]) #pull saved PNG from local storage
  assign(paste0('map_img', i), map_img, envir = globalenv())
}

#View maps; much faster to just view PNGs by finding and opening file in local export_path 
grid.raster(map_img1) #view PNG of map - takes a few seconds for map to show up after function runs 
grid.raster(map_img2)
grid.raster(map_img3)
grid.raster(map_img4)
grid.raster(map_img5)
grid.raster(map_img6)
grid.raster(map_img7)


## Does NOT work when maps are created with nhd data; they're saved to export_path so view them from there instead
# plot(map1)
# plot(map2)
# plot(map3)
# plot(map4)
...
# for (i in 1:length(caseStudy_list)) {
#   plot(get(paste0('map', i)))
# }
```


# End Functional RMD (besides writing/saving chunks at the end)
