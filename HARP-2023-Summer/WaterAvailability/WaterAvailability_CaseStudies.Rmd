...
for (i in 1:nrow(storage_data)) {
  ...
  if (imp_enabled == TRUE | imp_off == 0) {
    ...
    match90 <- (rownum_nex90yr == rownum_apx90)
    ...
    # minstorage30yr <- storage_data[i, 'SminL30mg_apx'] #get min Storage in l30 year (approx Smin)
    # minstorage90yr <- storage_data[i, 'SminL90mg_apx']
    # 
    # n_mins30 <- length(which(datpd_30yr$Storage==minstorage30yr))
    # n_mins90 <- length(which(datpd_90yr$Storage==minstorage90yr))
    # 
    # yearMinRow30 <- which.min(datpd_30yr$Storage)
    # yearMinRow90 <- which.min(datpd_90yr$Storage)
    # 
    # if (n_mins30 > 1) {  
