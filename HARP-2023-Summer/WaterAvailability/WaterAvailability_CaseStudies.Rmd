...
for (i in 1:nrow(caseStudies_df)) {
  ...
  map_labs <- c(" <= 0", #less than or equal to first value in pct vector
                 "0 to 10", #labeling the ranges between each value from pct vector 
                 "10 to 50",
                 "50 to 100",
                 "100 to 500",
                 "500 to 1000",
  ...
  map <- ggplot(rsegs_bbox) + geom_sf() + 
    ...
    scale_fill_manual(values = c("firebrick2","darkorange","yellow",
                                 "white", "yellowgreen", "green3", "dodgerblue"),
                      breaks = seq(1:7),
                      labels = map_labs,
