...
params: 
  caseStudy_list: ["PL0_5141_5140", "JU3_6900_6950", "JA5_7480_0001", "YP2_6390_6330", "OR3_7740_8271_carvins", "PU3_4451_4450"]
...
for (i in 1:length(caseStudy_list)) {
  caseStudyRow <- metric_data[grep(caseStudy_list[i], metric_data$riverseg), ] #get row from metric data for case study
  caseStudies_df <- rbind(caseStudies_df, caseStudyRow)
}

#Add columns analyzing storage/Smin methods to case study dataframe
for (i in 1:length(caseStudy_list)) { 
  if (caseStudy_list[i] %in% storage_data$riverseg) { #need to check if the case study exists in the storage dataframe first
    caseStudies_df$min_in_pd30[i] <- storage_data[storage_data$riverseg == caseStudy_list[i], "min_in_pd30"]
    caseStudies_df$min_in_pd90[i] <- storage_data[storage_data$riverseg == caseStudy_list[i], "min_in_pd90"]
    caseStudies_df$outside_pd30[i] <- storage_data[storage_data$riverseg == caseStudy_list[i], "outside_pd30"]
    caseStudies_df$outside_pd90[i] <- storage_data[storage_data$riverseg == caseStudy_list[i], "outside_pd90"]
...
for (i in 1:length(caseStudy_list)) {
  ...
  caseStudyName_storage <- gsub('[0-9]+', '', storage_data[storage_data$riverseg == caseStudy_list[i], "propname"])
...
for (i in 1:length(caseStudy_list)) { 
  ...
  if ("github" %in% write_tables) {
    write.csv(table_caseStudyWshed, paste0(github_location, 
                                           ...
                                           '_CaseStudyWshed_', table_caseStudies$Riverseg[i], '_', demand_scenario, '_', 
...
for (i in 1:length(caseStudy_list)) { 
  ...
  rownum_csL30 <- which(csws_df$riverseg == caseStudy_list[i]) #get rownum for case study
  ...
  rownum_csL90 <- which(csws_df$riverseg == caseStudy_list[i]) #get rownum for case study
...
for (i in 1:nrow(caseStudies_df)) {
  ...
  plot30 <- ggplot(data = get(paste0('caseStudy_wshed_df_',i)), aes(x= reorder(riverseg, NUM))) +
    ...
    labs(title = 'Available vs Demand Flow (30-day)',
       subtitle = paste0("For ", get(paste0('caseStudyName',i)), " <br> Available Flow = Demand Flow - Min. Instream Flow"))
    
    assign(paste0('plot30_', i), plot30, envir = globalenv())
  
    plot30_filename[i] <- paste0(export_path, 'WAcaseStudies_Plot_', i, '_QvsQavail_L30.png')
 ...
 plot90 <- ggplot(data = get(paste0('caseStudy_wshed_df_',i)), aes(x= reorder(riverseg, NUM))) +
    ...
    plot90_filename[i] <- paste0(export_path, 'WAcaseStudies_Plot_', i, '_QvsQavail_L90.png')
    ggsave( #save image of plot
...
for (i in 1:nrow(caseStudies_df)) {
  ...
  map30 <- ggplot(rsegs_bbox) + geom_sf() + #map all segments within bbox as background
    ...
    geom_sf(data = rsegs_sf[rsegs_sf$riverseg == caseStudy_list[i],] ,
    ...
    ggtitle(paste0('Water Availability (30-Day) for ', get(paste0('caseStudyName',i)))) +
  ...
  WAmapfilename30[i] <- paste(export_path, "WAcaseStudiesMapL30_", i, "_", caseStudies_df$riverseg[i], ".png", sep="")
  ggsave(
  ...
  map90 <- ggplot(rsegs_bbox) + geom_sf() + #map all segments within bbox as background
    ...
    geom_text_repel(data = flowline_labs,
                    aes(x=lng, y=lat, label = name), size = 6)  + #label large rivers 
    ggtitle(paste0('Water Availability (90-day) for ', get(paste0('caseStudyName',i)))) +
    theme(plot.title = element_text(size = 40),
          plot.subtitle = element_text(size = 20),
          legend.text = element_text(size = 25), 
          legend.title = element_text(size = 30)) + 
  ...
  #save image of L30 map
  WAmapfilename90[i] <- paste(export_path, "WAcaseStudiesMapL90_", i, "_", caseStudies_df$riverseg[i], ".png", sep="")
  ggsave(
...
```{r View Case Study Maps and Tables, out.width = '100%', out.height='100%', results='asis', echo=FALSE}
for (i in 1:length(caseStudy_list)){
  #L30 map
