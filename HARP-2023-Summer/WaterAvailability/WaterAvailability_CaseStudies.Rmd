...
for (i in 1:nrow(storage_data)) {
  ...
  #fn_get_runfile uses print() which is not successfully hidden with chunk options. capture.output is solution for disabiling print() for clean knitting
  #capture.output(dat <- fn_get_runfile(storage_data$elid[i], runid_dem, site= omsite,  cached = TRUE), file = nullfile()) #get timeseries data (zoo), disable print form function
...
for (i in 1:nrow(metric_data)) {
  ...
  if(!grepl("0000_0000",metric_data$riverseg[i])){
     ...
     #capture.output(ups_imp <- fn_extract_basin(metric_data,metric_data$riverseg[i]), file = nullfile()) #disabling print() output
  }else{
...
caseStudiesMetrics_df <- data.frame()
caseStudiesStorage_df <- data.frame()

for (i in 1:length(caseStudy_list)) {
  caseStudyRow_metrics <- metric_data[grep(caseStudy_list[i], metric_data$riverseg), ]
  caseStudiesMetrics_df <- rbind(caseStudiesMetrics_df, caseStudyRow_metrics)
  
  caseStudyRow_storage <- storage_data[grep(caseStudy_list[i], storage_data$riverseg), ]
  caseStudiesStorage_df <- rbind(caseStudiesStorage_df, caseStudyRow_storage)
}

#Remove duplicate rows
caseStudiesMetrics_df <- caseStudiesMetrics_df[!duplicated(caseStudiesMetrics_df), ]
...
for (i in 1:length(outlet_list)) {
  ...
  #capture.output(ups_data <- fn_extract_basin(metric_data, outlet_list[i]), file = nullfile()) #disabling print() output
...
table_caseStudies <- data.frame(Propname = caseStudiesStorage_df$propname,
                     Riverseg = caseStudiesStorage_df$riverseg,
                     SminL30mg_apx = caseStudiesStorage_df$SminL30mg_apx,
                     SminL30mg_nex = caseStudiesStorage_df$SminL30mg_nex,
                     SminL90mg_apx = caseStudiesStorage_df$SminL90mg_apx,
                     SminL90mg_nex = caseStudiesStorage_df$SminL90mg_nex,
                     outside_pd30 = caseStudiesStorage_df$outside_pd30,
                     outside_pd90 = caseStudiesStorage_df$outside_pd90)
...
for (i in 1:nrow(caseStudiesMetrics_df)) {
  
  #get case study name for titling 
  caseStudyName <- gsub('[0-9]+', '', caseStudiesMetrics_df$propname[i])
...
```{r Mapping without fn_mapgen(), eval=FALSE}
...
for (i in 1:nrow(caseStudiesMetrics_df)) {
  ...
  map <- ggplot(rsegs_bbox) + geom_sf() + 
    ...
    geom_sf(data = rsegs_df[rsegs_df$riverseg == caseStudy_list[i],] ,
            fill = NA, color = "yellow", lwd = 1) + #highlight outline case study segment
    geom_text_repel(data = rsegs_df, aes(x=lng, y=lat, label = NUM), size = 8) + #label segments 
    ggtitle(paste0('Water Availability (L30) for ', caseStudiesMetrics_df$riverseg[i])) +
  ...
  #save image of map
  WAmapfilename[i] <- paste(export_path, "WAcaseStudiesMap_", i, "_", caseStudiesMetrics_df$riverseg[i], ".png", sep="")
  ggsave(
...
```{r Basemap Data Retrieval & Processing for fn_mapgen()}
...
```{r Configure fn_mapgen()}
rivseg_pct_vect <- div #last value should be higher than any value expected, since classification is done using <=
rseg_leg_title <- "Water Availability for L30 (mgd)"  
rivmap_labs <- bin_labs
...
for (i in 1:nrow(caseStudiesMetrics_df)) {
  ...
  bbox <- st_bbox(bbox_sfc)
  bbox_points <- data.frame(long = c(bbox[1], bbox[3]), lat = c(bbox[2], bbox[4]))
  nhd  <- memo_plot_nhdplus(bbox = bbox, actually_plot = FALSE) #get nhd data
  
  #processing labels for mapping 
  nhdlabs <- fn_nhd_labs(data=nhd)
  if (exists("maplabs") == TRUE) {
    rm(maplabs) #need to remove maplabs if it exists before recreation 
  }
