...
```{r Get Rsegs, echo=FALSE}
#Get rsegs -- doesn't result as intended, 100 less than CB's method
df_rsegs <- data.frame(
  'model_version' = c('vahydro-1.0', 'vahydro-1.0'),
  'runid' = c(demand_scenario, demand_scenario),
  'metric' = c('Qout', 'Smin_L30_mg'),
  'runlabel' = c('Qout','SminL90mg_vah')
)
rsegs_data <- om_vahydro_metric_grid(
  metric = metric, runids = df_rsegs, bundle = 'all', ftype = "all",
  base_url = paste(site,'entity-model-prop-level-export',sep="/"),
  ds = ds
)
rsegs <- data.frame(riverseg = unique(rsegs_data$riverseg), blankColumn = NA)
```

```{r Get Rsegs (CB's method)}
#CB's rseg method (best):
...
rsegsCB <- fn_download_read(url = paste(site,"/vahydro_riversegs_nogeom_export",sep = ""),
...
rsegsCB <- rsegsCB$hydrocode[grepl("^vahydrosw_wshed_.+_[0-9]{4}_[0-9]{4}$",rsegsCB$hydrocode)]
...
rsegsCB <- data.frame(riverseg = gsub("vahydrosw_wshed_","",rsegsCB),
                    blankColumn = NA)
```
...
```{r Smin Calculations, include=FALSE, echo=FALSE, message=FALSE}
storage_data$SminL30mg_nex <- NA #empty columns to hold  storage values
...
for (i in 1:nrow(metric_data)) {
  if (!grepl(".*0000_0000",metric_data$riverseg[i])) { #skip outlet segs to save time
    ups_imp <- fn_extract_basin(rsegsCB, metric_data$riverseg[i]) #find if any upstream segments have an impoundment within
  } else {
...
#get downstream segments: 3 segments downstream of each case study segment
ds_df <- data.frame() 
for (i in 1:nrow(caseStudies_df)) { 
...
for (i in 1:nrow(caseStudies_df)) { 
  ...
  #ftable_csws <- add_header_row(ftable_csws, values= title_table_caseStudyWshed) #add title 
  ...
  write.csv(table_caseStudyWshed, paste0(export_path, 'CaseStudies_table6_CaseStudyWshed_', table_caseStudies$Riverseg[i], '_',
                           demand_scenario, '_', baseline_scenario, '_', mif_coef, '.csv')) #write table
  flextable::save_as_image(ftable_csws, path = paste0(export_path, 'CaseStudies_table6_CaseStudyWsheds_', table_caseStudies$Riverseg[i], '_',
                                                  demand_scenario, '_', baseline_scenario, '_', mif_coef, '.png')) #save image of flextable
}
...
#l30 graph
plot30 <- ggplot(data = ups_ds_df_2, aes(x=riverseg)) +
