...
for (i in 1:nrow(caseStudies_df)) {
  ...
  #L30 graph
  plot30 <- ggplot(data = get(paste0('caseStudy_wshed_df_',i)), aes(x=riverseg)) +
    geom_bar(aes(y=l30_Qout_dem, fill = 'blue'), stat = 'identity') +
    geom_bar(aes(y=Qavailable_30_cfs, fill = 'green'), stat = 'identity') +
    theme(axis.text.x = element_text(size = 20, angle = 45, vjust = 1.0, hjust=1)) + #rotate x-axis labels
    theme(axis.title.x=element_blank()) +
    theme(legend.text = element_text(size = 25)) +
    ylab('cfs') + 
    scale_fill_manual(name = NULL, 
                      label = c("Qdemand", "Qavailable"), 
                      values = c("blue","green")) +
    theme(
      plot.title = element_markdown(size = 40),
      plot.subtitle = element_markdown(size = 30)) +
    labs(title = 'Qdemand vs Qavailable (L30)',
       subtitle = paste0("For ", get(paste0('caseStudyName',i)), "<br> Qavailable = Qdemand - MIF*Qbaseline"))
  ...
  #L90 graph
  plot90 <- ggplot(data = get(paste0('caseStudy_wshed_df_',i)), aes(x=riverseg)) +
    ...
    theme(
      plot.title = element_markdown(size = 40),
      plot.subtitle = element_markdown(size = 30)) +
    labs(title = 'Qdemand vs Qavailable (L90)',
       subtitle = paste0("For ", get(paste0('caseStudyName',i)), "<br> Qavailable = Qdemand - MIF*Qbaseline"))
...
for (i in 1:nrow(caseStudies_df)) {
  ...
  map30 <- ggplot(rsegs_bbox) + geom_sf() + #map all segments within bbox as background
    ...
    geom_text_repel(data = rsegs_sf, aes(x=lng, y=lat, label = NUM), size = 8) + #label segments 
    ...
    ggtitle(paste0('Water Availability (L30) for ', caseStudies_df$riverseg[i]), 
            subtitle = get(paste0('caseStudyName',i))) +
  ...
  map90 <- ggplot(rsegs_bbox) + geom_sf() + #map all segments within bbox as background
    ...
    geom_text_repel(data = flowline_labs,
                    aes(x=lng, y=lat, label = name), size = 6)  + #label large rivers 
    ggtitle(paste0('Water Availability (L90) for ', caseStudies_df$riverseg[i]), 
            subtitle = get(paste0('caseStudyName',i))) +
    theme(plot.title = element_text(size = 40),
          plot.subtitle = element_text(size = 20),
          legend.text = element_text(size = 25), 
