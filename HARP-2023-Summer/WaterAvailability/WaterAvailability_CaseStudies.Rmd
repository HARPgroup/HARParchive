...
```{r Get Impoundment Model, Class & Elid, echo=FALSE, message=FALSE, warning=FALSE}
...
## Currently Disabled -- re-implementing WSEs
#Remove waterSupplyElement objects
...
for (i in 1:nrow(caseStudies_df)) { 
  if (!caseStudies_df$riverseg[i] %in% c("OR3_7740_8271_carvins")) { #don't do normal upstream process for carvins cove 
    ups_segs <- fn_extract_basin(rsegs, caseStudies_df$riverseg[i]) #always extract using rsegs as full riverseg list to get upstream basins
    ups_data <- metric_data[metric_data$riverseg %in% ups_segs$riverseg,] #get metric data for upstream segments 
  if (nrow(ups_data) > 0) {
  }
    ...
    ups_segs <- fn_extract_basin(rsegs, ds_seg_carv) #get upstream segments from that downstream seg 
    ups_data <- metric_data[metric_data$riverseg %in% ups_segs$riverseg,]
  }
  ups_data <- rbind(ups_data[nrow(ups_data),],ups_data[1:(nrow(ups_data)-1),]) #move last row to top (outlet) to have df ordered downstream to upstream 
  ups_data <- ups_data[nrow(ups_data):1,]#reverse order to get upstream to downstream
  ups_data <- ups_data[!duplicated(ups_data), ] #remove duplicate rows
  assign(paste0('ups_df_', i), ups_data)
}
...
for (i in 1:nrow(caseStudies_df)) { 
  ds_seg1 <- fn_downstream(caseStudies_df$riverseg[i], rsegs$riverseg)
  ds_segs <- c(ds_seg1)
  if (ds_seg1 != 'NA') {
    ds_seg2 <- fn_downstream(ds_seg1, rsegs$riverseg)
    ds_segs <- c(ds_seg1, ds_seg2)
  }
  if (ds_seg2 != 'NA') {
    ds_seg3 <- fn_downstream(ds_seg2, rsegs$riverseg)
...
for (i in 1:nrow(caseStudies_df)) { 
  ...
  rownum_cs <- which(csws_df$riverseg == caseStudies_df$riverseg[i]) #highlight case study row
...
# Map 1: Water Availability for L30
...
```{r Basemap Data Retrieval & Processing for fn_mapgen(), echo=FALSE}
...
```{r Labeling for fn_mapgen(), echo=FALSE}
...
map <- fn_mapgen(mapnum = 2, featr_type = "facility", origin_type = "basin",
                 style = styles[[map_style]], metric = "WA_mgd_30_apx", origin = caseStudies_df$riverseg[1],
...
```{r Write CSVs Locally, echo=FALSE}
...
```{r Write CSVs to GitHub, echo=FALSE}
