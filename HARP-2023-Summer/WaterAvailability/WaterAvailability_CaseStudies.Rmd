...
#We will only use those rsegs that follow the standard format vahydrosw_wshed_XXX_XXXX_XXXX
#First, find only those riversegments that begin with vahydrosw_wshed followed by
#"_somecharacters_4 digits_4 digits"
...
for (i in 1:nrow(storage_data)) {
  ...
  dat <- fn_get_runfile(elid, runid_dem, site= omsite,  cached = TRUE) #get timeseries data (zoo)
...
# Find all outlet watersheds in VA. THese are designated as those segments that
# flow into segment 0001. So, use reular expression to find river segments that
# end in "_0001":
# outletWatersheds <- rsegs$riverseg[grepl("_0001$",rsegs$riverseg)]

#Alternatively, Find all outlet watersheds in VA. These are designated as those
#river segments that flow into river segment 0001 or 0000. So, use regular
#expression to find river segments that end in flow into "_0000" or "_0001" and
#have an ID that has at least one non-zero in it e.g. will not match
# MN0_0000_0000 but will match MN0_0100_0000 or MN0_0001_0000 
outletWatersheds <-rsegs$riverseg[grepl(".*_.*[1-9]+.*_000(1|0)$",rsegs$riverseg)]

# Sort by river segment ID for convenience
outletWatersheds <- outletWatersheds[order(outletWatersheds)]

#A simple function that gets all upstream segments of a given river segment
#(end_seg) and then note the downstream most segment as "outletseg" in the
#output data frame
getUpstreamSegs <- function(cia_data_frame,end_seg){
  print(paste0("Processing ",end_seg))
  upstreamSegs <- fn_extract_basin(cia_data_frame,end_seg)
  upstreamSegs$blankColumn <- end_seg
  names(upstreamSegs) <- c("riverseg","outletseg")
  return(upstreamSegs)
}

#Use mapply of getUpstreamSegs on rsegs and outletWatersheds to get a list of
#data frames with the upstream segments of each outletWatershed
upstreamSegs <- mapply(FUN = getUpstreamSegs,end_seg = outletWatersheds,
                       MoreArgs = list(cia_data_frame = rsegs),
                       SIMPLIFY = FALSE,USE.NAMES = FALSE)
#Combine the data frames in upstreamSegs as one. Use do.call to access each
#element of list upstreamSegs
upstreamSegsDF <- do.call(rbind,upstreamSegs)
...
for (i in 1:nrow(metric_data)) {
  ...
  if(!grepl("0000_0000",metric_data$riverseg[i])){
    #Find the outlet river segment of the full watershed that contains
    #metric_data$riverseg[i]
    outletRiverSeg <- upstreamSegsDF$outletseg[
      which(upstreamSegsDF$riverseg == metric_data$riverseg[i])]
    
    #Find all riversegments in the full watershed that contains
    #metric_data$riverseg[i]
    thisWatershed <- upstreamSegsDF[upstreamSegsDF$outletseg == outletRiverSeg,]
    
    #If no data was returned, this segment flows directly into a tidal basin and
    #is likely a southern river or on a fully tidal river. In these cases, we
    #can use fn_extract_basin on the full list of segments without performance
    #issues as these are small basins. If this is an outlet watershed, return
    #the stored list. In all other cases, we can use our stored lists of outlet
    #basins above to speed up fn_extract_basin()
    if(nrow(thisWatershed) > 0 &&
       (outletRiverSeg == metric_data$riverseg[i])){
      #If this is a watershed outlet, return all of thisWatershed as it
      #represents the fn_extract_basin() call on the outlet seg
      ups_imp <- thisWatershed
    }else if(nrow(thisWatershed) > 0){
      #Get only the riversegments upstream of metric_data$riverseg[i]
      ups_imp <- fn_extract_basin(thisWatershed,metric_data$riverseg[i])
    }else{
      #Get only the riversegments upstream of metric_data$riverseg[i]. Use the
      #full list of river segments as this appears to be in a tidal or SR basin
...
#upstream dataframes ordered correctly (upstream -> downstream)
...
for (i in 1:nrow(caseStudies_df)) { 
  ...
  #remove last row only for Lake Manassas case study
...
# Graph 1: Demand Flow vs Available Flow

```{r Graph 1.1, echo=FALSE, fig.cap="", fig.id = ""}

#create bar chart showing both WA and Qnsf (L30/90) or Qbaseline
scale = 1 #scale for transforming first y axis to 2nd

#l30 graph
plot30 <- ggplot(data = ups_ds_df_6, aes(x=riverseg)) +
  geom_bar(aes(y=l30_Qout_dem, fill = 'blue'), stat = 'identity') +
  geom_bar(aes(y=Qavailable_30_cfs, fill = 'green'), stat = 'identity') +
  scale_y_continuous(sec.axis =  sec_axis(trans = ~.*scale, name = 'Qavailable (cfs)')) +
  ...
  scale_fill_manual(name = "Legend", 
                    ...
                    values = c("blue","green")) +
  ggtitle('Demand Flow vs Qavailable (L30)') 

plot30


#l90 graph
plot90 <- ggplot(data = df_WA, aes(x=riverseg)) +
  geom_bar(aes(y=WA_mgd_90_apx, fill = 'blue'), stat = 'identity') +
  geom_bar(aes(y=l90_Qout_dem, fill = 'green'), stat = 'identity') +
  scale_y_continuous(sec.axis =  sec_axis(trans = ~.*scale, name = 'L90_Qout (cfs)')) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + #rotate x-axis labels
  theme(axis.title.x=element_blank()) +
  ylab('Water Availability (mgd)') +
  ggtitle('WA vs Flow (L90)')
...
write.csv(table_WA, paste0(export_path, 'CaseStudies_table1_WaterAvailability_', 
                           demand_scenario, '_', baseline_scenario, '_', mif_coef, '.csv')) 

write.csv(table_Smin, paste0(export_path, 'CaseStudies_table2_Smin_', 
                           demand_scenario, '_', baseline_scenario, '_', mif_coef, '.csv')) 
...
flextable::save_as_image(ftable_WA, path = paste0(export_path, 'CaseStudies_table1_WA_', 
                                                  demand_scenario, '_', baseline_scenario, '_', mif_coef, '.png')) #table1

flextable::save_as_image(ftable_Smin, path = paste0(export_path, 'CaseStudies_table2_Smin_', 
