...
metric_data$Qavailable_30 <- round((metric_data$l30_Qout_dem - mif_coef*metric_data$l30_Qout_base), digits = 3)
metric_data$Qavailable_90 <- round((metric_data$l90_Qout_dem - mif_coef*metric_data$l90_Qout_base), digits = 3)
...
title_table_caseStudies <- paste0('TABLE 4: CASE STUDIES. Demand: ', demand_scenario, ', Baseline: ', baseline_scenario, ', MIF: ', mif_coef)

#get impoundments of interest 
caseStudies_df <- data.frame() #empty df to hold impoundment objects 
...
# Table 5: WA in Case Study Watersheds
...
for (i in 1:nrow(caseStudies_df)) { 
  ...
  #ups_segs <- rbind(ups_segs, metric_data[metric_data$riverseg == imp_df$riverseg[i],]) #add case study segment to upstream dataframe
...
#combine upstream and downstream datframes
for (i in 1:nrow(caseStudies_df)) { 
  assign(paste0('ups_ds_df_', i), rbind(get(paste0('ups_df_', i)), get(paste0('ds_df_', i))))
}

#upstream and downstream dataframes are NOT ordered correctly yet (upstream to downstream)
#order by wd_cumulative first, which should give upstream -> downstream
for (i in 1:nrow(caseStudies_df)) { 
  ordered_df <- sqldf(paste0("SELECT a.* FROM ups_ds_df_", i ," as a 
                       ORDER BY (a.wd_cumulative_mgd)")) 
  assign(paste0('ups_ds_df_', i), ordered_df)
}

# then ordering will have to be by rseg naming convention - how can this be done in R?


#separate flextables for each case study with case study rseg in header 
for (i in 1:nrow(caseStudies_df)) { 
  title_table_caseStudyWshed <- paste0('TABLE 5: WA IN CASE STUDY WATERSHEDS. Demand: ', 
  ...
  table_caseStudyWshed <-   data.frame(Propname = csws_df$propname,
                      ...
                      Qavailable_30 = csws_df$Qavailable_30,
                      ...
                      Qavailable_90 = csws_df$Qavailable_90,
...
# Graph 1: Water Availability vs Flow 
...
#l30 graph
plot30 <- ggplot(data = df_WA, aes(x=riverseg)) +
  geom_bar(aes(y=WA_mgd_30_apx), stat = 'identity', fill = 'blue') +
  geom_bar(aes(y=l30_Qout_dem), stat = 'identity', fill = 'green') +
  scale_y_continuous(sec.axis =  sec_axis(trans = ~.*scale, name = 'L30_Qout')) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + #rotate x-axis labels
  theme(axis.title.x=element_blank()) +
  ggtitle('WA vs Flow (L30)')
...
plot90 <- ggplot(data = df_WA, aes(x=riverseg)) +
  geom_bar(aes(y=WA_mgd_90_apx), stat = 'identity', fill = 'blue') +
  geom_bar(aes(y=l90_Qout_dem), stat = 'identity', fill = 'green') +
