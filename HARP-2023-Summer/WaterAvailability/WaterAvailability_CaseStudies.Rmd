...
#Find all outlet watersheds in VA. These are designated as those river segments
#that flow into river segment 0001. So, use regular expression to find river
#segments that end in "_0001"
...
for (i in 1:nrow(metric_data)) {
  ups_imp <- data.frame()
  #Look for XXX_XXXX_0001 as outlet watersheds, run fn_upstream, store results, reference the stored results in fn_upstream for each watershed in metric_data
  
  #We want to find upstream segments for each riverseg, but 0000_0000 will take
  #forever so lets skip it
  if(!grepl(".*0000_0000",metric_data$riverseg[i])){
    ...
    ups_imp <- fn_extract_basin(thisWatershed,metric_data$riverseg[1])
    
  }else{
    #Skip bay segments and SR starts/ends
    print(paste0("Skipping segment ",metric_data$riverseg[i]," as it is a SR or Bay start/end"))
  }
