...
rsegs <- fn_download_read(url = paste(site,"/vahydro_riversegs_nogeom_export",sep = ""),
                          filetype = "csv", zip = FALSE) 
...
for (i in 1:nrow(metric_data)) {
  ...
  if(!grepl(".*0000_0000",metric_data$riverseg[i])){
    ups_imp <- fn_extract_basin(rsegs, metric_data$riverseg[i]) #find if any upstream segments have an impoundment within
  }else{
    print(paste0("Skipping segment ",metric_data$riverseg[i]," as it is a SR start"))
  }
...
table_ups <- data.frame(Propname = ups_df$propname,
                     ...
                     wd_cu_mgd = round(ups_df$wd_cumulative_mgd, digits = 3),
                     ps_cu_mgd = round(ups_df$ps_cumulative_mgd, digits = 3),
                     ...
                     SminL30mg_local = ups_df$SminL30mg_apx_local,
                     SminL30mg_upstream = ups_df$SminL30mg_apx_ups,
                     SminL90mg_local = ups_df$SminL90mg_apx_local,
                     SminL90mg_upstream = ups_df$SminL90mg_apx_ups,
                     WA_mgd_30 = ups_df$WA_mgd_30_apx,
                     WA_mgd_90 = ups_df$WA_mgd_90_apx)
...
plot30 <- ggplot(data = df_WA, aes(x=riverseg)) +
  ...
  scale_y_continuous(name = 'Water Availability (mgd)') +
...
plot90 <- ggplot(data = df_WA, aes(x=riverseg)) +
  ...
  scale_y_continuous(name = 'Water Availability (mgd)') +
  scale_y_continuous(sec.axis =  sec_axis(trans = ~.*scale, name = 'L90_Qout')) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + #rotate x-axis labels
  theme(axis.title.x=element_blank()) +
...
write.csv(table_WA, paste0(export_path, 'CaseStudies_table1_WA_', 
                           demand_scenario, '_', baseline_scenario, '_', mif_coef, '.csv'), col.names = TRUE) 
...
write.csv(table_imp, paste0(export_path, 'CaseStudies_table4_imp_', 
                           demand_scenario, '_', baseline_scenario, '_', mif_coef, '.csv'), col.names = TRUE) 

write.csv(table_ups, paste0(export_path, 'CaseStudies_table5_ups_', 
                           demand_scenario, '_', baseline_scenario, '_', mif_coef, '.csv'), col.names = TRUE) 
