...
# metric_data_sql <- sqldf("SELECT a.*, b.riverseg
#                               FROM metric_data AS a
#                               LEFT JOIN rsegs AS b
#                               WHERE (a. riverseg = b.riverseg) ") #results in 569 rows
...
for (i in 1:nrow(caseStudies_df)) { 
  if (!caseStudies_df$riverseg[i] %in% c("OR3_7740_8271_carvins")) { #for segments not Carvins
    ups_data <- fn_extract_basin(metric_data, caseStudies_df$riverseg[i]) 
    ...
    ups_data <- fn_extract_basin(metric_data, ds_seg_carv) #get upstream segments from that downstream seg 
  }
  ups_data <- rbind(ups_data[nrow(ups_data),],ups_data[1:(nrow(ups_data)-1),]) #move last row to top to have df ordered ds to us
