...
for (i in 1:nrow(metric_data)) {
  ...
  if (nrow(ups_imp) > 0) {
    ...
    metric_data$SminL30mg_nex_ups[i] <- round(sum(ups_df$SminL30mg_nex_local) - metric_data$SminL30mg_nex_local[i], digits = 3)
    metric_data$SminL90mg_nex_ups[i] <- round(sum(ups_df$SminL90mg_nex_local) - metric_data$SminL90mg_nex_local[i], digits = 3)
  }
...
metric_data$WA_mgd_30_nex = round((metric_data$Qavailable_30_cfs / 1.547) + 
  (metric_data$SminL30mg_nex_total / 30), digits = 3)

metric_data$WA_mgd_90_nex = round((metric_data$Qavailable_90_cfs / 1.547) + 
...
#Add columns analyzing storage/Smin methods to case study dataframe
for (i in 1:length(outlet_list)) { 
  if (outlet_list[i] %in% storage_data$riverseg) { #need to check if the case study exists in the storage dataframe first
    caseStudies_df$min_in_pd30[i] <- storage_data[storage_data$riverseg == outlet_list[i], "min_in_pd30"]
    caseStudies_df$min_in_pd90[i] <- storage_data[storage_data$riverseg == outlet_list[i], "min_in_pd90"]
    caseStudies_df$outside_pd30[i] <- storage_data[storage_data$riverseg == outlet_list[i], "outside_pd30"]
    caseStudies_df$outside_pd90[i] <- storage_data[storage_data$riverseg == outlet_list[i], "outside_pd90"]
  } else { #if it doesn't exist, fill with NA -- means sge not run with updated summary containing Smin routine yet
    caseStudies_df$min_in_pd30[i] <- NA
    caseStudies_df$min_in_pd90[i] <- NA
    caseStudies_df$outside_pd30[i] <- NA
    caseStudies_df$outside_pd90[i] <- NA
  }
}
...
names(table_WAhr)[names(table_WAhr) == "SminL30mg_apx_total"] <- "30-Day Total Storage (approx, mg)"
names(table_WAhr)[names(table_WAhr) == "SminL30mg_nex_total"] <- "30-Day Total Storage (near-exact, mg)"
names(table_WAhr)[names(table_WAhr) == "SminL90mg_apx_total"] <- "90-Day Total Storage (approx, mg)"
names(table_WAhr)[names(table_WAhr) == "SminL90mg_nex_total"] <- "90-Day Total Storage (near-exact, mg)"
names(table_WAhr)[names(table_WAhr) == "WA_mgd_30_apx"] <- "30-Day Water Availability (approx, mgd)"
names(table_WAhr)[names(table_WAhr) == "WA_mgd_30_nex"] <- "30-Day Water Availability (near-exact, mgd)"
names(table_WAhr)[names(table_WAhr) == "WA_mgd_90_apx"] <- "90-Day Water Availability (approx, mgd)"
names(table_WAhr)[names(table_WAhr) == "WA_mgd_90_nex"] <- "90-Day Water Availability (near-exact, mgd)"
...
```{r Table 2}
## Note: This should be a WA-focused table rather than storage because we've switched form impoundment segments to outlet segments 
...
table_caseStudies <- data.frame(Propname = caseStudies_df$propname,
                     ...
                     SminL30mg_apx = caseStudies_df$SminL30mg_apx_local,
                     SminL30mg_nex = caseStudies_df$SminL30mg_nex_local,
                     SminL90mg_apx = caseStudies_df$SminL90mg_apx_local,
                     SminL90mg_nex = caseStudies_df$SminL90mg_nex_local,
...
#Human readable column names for flextable 
names(table_caseStudiesHr)[names(table_caseStudiesHr) == "SminL30mg_apx"] <- "30-Day Storage (approx, mg)"
names(table_caseStudiesHr)[names(table_caseStudiesHr) == "SminL30mg_nex"] <- "30-Day Storage (near-exact, mg)"
names(table_caseStudiesHr)[names(table_caseStudiesHr) == "SminL90mg_apx"] <- "90-Day Storage (approx, mg)"
names(table_caseStudiesHr)[names(table_caseStudiesHr) == "SminL90mg_nex"] <- "90-Day Storage (near-exact, mg)"
names(table_caseStudiesHr)[names(table_caseStudiesHr) == "outside_pd30"] <- "Days Separating 30-Day Storage Methods"
names(table_caseStudiesHr)[names(table_caseStudiesHr) == "outside_pd90"] <- "Days Separating 90-Day Storage Methods"
...
ftable_caseStudies <- add_footer_row(ftable_caseStudies, top = TRUE, values = "Note: Local storage shown", colwidths = length(colnames(table_caseStudies))) #add footer
...
## This has been incorporated into table 5
