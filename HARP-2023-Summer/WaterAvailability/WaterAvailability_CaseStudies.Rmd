...
for (i in 1:nrow(caseStudiesMetrics_df)) {
  ...
  plot30 <- ggplot(data = get(paste0('ups_ds_df_',i)), aes(x=riverseg)) +
    ...
    labs(title = paste0('Qdemand vs Qavailable (L30) for ', caseStudiesMetrics_df$riverseg[i], ': ', caseStudyName),
       subtitle = paste0("**", caseStudyName, "**<br>Qavailable = Qdemand - MIF*Qbaseline"))
...
for (i in 1:nrow(caseStudiesMetrics_df)) {
  ...
  map <- ggplot(rsegs_bbox) + geom_sf() + 
    ...
    geom_text_repel(data = rsegs_df, aes(x=lng, y=lat, label = NUM), size = 5) + #label segments 
  ...
  #save image of map
  WAmapfilename[i] <- paste(export_path, "WAcaseStudiesMap_", caseStudiesMetrics_df$riverseg[i], ".png", sep="")
  ggsave(
