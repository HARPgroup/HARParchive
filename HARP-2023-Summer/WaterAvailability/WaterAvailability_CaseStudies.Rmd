...
for (i in 1:nrow(caseStudies_df)) {
  ...
  rsegs_df <- fn_centroid_coords(rsegs_df)
  ...
  map <- ggplot(rsegs_df) + geom_sf(mapping = aes(fill = WA_mgd_30_apx)) +
    scale_fill_gradient(low = "red", high = "green") +
      geom_text_repel(data = rsegs_df, aes(x=lng, y=lat, label = riverseg), size = 3) +
      ggtitle(paste0('Water Availability for ', caseStudies_df$riverseg[i]),
            subtitle = '') 
