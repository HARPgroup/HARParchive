...
if (object_class=="waterSupplyModelNode") {
  ...
  source(paste0("~/HARParchive/HARP-2023-Summer/WaterAvailability/waterSupplyModelNode.R")) #riverseg imp
} else if (object_class=="waterSupplyElement") {
  commandArgs <- function(...) c(pid,elid,runid)
  source(paste0("~/HARParchive/HARP-2023-Summer/WaterAvailability/waterSupplyElement.R")) #facility imp
} else if (object_class=="hydroImpoundment") {
  commandArgs <- function(...) c(pid,elid,runid)
  source(paste0("~/HARParchive/HARP-2023-Summer/WaterAvailability/hydroImpoundment.R")) #standalone imp
}
