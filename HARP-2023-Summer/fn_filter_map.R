...
if (distance > 300) {
  ...
  labelsP$segsize <- as.numeric( gsub(1, 0, labelsP$segsize) ) #no label "lollipop" for counties @ small distances
} else {
  zoom = 10
  ...
  labelsP$segsize <- as.numeric( gsub(1, 0, labelsP$segsize) ) 
}
  ...
  assign('zoom', zoom, envir = globalenv())
}