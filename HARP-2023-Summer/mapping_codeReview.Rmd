...
if (type == "facility") {
 if (metric_type == "modeled") { 
  ...
  #### Need to join facility-level foundational data with model data here -- featureid = hydroid
...
```{r VAhydro bbox if Desired, eval=FALSE, include=FALSE}
...
if (type=="source"){
  ...
  # - - permitted capacity @ source level disabled for now  
#  for (i in 1:nrow(facils$within)){ ... same as process below
...
#  table$vwp_max_mgy <-  replace(table$vwp_max_mgy, table$vwp_max_mgy==0, "No Permit")
 ...
 #Re-order columns
  ...
  table <- data.frame(Number= facils$within$NUM, 
                        ...
                        Rivseg= facils$within$riverseg,
                        Locality = facils$within$Locality,
                        metric1  = facils$within[,df$runlabel[1]],
                        metric2 = facils$within[,df$runlabel[2]]
                        ) 
  
  names(table)[names(table) == 'metric1'] <- df$runlabel[1]
  names(table)[names(table) == 'metric2'] <- df$runlabel[2]
  ...
  colnames(table)[grepl('runid_13',colnames(table))] <- paste0('2040 ',metric)
    ...
    table <- data.frame(Number= facils$within$NUM, 
                        ...
                        Locality = facils$within$Locality,
                        fiveYrAvg_mgy= facils$within$fiveyr_avg_mgy)
    ## add permit capacity -- use facil hydroid as featureid
    for (i in 1:nrow(facils$within)){
    systemObj <- RomProperty$new(ds,list(
      featureid = facils$within$Facility_hydroid[i],
      propcode= "vahydro-1.0"), 
    TRUE)
    ## add action if no obj found to skip search for propname
    if (is.na(systemObj$pid) == TRUE) {
...
# Call Table 
``` {r Call Table Function, echo=FALSE, warning=FALSE, message=FALSE}
...
rmarkdown::render("~/Desktop/GitHub/HARParchive/HARP-2023-Summer/mapping_codeReview.Rmd", output_file = "~/Desktop/HARPteam23/mappingRMD_knit", params = list(rivseg = "JL6_7430_7320", locality = "Spotsylvania", region = "Shenandoah_2", type = "facility", model_version = "vahydro-1.0", runid_list = c("runid_11","runid_13"), metric = "wd_mgd", metric_type = "modeled", map_type = "locality", bbox_type = "auto"))
```
