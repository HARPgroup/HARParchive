...
#----Foundational data @ the source/MP level----
## For Analysts: config.local.private determines wd variable: includes path from main drive up to where individual stores GitHub Repo's
...
## OR, set your working directory to the location of the Foundational csv, with the name above ^. Then un-comment and run:
...
# first, check if riverseg exists. It will already be in table if model data was pulled & joined 
 if (!"riverseg" %in% colnames(facils$within)) { # TRUE when riverseg column not present, will execute loop
  ...
  facils$within <- cbind(facils$within, coords)
  colnames(facils$within) <- gsub("hydrocode", "rseg_hydrocode", colnames(facils$within))
}
```
...
#  }
  ...
  for (i in 1:nrow(facils$within)){ #NOT functional
    systemObj <- RomProperty$new(ds,list(
      hydroid = facils$within$Facility_hydroid[i],
      propname = facils$within$Facility[i],
      bundle = "facility",
#    featureid = facils$within$featureid[i], ## featureid won't exist in source-level data since model data wasn't joined 
      propcode= "vahydro-1.0"), 
      TRUE)
