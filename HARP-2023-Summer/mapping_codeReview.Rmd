...
params:
  ...
  rivseg: ["JL6_6890_6990", "JL6_7320_7150"]  
  locality: ["Montgomery"]
...
roads$sf <- subset(roads$sf, RTTYP=="I" | FULLNAME %in% grep("US Hwy.*", roads$sf$FULLNAME, value=TRUE) | RTTYP=="S") #being switched to sqldf

#roads_sf <- roads$sf

#roads_sf_full <- roads$sf$FULLNAME
#roads$sf <- sqldf("select * from roads_sf where RTTYP= 'I' OR RTTYP= 'S' OR FULLNAME in " ) #STILL EDITING
...
roads$sf <- subset(roads$sf, !(FULLNAME %in% grep(".* .*", roads$sf$FULLNAME, value=TRUE))) #Being switched to sqldf

#roads_sf_full <- roads$sf$FULLNAME
#roads$sf <- sqldf("select * from roads_sf where FULLNAME in roads_sf_full= .*" ) #STILL EDITING
...
for(i in cities$population){ #NEEDS TO BE ADJUSTED FOR NEW CSV
...
#### TEMPORARY - alternate way to get county geometries 
...
# needs to be adjusted once REST is working again 
regions_geo <- sqldf( #adds geometries from counties df to the regions df
  "SELECT regions.*, counties.dh_geofield  
   FROM regions
   left outer join counties
   on (regions.county = counties.fips_name)
   ")
...
if (type == "locality") {
  ...
  locality_sf <- counties$sf[counties$sf$name == locality, ]
  sf_use_s2(FALSE) # switch off Spherical geometry
...
#  facils$within <- sqldf("SELECT facils_all.* FROM facils_all WHERE riverseg IN (select segs_basin.riverseg from segs_basin) AND riverseg IS NOT NULL") ## need to mod for localities 
}
...
if (type == "region") {
  ...
  region_sf <- region_geom_list[names(region_geom_list) == region]
}
```
...
labels <- data.frame(
  name= c(counties$sf$name, cities$NAME, lb_rivr$gnis_name, lb_wtbd$gnis_name, roads$sf$FULLNAME[!duplicated(roads$sf$FULLNAME)]),
  class= c(counties$sf$ftype, cities$CLASS, lb_rivr$StreamOrde, lb_wtbd$FTYPE, roads$sf$RTTYP[!duplicated(roads$sf$FULLNAME)]),
  lng= as.numeric( c(counties$sf$lng, cities$X, lb_rivr$lng, lb_wtbd$lng, roads$sf$lng[!duplicated(roads$sf$FULLNAME)]) ),
  lat= as.numeric( c(counties$sf$lat,cities$Y, lb_rivr$lat, lb_wtbd$lat, roads$sf$lat[!duplicated(roads$sf$FULLNAME)]) )
)
remove(lb_rivr)
remove(lb_wtbd)
...
# Saving map as a png
ggsave(
  filename = paste(export_path, map_name, ".1", ".png", sep=""),
  plot = map,
  width = 25,
  height = 20
  )
...
#might want to change naming if we are changing whats mapped by source and whats mapped by facility
colnames(table)[Facility] <- "Source Name"
colnames(table)[NUM] <- "Source Number"
colnames(table)[Source_Type] <- "Source Type"
colnames(table)[Rivseg] <- "River Segment"
colnames(table)[rivsegName] <- "River Segment Names"
colnames(table)[Locality] <- "County"
colnames(table)[Upstream_Downstream] <- "Upstream Vs. Downstream"
colnames(table)[vwp_max_mgy] <- "Permitted Capacity(mgy)"
colnames(table)[fiveYr] <- "5 Year Average Use- Source(mgy)"
```
# Call Table 
