...
for (k in 1:length(rivseg_metric)){
  ...
  colname1[k] <- paste0(rivseg_runids[1],'_',rivseg_metric[k])
  colname2[k] <- paste0(rivseg_runids[2],'_',rivseg_metric[k])
 ...
 # segs$basin[,rivseg_metric[k]] <- as.numeric(segs$basin[,colname2[k]]) - as.numeric(segs$basin[,colname1[k]])
  segs$basin[,paste0('diff','_',rivseg_metric[k])] <- as.numeric(segs$basin[,colname2[k]]) -as.numeric(segs$basin[,colname1[k]])
 ...
 # segs$basin$diff <- as.numeric(segs$basin[,colname2[k]]) - as.numeric(segs$basin[,colname1[k]])
  segs$basin[,paste0('percentDiff','_',rivseg_metric[k])] <- (as.numeric(segs$basin[,paste0('diff','_',rivseg_metric[k])]) / as.numeric(segs$basin[,colname1[k]])) * 100
...
#Adding categories to rivseg df based on the % difference 
rivdiv <- c(-20,-10,-5,0,5,10,20)
...
#rivbins = as.data.frame(rivdiv)
#rivbins$classid <- index(rivbins)
#names(rivbins) <- c("val", "bin")
...
rivmapname <- paste(export_path, map_name, "_rivsegs.png", sep="")
  ggsave(
    filename = rivmapname,
    plot = map,
    width = 25,
...
```{r Display Rivseg Map, echo=FALSE}
knitr::include_graphics(rivmapname)
```
