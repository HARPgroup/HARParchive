...
params:
  ...
  runid_list: [ "NULL" ] # should be 'NULL' if a non-runid metric used (fiveyr_avg_mgy)
...
if (runid_list == 'NULL') { # for metrics not associated w model runs
...
#### TEMPORARY - alternate way to get county geometries when REST doesn't work
...
#  locality_sf <- counties$sf[counties$sf$name == locality, ] #if REST IS working
  ...
  ### only use if REST is NOT working:
  # need to turn counties into a df within the list counties 
  counties_df <- counties
  counties <- list()
  counties$df <- as.data.frame(counties_df)
  rm(counties_df)
  counties$sp <- process_geom(counties$df, "fips_geom")
  counties$sf <- st_as_sf(counties$sp)
  locality_sf <- st_as_sf(counties$sf[counties$sf$fips_name == locality, ], sf_column_name = "fips_geom")
  st_crs(counties$sf) <- 4326
...
#adding ftype column to counties sf df:
counties$sf$ftype = "county"

labels <- data.frame(
  name= c(counties$sf$fips_name, cities$city, lb_rivr$gnis_name, lb_wtbd$gnis_name, roads$sf$FULLNAME[!duplicated(roads$sf$FULLNAME)]),
  class= c(counties$sf$ftype, cities$class, lb_rivr$StreamOrde, lb_wtbd$FTYPE, roads$sf$RTTYP[!duplicated(roads$sf$FULLNAME)]),
  lng= as.numeric( c(counties$sf$fips_longitude, cities$lng, lb_rivr$lng, lb_wtbd$lng, roads$sf$lng[!duplicated(roads$sf$FULLNAME)]) ),
  lat= as.numeric( c(counties$sf$fips_latitude,cities$lat, lb_rivr$lat, lb_wtbd$lat, roads$sf$lat[!duplicated(roads$sf$FULLNAME)]) )
)
##
...
fn_mapgen(type, metric = 'fiveyr_avg_mgy', rivseg, bbox, segs, facils, counties, roads, nhd, labelsP, locality) #execute mapping function
```
