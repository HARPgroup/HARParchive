...
#determining what metric(s) will be pulled and mapped
  ...
  metric_feat: ["wd_current_mgy"] #for non-modeled facility metrics 
...
#asthetic changes  
  ...
  table_col: ["runid_11_wd_mgd", "runid_13_wd_mgd", "wd_current_mgy"] ##specific metrics included in flextable
...
legend_titling <- function(metric_mod, runid_list){
  
  if(metric_mod=="wd_mgd"){ #titles for wd_mgd
    name <- "Withdrawal"
    unit <- "(MGD)"
  ...
  #titles for 5 yr avg
  if(metric_mod=="fiveyr_avg_mgy"){ #this metric isn't associated w a runid
    name <- "5-yr Avg Use"
    unit <- "(MGY)"
...
#if (type == "facility") { #modeled metrics pulled every facility-level run 
#  if (metric_mod != "fiveyr_avg_mgy"){
    for(i in 1:length(runid_list)){
      df$runlabel[i] <- paste0(runid_list[i], '_', metric_mod)
    } 
#  } else { #if 5-yr avg is metric of interest
#    df$runlabel <- df$metric_mod
#  }
...
# if (metric_mod != "fiveyr_avg_mgy") { #metric not 5 yr avg
...
# } else if (metric_mod =="fiveyr_avg_mgy") {
