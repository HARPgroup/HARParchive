...
params:
  ...
  rivseg: ["JA4_7280_7340" , "JB3_6820_7053","JL6_7320_7150", "JL7_7070_0001"] 
  locality: ["Montgomery"]
  region: ["NewRiver_2"]    
...
#New layers for each bin 
mp_bin_1 <-   sqldf('SELECT * FROM mp_layer WHERE bin = 1')
mp_bin_2 <-   sqldf('SELECT * FROM mp_layer WHERE bin = 2')
mp_bin_3 <-   sqldf('SELECT * FROM mp_layer WHERE bin = 3')
mp_bin_4 <-   sqldf('SELECT * FROM mp_layer WHERE bin = 4')
mp_bin_5 <-   sqldf('SELECT * FROM mp_layer WHERE bin = 5')
mp_bin_6 <-   sqldf('SELECT * FROM mp_layer WHERE bin = 6')
mp_bin_7 <-   sqldf('SELECT * FROM mp_layer WHERE bin = 7')
mp_bin_8 <-   sqldf('SELECT * FROM mp_layer WHERE bin = 8')
mp_bin_9 <-   sqldf('SELECT * FROM mp_layer WHERE bin = 9')
mp_bin_10 <-  sqldf('SELECT * FROM mp_layer WHERE bin = 10')

mp_bin_list <- list(mp_bin_1, mp_bin_2, mp_bin_3, mp_bin_4, mp_bin_5, mp_bin_6, mp_bin_7, mp_bin_8, mp_bin_9, mp_bin_10)
```

```{r Apply fn_mapgen, warning=FALSE, message=FALSE}
fn_mapgen(type, metric = 'runid_11_wd_mgd', rivseg, bbox, segs, facils, counties, roads, nhd, labelsP, locality, region) #execute mapping function
```
