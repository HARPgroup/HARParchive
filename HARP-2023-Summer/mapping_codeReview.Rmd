...
#determining which type/level of map is being created
  ...
  map_type: ["locality"] #should be one of: basin, locality, or region 
...
#determining what metric(s) will be pulled and mapped
  ...
  rivseg_metric: ["l90_Qout", "7q10", 'l30_Qout'] #drought metric to be used in river segment maps, options: 7q10, l30_Qout, l90_Qout
...
if (type == "facility") { 
  ...
  facils$facil_geo <- fread(paste0(github_location, "/Foundational_Data/2023/facilities_all_geom.csv"))
} 
...
# Filter Data (Cleanup)
```{r Facilities, echo=FALSE, message=FALSE, warning=FALSE, eval=FALSE}

#filter out WSP entries from facility-level metric data:
facils$metrics <- with(facils, sqldf("SELECT * 
                                      FROM metrics 
                                      WHERE hydrocode not like 'wsp_%'"))

```

```{r Counties & Regions, echo=FALSE, message=FALSE, warning=FALSE, eval=FALSE}
