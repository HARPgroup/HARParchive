...
params:
  ...
  metric: ["wd_mgd"] # 5-year avg metric called 'fiveyr_avg_mgy' , which is part of foundational data frame or wd_mgd
  metric_type: ["modeled"] #either 'modeled' or 'measured'. Measured metrics not tied to a runid
  map_name: ["JB3_6820_7053"]
...
# (csv) for US cities
cities <- fread(paste0(getwd(), '/', 'USA_Major_Cities_GIS.csv')) # prev uscities.csv 
...
if (map_type == "region") {
 if (metric_type == "modeled") { 
  if (exists("facil_featr") == FALSE) { #only retrieve data if needed, process takes time
...
#change to sqldf- need to drop geom to do this, sqldf does not like geom- any other way to do it?
#roads <- st_drop_geometry(roads)
#roads <- sqldf("SELECT * FROM roads WHERE FULLNAME like 'US HWY_%' OR RTTYP is 'I' OR RTTYP is 'S'")
#roads <- sqldf("SELECT * from roads where FULLNAME NOT LIKE '___%'")
...
# Removing cities that are also county names, for cleaner mapping
...
```{r bbox & facils by map_type Param, warning=FALSE}
## Setting bbox and filtering MPs by extent we want, 
...
```{r Add riverseg column by spatial join if needed}
## Adding riversegs to facils$within data frame when not present using a spatial join
# segs$all contains segment IDs and geometries 
# facils$within contains MP names and coords
...
```{r Process basin geometries, warning=FALSE}
...
#facils$within needs to stay as MPs for table generation
...
#Adding bin column to dataframe 
bins_sql <-  paste("SELECT *,
	                  CASE WHEN demand_metric > 0 AND demand_metric <= ",div[1]," THEN '1'
...
```{r Set sp based on map_type}
## Setting sp variable based on map type to do reverse fill for all map types
if (map_type == "basin") {
    sp <- segs$basin_sp
...
fn_mapgen(map_type, style, metric, rivseg, bbox, segs, counties, roads, 
          nhd, maplabs, locality, region, mp_layer, metric_unit, sp)
# Error debugging:
