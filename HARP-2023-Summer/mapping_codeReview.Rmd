...
#determining what metric(s) will be pulled and mapped
  ...
  rivseg_metric: ["l90_Qout", "7q10"] #drought metric to be used in river segment maps, options: 7q10, l30_Qout, l90_Qout
...
cities <- sqldf("SELECT cities.* 
                ...
                AND (cities.ST == 'VA')
                ") #VA cities only & Remove city/town names that match their county/locality name

statemt <- paste("SELECT cities.*
                 FROM cities
                 WHERE POPULATION BETWEEN", quantile(cities$POPULATION, 0.1),"AND", quantile(cities$POPULATION, 0.5),
                 "IS (cities.class == 'smallTown')",
                 sep=" ")
smallTowns <- sqldf(statemt) ##!!!!!!!!!!!!!!! where ya left off
...
for (i in 1:length(table_col)){
  ...
  #adding requested columns to table 
    table1[,table_col[i]] <- round(facils$within[,table_col[i]], digits = 2) #round all values to 2 decimal place digits
  }
}

table <- table[,colSums(is.na(table1))<nrow(table)] #remove columns with all NAs, added to df for mapping as placeholders 
...
names(table1)[names(table1) == 'Number'] <- '#' #can't have empty column names for flextable
names(table1)[names(table1) == 'rivsegName'] <- 'River Segment Name'
names(table1)[names(table1) == 'Rivseg'] <- 'River Segment ID'
names(table1)[names(table1) == 'vwp_max_mgy'] <- 'Permitted Capacity (mgy)'
names(table1)[names(table1) == 'Source_Type'] <- 'Source Type'
names(table1)[names(table1) == 'fiveyr_avg_mgy'] <- '5-yr Avg Use (mgy)'
...
fn_tablegen(type = "facility", table = table1, alignment = "left") 
```
...
# Rivseg Map Creation
```{r Rivseg Map Creation, echo=FALSE, warning=FALSE}
#Adding bins to rivseg df based on the % difference 
rivdiv <- c(-20,-10,-5,0,5,10,20) #% difference breaks for rivseg map
segs_basin <- segs$basin #for sqldf
for (i in 1:length(rivseg_metric)) { #create separate map for each rivseg metric  
  segs_basin <- segs_basin[ ,!names(segs_basin) == "pct_diff"] #remove col to prevent duplicate colnames
  ...
  bins_sql <-  paste("SELECT segs_basin.*,
	                  CASE WHEN pct_diff <= ",rivdiv[1]," THEN '1'
		                WHEN pct_diff >  ",rivdiv[1]," AND pct_diff <= ",rivdiv[2]," THEN '2'
		                WHEN pct_diff >  ",rivdiv[2]," AND pct_diff <= ",rivdiv[3]," THEN '3'
		                WHEN pct_diff >  ",rivdiv[3]," AND pct_diff <= ",rivdiv[4]," THEN '4'
		                WHEN pct_diff >  ",rivdiv[4]," AND pct_diff <= ",rivdiv[5]," THEN '5'
		                WHEN pct_diff > ",rivdiv[5]," AND pct_diff <= ",rivdiv[6]," THEN '6'
		                WHEN pct_diff > ",rivdiv[6]," AND pct_diff <= ",rivdiv[7]," THEN '7'
		                WHEN pct_diff > ",rivdiv[7]," THEN '8'
		                ELSE 'X'
		                END AS bin
		                FROM segs_basin",sep="")
  
  segs$basin <- sqldf(bins_sql)
  
  segs$basin$geometry <- segs$basin_sf$geometry
  segs$basin_sf <- st_as_sf(segs$basin) #create sf object for mapping w/ geom_sf
  st_crs(segs$basin_sf) <- 4326
...
for (i in 1:length(rivseg_metric)) {
  mapfilename[i] <- paste(export_path, map_name, "_", rivseg_metric[i], ".png", sep="")
...
knitr::include_graphics(mapfilename)
