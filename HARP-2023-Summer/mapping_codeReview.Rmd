...
#skip metric pull if scenario pid = NA, meaning runid wasn't found
  ...
  model_metric <- RomProperty$new( #get metric from vahydro
    ...
    propname = rivseg_metric
    ),
    TRUE)

  segs$basin[i ,paste0(rivseg_runids[j],'_',rivseg_metric)] <- model_metric$propvalue
  } else { #if the scenario/runid wasn't found
    segs$basin[i ,paste0(rivseg_runids[j],'_',rivseg_metric)] <- NA
      }
   }
  }
}
...
```{r Rivseg Metric % Difference, echo=FALSE}
segs$basin <- st_drop_geometry(segs$basin) #needed to do diff & pct diff calcs 
colname1 <- paste0(rivseg_runids[1],'_',rivseg_metric)
colname2 <- paste0(rivseg_runids[2],'_',rivseg_metric)
segs$basin$diff <- as.numeric(segs$basin[,colname2]) - as.numeric(segs$basin[,colname1])
segs$basin$pct_diff <- (as.numeric(segs$basin$diff) / as.numeric(segs$basin[,colname1])) * 100
```
...
rivTable <- data.frame(
  ...
  Diff = segs$basin$diff,
  percDiff= segs$basin$pct_diff
)

#create metric type column- means get rid of metric title in titling atm

#create metric column- change titling as well


#fn_tablegen(type = "riverseg", table = rivseg_table, alignment = "left") #flextable saved as var ft
```
