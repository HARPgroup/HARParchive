...
# Rivseg Map Creation
```{r Old:Rivseg Map Creation, echo=FALSE, warning=FALSE, eval=FALSE}
...
```{r Upgrade: auto-bins for % diff, warning=FALSE}
## Need to adapt this for multiple maps

rivdiv <- c(-20,-10,-2,2,10,20,500) #% difference breaks for rivseg map - move to config

segs_basin <- segs$basin

rseg_sql <- paste('SELECT *, ',paste0('percentDiff','_',rivseg_metric[1]),' AS pct_diff
        FROM segs_basin' , sep="") #Renaming metric of interest for generalized sorting into bins
  
segs_basin <- sqldf(rseg_sql)
  
bins = as.data.frame(rivdiv) #rivdiv to be moved to config 
bins$classid <- index(bins)
names(bins) <- c("val", "bin")
...
segs$basin <- sqldf( #joining the bins with the metric data 
    "select a.*, b.bin 
    from segs_basin as a 
