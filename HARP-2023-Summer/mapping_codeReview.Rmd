...
```{r Load Functions from Online, echo=FALSE}
...
facilities_df$hydrocode <- "" #empty column where hydrocode will be stored
#Can we add a PID or hydrocode column to facilities_df? Need for sqldf joins in the future 
for (i in 1:nrow(facilities_df)){
  facilObj <- getFeature(inputs = list(
    hydroid = facilities_df$Facility_hydroid[i],
    bundle = "facility"),
    ds$get_token(rest_pw),
    site)
  facilities_df$hydrocode[i] <- facilObj$hydrocode
}

```

```{r Facility Data and Coords for Region maps, warning=FALSE, message=FALSE}
## For mapping by facility -- requires VAhydro data retrieval 
if (type == "facility") {
 if (metric_type == "modeled") { 
  ...
  ds = ds)
 #Get facility coordinates -- already in facilities df -- NOT functional
 facil_coords <- sqldf("SELECT a.*, b.Longitude, b.Latitude
                    FROM facil_featr as a
                    LEFT OUTER JOIN facilities_df as b
                    ON (a.propname = b.Facility)" ) 
...
```{r Add Locality Column if Needed}
#Get locality for facilities from foundation data frame -- if needed
...
table <- data.frame(Number= facils$within$NUM, 
                        ...
                        Rivseg= facils$within$riverseg,
                        Locality = facils$within$Locality,
                        Metric = facils$within[,df$runlabel[1]]
                        ) 
