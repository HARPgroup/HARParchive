...
params:
  ...
  metric_type: ["measured"] #either 'modeled' or 'measured'. Measured metrics not tied to a runid
...
bins = as.data.frame(c(0.5, 1.0,2.0,5.0,10.0,50,100,500,1000,2000,5000))
bins$classid <- index(bins)
names(bins) <- c("val", "classid")

entity_classes <- sqldf(
  "select a.NUM, min(b.classid) as classid
  from mp_layer as a 
...
classified_data <- sqldf(
  "select a.*, b.classid 
  from mp_layer as a 
  left outer join entity_classes as b 
  on a.NUM = b.NUM "
)
# column with bins named 'classid'
...
table <- data.frame(Number= facils$within$NUM, 
                        ...
                        fiveYr= facils$within$fiveyr_avg_mgy
                        ) 
