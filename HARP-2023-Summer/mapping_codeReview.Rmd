...
params:
  ...
  table_limit: ["boundary"] #either 'basins' or 'boundary', depending if all sources/facils in intersecting basins should be in table, or only points within locality/region boundary. Won't apply to map type basin
...
table_limit <- params$table_limit
...
```{r Pull Foundational Data w/ github_location, echo=FALSE, eval=FALSE}
...
```{r Apply Table Limit, include=FALSE}
if (table_limit=="boundary") {
  facils$within_sf <- st_as_sf(facils$within, coords = c("Longitude","Latitude"))
  st_crs(facils$within_sf) <- 4326
  if (map_type=="locality") {
    facils$within <- st_filter(facils$within_sf, locality_sf)
  } else if (map_type=="region") {
    st_crs(region_sfd) <- 4326
    facils$within <- st_filter(facils$within_sf, region_sfd)
  }
}
```
...
table <- data.frame(Number= facils$within$NUM, 
                      ...
                      fiveYrAvg_mgy= round(facils$within$fiveyr_avg_mgy, digits = 2)
                        )
# - - - - add rivseg names- - - - - -
