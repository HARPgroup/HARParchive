...
#asthetic changes  
  ...
  rivseg_met: ["nhdp_drainage_sqkm"] #choose riversegment map metric to use
---
...
```{r, data processing for rivseg maps, message=FALSE}

#pull metric(specified in rivseg_met param) into segs$basin dataframe

for (i in 1:nrow(segs$basin)) {
  rivsegMet <- RomProperty$new(ds,list(
          hydrocode = segs$basin$hydrocode[i],
          propname = rivseg_met),
          TRUE)
  segs$basin$rivsegMet[i] <- rivsegMet$propcode
}
```
