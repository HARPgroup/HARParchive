...
```{r Combine Locality Geoms into Regions}
...
```{r Joining Facility Data and Coords, warning=FALSE, message=FALSE}
...
if (type == "facility") {
 ...
 facil_featr <- sqldf("SELECT a.*, b.Longitude, b.Latitude, b. Locality
...
### facils$found_each has five-yr-avg use data at facility level
### facil_featr has modeled metric data at facility level
```
...
#blank column for river seg names 
table$riv
# - - - - add rivseg names- - - - - -
for(i in 1:nrow(table)){ ## this doesn't work
    loc_rivseg <- (segs$all[grep(table$Rivseg[i], segs$all$riverseg)],) ## don't use segs$basin
    table$rivsegName[i]  <- loc_rivseg$name 
}
#- - - - add permitted capacity - - -
...
table$metric1 <- facils$within$runid_11_wd_mgd ## don't hardcode
table$metric2 <- facils$within$runid_13_wd_mgd ## don't hardcode
