...
regions_geo <- sqldf( #adds geometries from counties df to the regions df
  "SELECT regions.*, counties.fips_geom  
   FROM regions
...
if (type == "region") {
  ...
  facils$within_reg <- st_filter(facils$sf, region_sf) #filter to MPs within the region
...
fn_mapgen(type, metric = 'fiveyr_avg_mgy', rivseg, bbox, segs, facils, counties, roads, nhd, labelsP, locality, region) #execute mapping function
```
