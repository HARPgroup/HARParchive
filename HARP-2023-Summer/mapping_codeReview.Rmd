...
## Pulling 5-year average use from VAhydro

for (i in 1:nrow(facils$basin)) {  #nrow used instead of length to overcome yesterday's error
 mp <- RomFeature$new(ds,list(
  hydroid=facils$basin$MP_hydroid[i],
  bundle = 'intake'), 
  TRUE) 
 ...
 facils$basin$fiveYrAvg[i] <- as.numeric(fiveYrAvg$propvalue)

}
# facils$basin$fiveYrAvg is a list rather than added to the data frame .. to fix tmrw
```
...
fn_mapgen(rivseg, bbox, segs, facils, counties, roads, nhd, labelsP) #execute function
```
...
```{r}
##### Everything below has been copied into the mapping function called above 

# Calculation of extent = get diagonal of bbox in miles
extent <- data.frame(lng = bbox[c("xmin", "xmax")], lat = bbox[c("ymin", "ymax")] )
