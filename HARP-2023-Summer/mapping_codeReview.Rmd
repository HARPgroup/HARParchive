...
params:
  ...
  locality: ["Culpeper"]
  region: ["NewRiver_2"]    
  type: ["locality"] #should be one of: basin, locality, or region ; depending on what map type is desired 
...
cities <- sqldf( 
  "SELECT cities.*  
   FROM cities
  where NAME is not (select counties_df.name from counties_df)"
)
...
```{r NEW Labeling with fn_labelprep}
...
fn_mapgen(type, metric, rivseg, bbox, segs, counties, roads, nhd, maplabs, locality, region, mp_layer, metric_unit, sp) #execute mapping function
```
...
table <- data.frame(Number= facils$within$NUM, 
                        ...
                        Source_Type = facils$within$`Source Type`,
                        Source = facils$within$`MP Name`,
