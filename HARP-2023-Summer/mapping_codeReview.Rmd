...
```{r (Old) Analysts can: Load Functions from Files, include=FALSE}
## Please leave this chunk during dev for analysts, makes testing & editing functions easier (doesn't req merge)
...
```{r (New) Pull Foundational Data w/ github_location, echo=FALSE, eval=FALSE}
...
# Pull Metrics
```{r Create df for Runs & Metric, echo=FALSE, message=FALSE, eval=FALSE}
...
#df without metric_type -- success TBD
df <- data.frame(runid=runid_list, model_version, metric=metric)
if (type == "facility") {
...
```{r Filter Sources/Facils by Segs, include=FALSE}
...
facils_within <- st_drop_geometry(facils$within) #for use in sqldf for ordering
```

```{r Order & Number Metric Data, include=FALSE}
