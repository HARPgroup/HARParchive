...
map <- basemap + #ggplot2::
   geom_sf(data = fips_sf, inherit.aes = FALSE, color = "#0033335F", fill = NA, lwd=1) +
    ...
    geom_sf(data = nhd$flowline,inherit.aes = FALSE,color = "#009999", fill = NA, mapping = aes(lwd = nhd$flowline$StreamOrde), show.legend = FALSE) + 
       scale_linewidth(range = c(0.25,2)) + # mapping all flowlines with thickness based on stream order
    geom_sf(data = nhd$flowline567,inherit.aes = FALSE,color = "#009999", fill = NA, lwd = 0.5) + # mapping flowlines of major rivers
...
#                    size = 4, color = "dodgerblue3") + #labeling order 4 streams
    geom_text_repel(data = labs_bind, aes(x = as.numeric(centroid_longitude), y = as.numeric(centroid_latitude), group = 1, label = GNIS_NAME, size = class),
                    color = "dodgerblue3", show.legend = FALSE) + scale_size(range = c(5,10)) + #labeling both major rivers and smaller rivers/streams
    # scalebar
  ...
  #### Test Scale Bar Using Joey's anchoring method 
