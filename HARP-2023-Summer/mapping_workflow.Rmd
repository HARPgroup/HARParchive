...
params:
  ...
  metric: ["wd_mgd"]
  map_name: ["Appomattox_wd_mgd_zoom14"]
---
...
streams <- streams_order4[!streams_order4$gnis_name==' ',] # Need to remove rows without names 

stream.centroid.df <-  data.frame(feature=(1:length(streams$id)), #creating data frame containing names and centroid locations 
...
for (r in 1:length(streams$id)) { #adding coordinates to the centroids df for labeling
    stream_geom_centroid <- st_centroid(streams$geometry[r])
    cent.coords <- as.data.frame(st_coordinates(stream_geom_centroid))
    stream.centroid.df$centroid_longitude[r] <- cent.coords$X
    stream.centroid.df$centroid_latitude[r] <- cent.coords$Y
    }
```
