...
waterbodies.centroid.df$class <- 5
riv.centroid.df$angle <- 20 # angles to display text
stream4.centroid.df$angle <- 20
waterbodies.centroid.df$angle <- 0
labs_bind <- rbind(riv.centroid.df, stream4.centroid.df, waterbodies.centroid.df) # binding data frames together
```
...
map <- basemap + #ggplot2::
    ...
    geom_sf(data = nhd$flowline_plot,inherit.aes = FALSE,color = "deepskyblue3", fill = NA, mapping = aes(lwd = nhd$flowline_plot$StreamOrde), 
            show.legend = FALSE) + scale_linewidth(range = c(0.4,1.5)) + # mapping flowlines with thickness based on stream order

    geom_sf(data = nhd$network_wtbd,inherit.aes = FALSE,color = "deepskyblue3", fill = 'deepskyblue3', size = 1) +
    geom_sf(data = nhd$off_network_wtbd,inherit.aes = FALSE,color = "deepskyblue3", fill = 'deepskyblue3', size = 1) +
  ...
  #  geom_label(data = facil_labels, aes(X, Y, label = NUM), colour = "black", size = 10, 
    ...
    geom_text_repel(data = labs_bind, 
                    aes(x = as.numeric(centroid_longitude), y = as.numeric(centroid_latitude), 
                        group = 1, label = GNIS_NAME, size = class, fontface="bold", family="serif", angle= angle ),
...
#add locality/county
# right now they are all filling in blank, in the fipsnew data a lot of the dh_link_admin_dha_usafips are blank, if you try to pull one that isnt blank it will result in the fips code
...
for (i in 1:nrow(facil_sf)){
fipsnew <- getAdminregFeature(inputs = list(bundle = 'permit'), 
                              base_url = site)
usafips <- fipsnew$dh_link_admin_dha_usafips
tableData$usafips[i] <- usafips
}
...
for (i in 1:nrow(facil_sf)){
val2 <- RomProperty$new(ds,list(
    riverseg= facil_sf$riverseg[i],
    propname= "fac_current_mgy"), 
  TRUE)
permitMax <- val2$propvalue
tableData$permitMax[i] <- permitMax
}

##new, from glenn -- seems to be successful 
for (i in 1:nrow(facil_sf)){
  ...
  facil_sf$fac_current_mgy[i] <- fiveYrAvg$propcode #adds 5-yr avg use as a new col to facil_sf data frame 
}
