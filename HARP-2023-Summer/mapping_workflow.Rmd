...
##               colour = "#002269", fill=NA, size=textsize[3]) +
  ...
  #  geom_label(data = facil_labels, aes(X, Y, label = NUM), colour = "black", size = 10, 
    ...
    geom_text_repel(data = waterbodies.centroid.df, aes(x = as.numeric(centroid_longitude), y = as.numeric(centroid_latitude), 
                    label = GNIS_NAME), color = '#002289', size = 5) + #label waterbodies
...
ggsave(
  ...
  filename = paste(export_path, rivseg, '.runid13', ".png", sep=""),
  plot = map,
  width = 25,
  height = 20
  )
