...
params:
  ...
  metric: ["wd_mgd"]
  map_name: ["James_JL6_7320_7150_roads3"]
---
...
```{r Foundational Data, message=FALSE, warning=FALSE}
...
# remove cols not expected to use: ?
#foundatn <- foundatn %>% dplyr::select(-(contains("19") | contains("20") | contains("OWS Planner")) )
```
...
```{r}
## To label measuring points using foundational data frame 
mp_labs <- subset(foundatn_basin, select = c(MP_hydroid, `MP Name`, Latitude, Longitude, metric_runid1)) #creating a df with only the needed data
mp_labs <- mp_labs[order(-mp_labs$metric_runid1),] #sort by decreasing metric values 
mp_labs$NUM <- seq(1:length(mp_labs$`MP Name`)) # adding column for labeling by number 
```
...
#new labels with foundation
#foundatn_sf <- foundatn_sf[order(-foundatn_sf$metric_runid1),]
...
```{r US Hwy Data, warning=FALSE}
# State Routes (shp)
localpath <- tempdir()
url <- "https://www2.census.gov/geo/tiger/TIGER2022/PRIMARYROADS/tl_2022_us_primaryroads.zip"
filename <- basename(url)
download.file(url, filename)
unzip(filename, exdir=localpath)
roads <- read_sf(dsn=localpath, layer="tl_2022_us_primaryroads") #US primary roads

url <- "https://www2.census.gov/geo/tiger/TIGER2022/STATE/tl_2022_us_state.zip"
filename <- basename(url)
download.file(url, filename)
unzip(filename, exdir=localpath)
VA <- read_sf(dsn=localpath, layer="tl_2022_us_state") #US states

VA <- states[states$NAME=="Virginia", ] 
roads <- st_intersection(roads, VA) #only VA primary roads

roads <- subset(roads, RTTYP=="I") #only interstates
roads$FULLNAME <- gsub("\\- ", "", roads$FULLNAME) #shorten names; no space too

rd_names <- roads[!duplicated(roads$FULLNAME),] #don't label the same name multiple times, but still plot full road geometry

for (r in 1:nrow(rd_names)) { #adding centroid coord. for labeling
    rd_centroid <- st_centroid(rd_names$geometry[r])
    centroid <- as.data.frame(st_coordinates(rd_centroid))
    rd_names$lng[r] <- centroid$X
    rd_names$lat[r] <- centroid$Y
    }
```
...
maj_cities <- cities_va[cities_va$population >= quantile(cities_va$population, probs = c(0.75)),]
```
...
riv.centroid.df$size <- 2

stream4.centroid.df$class <- 1
stream4.centroid.df$size <- 1
labs_bind <- rbind(riv.centroid.df, stream4.centroid.df) # binding data frames together
labs_bind <- labs_bind[,-1]

cities <- maj_cities[,c("city", "lng", "lat")]
cities$class <- 2
cities$size <- 1
...
fontfaces <- c( #c(cnty, maj, str, city)
                ...
                rep("bold",  repetitions(1)),
                rep("plain",  repetitions(2))
              ) 
fontfams <- c( #c(cnty, maj, str, city)
                rep("serif",  repetitions(4)),
                rep("serif",  repetitions(3)),
                rep("serif",  repetitions(1)),
                rep("sans",  repetitions(2))
              )
angles <- c( #c(cnty, maj, str, city)
                rep(0,  repetitions(4)),
                rep(20,  repetitions(3)),
                rep(20,  repetitions(1)),
                rep(0,  repetitions(2))
              )
bgrs <- c( #c(cnty, maj, str, city)
                rep(.03,  repetitions(4)),
                rep(.05,  repetitions(3)),
                rep(.1,  repetitions(1)),
                rep(.05,  repetitions(2))
              )
seg_cols <- c( #c(cnty, maj, str, city)
                rep("#003333",  repetitions(4)),
                rep("deepskyblue4",  repetitions(3)),
                rep("deepskyblue4",  repetitions(1)),
                rep("black",  repetitions(2))
              )
text_color <- c("deepskyblue4","black","deepskyblue4","#003333") #c(str, city, maj, cnty)

seg_size <- c( #c(cnty, maj, str, city)
            rep(1,  repetitions(4)),
            rep(0.75,  repetitions(3)),
            rep(0.5,  repetitions(1)),
            rep(0.5,  repetitions(2))
            )

```

```{r Boundary Box, message=FALSE, warning=FALSE}
...
basemap_sf <- st_as_sf(basemap_sp)
st_crs(basemap_sf) <- 4326
```

```{r Generate Map Object, echo=FALSE}
map <- basemap + #ggplot2::
    # Lighten base-map to help readability
    geom_sf(data = basemap_sf, inherit.aes = FALSE, color = NA, fill="honeydew", alpha=0.3) +
    # County & River Lines
    ...
    # Road Lines
    geom_sf(data = roads, inherit.aes = FALSE, color = "black", fill = NA, lwd=0.75, linetype="twodash") +
    # Basin Outlines
    geom_sf(data = basin_sf, inherit.aes = FALSE, color = "sienna1", fill = NA, lwd=textsize[5], linetype="dashed") +
              # color = "#002289", fill = #0022974C

    theme(text = element_text(size = 30),axis.title.x=element_blank(),axis.title.y=element_blank()) +
  
  # Road Labels
    geom_label_repel(data=rd_names,
                aes(x = lng, y = lat, label=FULLNAME, 
                    fontface="plain", family="Comic Sans MS"), show.legend=FALSE,
                size=textsize[3]-1,
  ...
  # Labels
    ...
    geom_text(data = facil_labels, 
                  aes(X, Y, label = NUM, fontface="bold"), 
                  colour = NA, size = textsize[4]+1
                  ) +
    # Rsegs
...
##               colour = "#002269", fill=NA, size=textsize[3]) +
    # Cities
    geom_point(data = maj_cities, 
               aes(x = as.numeric(lng), y = as.numeric(lat)),
               color = "black", size = 2
               ) +
#    ggrepel::geom_text_repel(data=maj_cities,
#                aes(x = as.numeric(lng), y = as.numeric(lat), label=city, 
#                    fontface="plain", family="sans"), show.legend=FALSE,
#                size=textsize[3],
#                colour="black", bg.color="white", bg.r=0.05,
#                min.segment.length= 0.1, segment.color = "black", segment.size = 0.5,
#                force=30,
#                direction="both",
#                ) +
    # Rivers
#    geom_text_repel(data=riv.centroid.df,
#                aes(x = as.numeric(centroid_longitude), y = as.numeric(centroid_latitude), label=GNIS_NAME, 
#                    fontface="bold", family="serif", angle=17), show.legend=FALSE,
#                size=textsize[2],
#                colour="deepskyblue4", bg.color="white", bg.r=0.05,
#                min.segment.length= 0.1, segment.color = "deepskyblue4", segment.size = 0.75,
#                force=40,
#                direction="both",
#                ) +
    # Streams
#    geom_text_repel(data=stream4.centroid.df,
#                aes(x = as.numeric(centroid_longitude), y = as.numeric(centroid_latitude), label=GNIS_NAME, 
#                    fontface="bold", family="serif", angle=17), show.legend=FALSE,
#                size=textsize[3],
#                colour="deepskyblue4", bg.color="white", bg.r=0.1,
#                min.segment.length= 0.1, segment.color = "deepskyblue4", segment.size = 0.5,
#                force=30,
#                direction="both",
#                ) +
    # Counties
#    geom_text_repel(data = fips_labels, 
#               aes(X, Y, label = NAME, 
#                   fontface="bold.italic", family="serif"), show.legend=FALSE,
#               size=textsize[1],
#               colour="#003333", bg.color="white", bg.r=0.03,
#               min.segment.length= 0.5, segment.color = "#003333", segment.size = 1,
#               force=40,
#               direction="both",
#               ) +
 ...
 # Labels
    ...
    geom_text_repel(data = labs_bind, 
                    ...
                    force=35,
                    direction="both",
                    bg.color="white", bg.r= bgrs,
                    min.segment.length= 0.5, segment.color = seg_cols, segment.size = seg_size,
                    ) + 
          scale_size(range= range(textsize[3:1]), breaks=textsize[3:1]) + 
          scale_colour_manual(values=text_color) +
  
  # 2020 MGD
      new_scale("size") +
      geom_point(data = facil_labels, aes(X, Y, size=metric_runid1 ), alpha=0.75, #, alpha=metric_runid1),
               col="#F7FF00", shape = 19, stroke = 0.75) +
      scale_size(range = c(10,33), 
...
# Facility Labels
    geom_text(data = facil_labels, 
                    aes(X, Y, label = NUM, fontface="bold"), 
                    colour = "black", size = textsize[4],
                    check_overlap=TRUE
                    #min.segment.length= 0.1, segment.color = "black", segment.size = 0.5,
                    ) +

# Measuring Points and Labels 
#  geom_point(data = mp_labs, aes(Longitude, Latitude, size = metric_runid1, alpha = metric_runid1), color = '#FF00FF') +
#      scale_size(range = c(10,33)) + scale_alpha(range = c(0.3, 0.85)) +
#  geom_text(data = mp_labs, aes(Longitude, Latitude, label = NUM), size = 5, check_overlap = TRUE) +

  
  # Reverse Fill
    geom_sf(data = nonbasin_sf, inherit.aes = FALSE, color = NA, fill = "#4040408F", lwd=1) + #color was "#002289";fill was CCCCCC8F
...
```{r Road Experiment}
roads_no_overlap <- geom_text_repel(data=rd_names,
                aes(x = lng, y = lat, label=FULLNAME, 
                    fontface="plain", family="Comic Sans MS", angle=0), #show.legend=FALSE,
                size=textsize[3]-1,
                colour="black", bg.color="white", bg.r=0.15,
                force=0, max.overlaps=1
                )
...
road_diamonds <- geom_point(data = roads_no_overlap[["data"]], aes(lng, lat),
               col="black", fill="white", shape = 23, size=8, stroke = 0.9)
...
```{r OLD Create table w/ more facility info, echo= FALSE, warning=FALSE, message=FALSE, eval=FALSE}

#KEEP ONLY FOR REFERNCE, DONT RUN THIS SECTION, GENERATES INCOMPLETE TABLE 
...
```{r NEW Table, warning=FALSE, message=FALSE}
## NEW - Creating table to accompany map using new joined foundational data frame
 
#might want to add this to the join later to be cleaner
facil_labels <- data.frame(sf::st_coordinates(foundatn_sf), NAME=foundatn_sf$Facility) #create df for facility labels 
facil_labels$NUM <- seq(1, nrow(facil_labels)) #add column for number sequence 
  

# creating initial data frame that will accompany map
tableData2 <- data.frame(Number= facil_labels$NUM, #commented out to be fixed later with updates 
                        Facility=foundatn_sf$Facility, 
...
# add locations(subwatershed basin) not required anymore
...
tableData2$vwp_max_mgy <-  replace(tableData2$vwp_max_mgy, tableData2$vwp_max_mgy==0, "No Permit")
...
val <- RomProperty$new(ds,list(
  ...
  TRUE)
five_mgy <- val$propvalue
tableData2$five_mgy[i] <- round(five_mgy,2)
}

#5 year avg for sources 

#what 5 years should we be using 
#why is 
for (i in 1:nrow(foundatn_sf)){
  fiveavg <- mean(na.omit(foundatn_sf$'2022'[i],foundatn_sf$'2021'[i],foundatn_sf$'2020'[i],foundatn_sf$'2019'[i],foundatn_sf$'2018'[i], na.rm= TRUE))
  tableData2$five_mgy_new[i] <- round(fiveavg, 2)
}
```
...
table <- set_header_labels(table,
  ...
  five_mgy = "5 Year Average Use(mgy)",
...
# Checking out Road data:
setwd(wd)
#roads <- read_sf(dsn="/aa_HARP/tl_2021_us_primaryroads", layer="tl_2021_us_primaryroads")
roads <- read_sf(dsn="/aa_HARP/tl_2021_51_prisecroads", layer="tl_2021_51_prisecroads") #VA's primary and secondary roads
roads <- roads[grep("\\State Rte.*", roads$FULLNAME),] #filter to State Rte only
roads$FULLNAME <- gsub("\\State Rte ", "Rte", roads$FULLNAME) #shorten route names; no space too
rd_names <- roads[!duplicated(roads$FULLNAME),] #don't label the same name multiple times, but still plot full road geometry
...
ggmap_bbox <- st_buffer(st_as_sfc(st_bbox(roads)), .05)
ggmap_bbox <- setNames(st_bbox(ggmap_bbox), c("left", "bottom", "right", "top"))
basemap <- ggmap::get_stamenmap(maptype = "terrain", color="color", bbox = ggmap_bbox, zoom = 9)
basemap <- ggmap(basemap)
VA <- basemap +
    geom_sf(data = roads, inherit.aes = FALSE, color = "black", fill = NA, lwd=0.2) +
    geom_text_repel(data=rd_names,
                aes(x = lng, y = lat, label=FULLNAME, 
                    fontface="plain", family="Optima", angle=0), #show.legend=FALSE,
                size=3,
...
ggsave(
  filename = paste(export_path, "VA_roads", ".png", sep=""),
  plot = VA,
  width = 25,
  height = 20
  )
...
```{r}
## VAhydro map customization storage, example of extent RETRIEVAL -- Testing for now
broadway_wtp <- RomProperty$new(ds,list(
    featureid = '74345',  # hard-coded only for testing, but fid should be able to come from foundational df
    propcode= "vahydro-1.0"), 
  TRUE)
reports <- RomProperty$new(ds,list(
    featureid = broadway_wtp$pid,
    propname= "reports"), 
  TRUE)
map <-  RomProperty$new(ds,list(
    featureid = reports$pid,
    propname= "map"), 
  TRUE)
extent <- RomProperty$new(ds,list(
    featureid = map$pid,
    propname= "extent"), 
  TRUE)
extent_map <- extent$propcode
extent_map <- strsplit(extent_map, split = ',')
bbox_points_vah <- tibble(long = c(extent_map[[1]][1], extent_map[[1]][3]), lat = c(extent_map[[1]][2], extent_map[[1]][4]))
bbox_sf_vah <- st_bbox(st_as_sf(bbox_points_vah, coords = c('long','lat'), crs = 4326))
ggmap_bbox_vah <- setNames(st_bbox(bbox_sf_vah), c("left", "bottom", "right", "top"))
```
