...
params:
  ...
  metric: ["wd_mgd"]
  map_name: ["James_JL6_7320_7150_June12_2"]
---
...
##               colour = "#002269", fill=NA, size=textsize[3]) +
    ...
    geom_text_repel(data=riv.centroid.df,
                aes(x = as.numeric(centroid_longitude), y = as.numeric(centroid_latitude), label=GNIS_NAME, fontface="bold", family="serif", angle=20),
                colour="deepskyblue4", size=textsize[2], show.legend=FALSE, max.overlaps=1) +
    geom_text_repel(data=stream4.centroid.df,
                aes(x = as.numeric(centroid_longitude), y = as.numeric(centroid_latitude), label=GNIS_NAME, fontface="bold", family="serif", angle=20),
                colour="deepskyblue4", size=textsize[3], show.legend=FALSE, max.overlaps=2) +
    # County Labels
    geom_text_repel(data = fips_labels, 
               aes(X, Y, label = NAME, fontface="bold.italic", family="serif"), 
               colour = "#003333", size=textsize[1], show.legend=FALSE, force=6 ) +
...
##          scale_colour_manual(values=text_color) +
    ...
    geom_point(data = maj_cities, aes(x = as.numeric(lng), y = as.numeric(lat)), color = "black", size = 2) + # points for cities
...
##                 labels = round(c(metric_runid1[metric_runid1!=0], 0), digits = 3),
  ...
  # Cities
    ggrepel::geom_label_repel(data=maj_cities,
                aes(x = as.numeric(lng), y = as.numeric(lat), label=city, fontface="plain", family="sans"),
                colour="black", size=textsize[4], label.size=0, fill=NA, show.legend=FALSE,
                label.r=unit(0,"lines"),box.padding=unit(0,"lines"), label.padding=unit(0,"lines"))
