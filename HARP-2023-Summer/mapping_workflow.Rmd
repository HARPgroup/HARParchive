...
# Get all facilities
df <- data.frame(runid=runid_list, model_version, metric='wd_mgd')
...
WD_MGD_runid_11 <- facil_sf$WD_MGD_runid_11
...
map <- basemap + 
    ...
    geom_point(data = facil_labels,aes(X, Y, size=WD_MGD_runid_11), col="red", shape = 19,alpha = .7) +
      scale_size_area(breaks = c(0, WD_MGD_runid_11[WD_MGD_runid_11!=0]),
                      labels = c(0, WD_MGD_runid_11[WD_MGD_runid_11!=0])) +
  
    theme(text = element_text(size = 30),axis.title.x=element_blank(),axis.title.y=element_blank()) +
    
    # plot labels
    geom_text(data = rseg_labels, aes(X, Y, label = NAME), colour = "#002269", size = 8) +
    geom_label(data = facil_labels, aes(X, Y, label = NUM), colour = "red", size = 10, nudge_x = -0.01, nudge_y = 0.01) +
