...
# Adding columns to each df for unique labeling 
riv.centroid.df$class <- 9
stream4.centroid.df$class <- 6
labs_bind <- rbind(riv.centroid.df, stream4.centroid.df) # binding data frames 
...
#                    size = 4, color = "dodgerblue3") + #labeling order 4 streams
    geom_text_repel(data = labs_bind, aes(x = as.numeric(centroid_longitude), y = as.numeric(centroid_latitude), group = 1, label = GNIS_NAME, size = class),
                    color = "dodgerblue3", show.legend = FALSE) + scale_size(range = c(5,10)) + 
    # scalebar
...
ggsave(
  filename = paste(export_path, map_name, '7',".png",sep=""),
  plot = map,
  width = 25,
  height = 20
  )
