...
```{r, eval = FALSE}
#### Manual bbox creation -- FOR TESTING
...
ggmap_bbox <- setNames(st_bbox(bbox_), c("left", "bottom", "right", "top"))
```
...
streams_order6 <- flowline[flowline$StreamOrde == 6,]
streams6 <- streams_order6[!streams_order6$gnis_name==' ',] # Need to remove rows without names 
streams6 <- streams6[!duplicated(streams6$gnis_id),] #Only want 1 row per stream name 

stream6.centroid.df <-  data.frame(feature=(1:length(streams6$id)), #creating data frame containing names and centroid locations 
                                GNIS_NAME=streams6$gnis_name,
                                centroid_longitude="",
                                centroid_latitude="",
                                stringsAsFactors=FALSE) 

for (r in 1:length(streams6$id)) { #adding coordinates to the centroids df for labeling
    stream_geom_centroid <- st_centroid(streams6$geometry[r])
    cent.coords <- as.data.frame(st_coordinates(stream_geom_centroid))
    stream6.centroid.df$centroid_longitude[r] <- cent.coords$X
    stream6.centroid.df$centroid_latitude[r] <- cent.coords$Y
    }
```

```{r}
# Creating subset of order 6 stream flowlines for mapping thicker lines 
flowline6 <- flowline[flowline$StreamOrde == 6,] #removing all rows with a strem order of 1
flowline6_sf <- st_sf(flowline6) #converting to simple feature format for rejoining with nhd object
nhd$flowline6 <- flowline6_sf
```
...
#    geom_sf(data = fips_sf, inherit.aes = FALSE, color = "#0033335F", fill = NA, lwd=1) +
    ...
    geom_sf(data = nhd$flowline,inherit.aes = FALSE,color = "#009999", fill = NA, lwd = 0.3) + 
    geom_sf(data = nhd$flowline6,inherit.aes = FALSE,color = "blue", fill = NA, lwd = 0.6) + 
...
#    geom_text(data = facil_labels, aes(X, Y, label = NUM), colour = "black", size = 7) +
  #  geom_label(data = facil_labels, aes(X, Y, label = NUM), colour = "black", size = 10, 
    ...
    geom_text(data = stream6.centroid.df, aes(x = as.numeric(centroid_longitude), y = as.numeric(centroid_latitude), group = 1, label = GNIS_NAME),size = 10, 
                    color = "red") + #labeling order 6 streams -- also majopr rivers
...
ggsave(
  filename = paste(export_path, map_name, '3',".png",sep=""),
  plot = map,
  width = 25,
  height = 20
  )
