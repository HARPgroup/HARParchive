...
```{r Labeling major rivers alternate method, eval = FALSE}
...
# Removing rows which already match a major river from MajorRivers.csv
...
basemap_0 <- ggmap::get_stamenmap(maptype = "terrain", color="color", bbox = ggmap_bbox, zoom = 11)
...
# Adjusting which flowlines to map based on boundary box size for clean mapping 
if (dist_mi > 500) {
  nhd$flowline <- nhd$flowline[!nhd$flowline$StreamOrde == c(1,2,3),]
} 
if (dist_mi > 300 && dist_mi <= 500) {
  nhd$flowline <- nhd$flowline[!nhd$flowline$StreamOrde == c(1,2),]
} 
if (dist_mi > 150 && dist_mi <= 300) {
  nhd$flowline <- nhd$flowline[!nhd$flowline$StreamOrde == 1,]
} 
```
...
map <- basemap + #ggplot2::
    ...
    geom_sf(data = nhd$flowline,inherit.aes = FALSE,color = "deepskyblue3", fill = NA, mapping = aes(lwd = nhd$flowline$StreamOrde), show.legend = FALSE) + 
       scale_linewidth(range = c(0.4,2)) + # mapping all flowlines with thickness based on stream order
    ...
    geom_sf(data = basin_sf, inherit.aes = FALSE, color = "#002289", fill = NA, lwd=1) + # fill = #0022974C
    ...
    # Labels
    geom_label(data = rseg_labels, 
               aes(X, Y, label = SEG, fontface="bold", family="mono"), 
               colour = "#002269", fill=NA, size=7, label.size=0, label.r= unit(0.3, "mm")) +
    geom_label(data = fips_labels, 
               aes(X, Y, label = NAME, fontface="bold.italic", family="serif"), 
               colour = "#003333", fill= NA, size=10, label.size=0, label.r= unit(0.3, "mm")) +
  #  geom_label(data = facil_labels, aes(X, Y, label = NUM), colour = "black", size = 10, 
     #          nudge_x = -0.01, nudge_y = 0.01) +
#    geom_text(data = riv.centroid.df, aes(x = as.numeric(centroid_longitude), y = as.numeric(centroid_latitude), group = 1, label = GNIS_NAME),size = 8, 
...
#                    size = 8, color = "dodgerblue3") + #labeling order 6 & 7 streams -- also major rivers
#    geom_text_repel(data = stream4.centroid.df, aes(x = as.numeric(centroid_longitude), y = as.numeric(centroid_latitude), group = 1, label = GNIS_NAME),
#                    size = 4, color = "dodgerblue3") + #labeling order 4 streams
    geom_text_repel(data = labs_bind, 
                    aes(x = as.numeric(centroid_longitude), y = as.numeric(centroid_latitude), 
                        group = 1, label = GNIS_NAME, size = class, fontface="bold", angle=20),
                    color = "deepskyblue4", show.legend = FALSE) + scale_size(range = c(6,10)) + 
                    #labeling both major rivers and smaller rivers/streams
  # Reverse Fill
    geom_sf(data = nonbasin_sf, inherit.aes = FALSE, color = "#002289", fill = "#CCCCCC8F", lwd=1) +
  ...
  # Facility Labels
      geom_text(data = facil_labels, aes(X, Y, label = NUM, fontface="bold"), colour = "black", size = 6) +
    # 2040 MGD
...
tableData <- data.frame(Facility=facil_sf$propname,Number= facil_labels$NUM, Rivseg= facil_sf$riverseg, wd_mgd=facil_sf$metric_runid1) #need to fix numbers so they coordinate with mapping numbers, right now it just makes a new set 
...
county <- RomFeature$new(ds,list(
    bundle= "usafips", ftype= "facility"), 
  TRUE)
  
#add source(instake-measuring point)
...
table <- set_header_labels(table,
  ...
  NUM = "Facility Number", Rivseg= "River Segment", Location_Basin = "Location(Basins)",
  Upstream_Downstream = "Upstream Vs. Downstream", wd_mgd="2020 Cummulative Withdraw in mgd"
)
table
```
