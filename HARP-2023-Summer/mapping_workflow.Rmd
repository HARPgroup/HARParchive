...
#Modifying stream order to be plotted 
...
stream4.centroid.df <-  data.frame(feature=(1:length(streams$id)), #creating data frame containing names and centroid locations 
                                GNIS_NAME=streams$gnis_name,
                                centroid_longitude="",
                                centroid_latitude="",
                                stringsAsFactors=FALSE) 

for (r in 1:length(streams$id)) { #adding coordinates to the centroids df for labeling
...
map <- basemap + #ggplot2::
    ...
    geom_sf(data = nhd$flowline,inherit.aes = FALSE,color = "#009999", fill = NA, size = 0.5) +
  ...
  #  geom_label(data = facil_labels, aes(X, Y, label = NUM), colour = "black", size = 10, 
    ...
    geom_text(data = stream4.centroid.df, aes(x = as.numeric(centroid_longitude), y = as.numeric(centroid_latitude), group = 1, label = GNIS_NAME),size = 5, 
