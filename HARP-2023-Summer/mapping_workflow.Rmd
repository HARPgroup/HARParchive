...
flowline567 <- flowline[flowline$StreamOrde == c(5,6,7),] #removing all rows with a strem order of 1
flowline567_sf <- st_sf(flowline567) #converting to simple feature format for rejoining with nhd object
nhd$flowline567 <- flowline567_sf
```
...
basemap <- ggmap::get_stamenmap(maptype = "terrain", color="color", bbox = ggmap_bbox, zoom = 10)
...
#   geom_sf(data = fips_sf, inherit.aes = FALSE, color = "#0033335F", fill = NA, lwd=1) +
    ...
    geom_sf(data = nhd$flowline,inherit.aes = FALSE,color = "skyblue3", fill = NA, mapping = aes(lwd = nhd$flowline$StreamOrde), show.legend = FALSE) + 
       scale_linewidth(range = c(0.4,2)) + # mapping all flowlines with thickness based on stream order
    #geom_sf(data = nhd$flowline567,inherit.aes = FALSE,color = "cornflowerblue", fill = NA, lwd = 0.5) + # mapping flowlines of major rivers
...
#                    size = 4, color = "dodgerblue3") + #labeling order 4 streams
    geom_text_repel(data = labs_bind, aes(x = as.numeric(centroid_longitude), y = as.numeric(centroid_latitude), group = 1, label = GNIS_NAME, size = class),
                    color = "dodgerblue4", show.legend = FALSE) + scale_size(range = c(6,10)) + #labeling both major rivers and smaller rivers/streams
    # Original scalebar
...
ggsave(
  filename = paste(export_path, map_name, '10', ".png", sep=""),
  plot = map,
  width = 25,
  height = 20
  )
...
tableData <- data.frame(Facility=facil_sf$propname, NUM=seq(1, nrow(facil_sf)))

#add locations(locality or subwatershed)
i <-1
while (i:length(facil_sf)) {
  if(facil_sf$riverseg == basin_featr$riverseg){
    tableData[i,] <- basin_featr$name
    i <- i + 1
  }
}
...
for(i in 1:nrow(facil_in_basin)){
facil_loc <- RomFeature$new(ds,list(hydroid= facil_in_basin$pid[i], bundle= "usafips"), 
  TRUE)}
facil_loc.df <- cbind(facil_loc[["name"]], facil_loc[["hydroid"]])
 # colnames(counties.df) <- c("County", "hydrocode_fips")
