...
if (type=="facility"){
  table$vwp_max_mgy <- NA #create empty col for permit capac storage
    for (i in 1:nrow(facils$within)){ #adding permitted capacity
      ...
      systemObj <- RomProperty$new(ds,list( #rbind error returned when facil has multiple vahydro-1.0 model features
        featureid = facils$within$Facility_hydroid[i],
        propcode= "vahydro-1.0",
        propname= facilObj$name,
...
```{r Rename Table Columns & Create Flextable, include=FALSE, echo=FALSE}
...
fn_tablegen(type = "facility", table = table, alignment = "left") #Execute flextable function, which saves the flextable as variable 'ft' for displaying later
```
...
#ftable<- list()
for (k in 1:length(rivseg_metric)){
  rivTables[[k]] <- data.frame(Name = segs$basin$name)
  ...
  #Flextable function
  fn_tablegen(type = "riverseg", table = rivdf, alignment = "left") #flextable saved as var ft
...
```{r View Rivseg Flextable, results='asis', echo=FALSE}
for (k in 1:length(rivseg_metric)){
