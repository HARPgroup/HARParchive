...
if(type == "facility"){
  ...
  facils3 <- facils[facils[geoCol(facils)]!="" & facils[geoCol(facils)]!="NA",] #omits facilities with blank or NA geometry
}
#connect facilities to the watersheds they are in:
rsegs <- rsegs[-which(rsegs[geoCol(rsegs)] == "" | rsegs[geoCol(rsegs)] == "NA" ), ] #finds geom column & omits rsegs with blank or NA geometry
rsegs <- st_as_sf(rsegs, wkt = geoCol(rsegs), crs=4326)
facils3 <- st_as_sf(facils2, wkt = geoCol(facils2), crs=4326) #doesn't work. can't get rid of some NA-presenting values. breaks

for(i in 1:length(facils2$Facility)){
  if(i==1){
    facils3 <- st_as_sf(facils2[i,], wkt = geoCol(facils2), crs=4326)
  }
  if(i!=1){
    facils3 <- rbind(facils3, st_as_sf(facils2[i,], wkt = geoCol(facils2), crs=4326) )
  }
}

facils3 <- facils2[facils[geoCol(facils)]=="<NA>"|facils[geoCol(facils)]=="NA",]
...
```{r Facil-Feature Metric Pull (non-modeled), echo=FALSE, message=FALSE, warning=FALSE}
...
```{r Rivseg Metric % Difference, echo=FALSE, warning=FALSE}
for (k in 1:length(rivseg_metric)){
...
# Map Prep
```{r Create Bbox, echo=FALSE, warning=FALSE}
#Create bbox
...
```{r Filter Cities by Bbox, echo=FALSE, warning=FALSE, message=FALSE}
...
cities_plot <- head(cities_sorted, 5) #max 5 cities/towns on map, largest 10 by population
  ...
  table$vwp_max_mgy <- NA #create empty col for permit capac storage
    for (i in 1:nrow(facils$within)){ #adding permitted capacity
     ...
     #get permit status to label exempt
      ...
      TRUE)
        if (is.na(permit_status$propcode) == FALSE & permit_status$propcode == "exempt") { #source of error
...
for (k in 1:length(rivseg_metric)){
  ...
  drop <- c("segs.basin.name")
  rivTables[[k]] = rivTables[[k]][,!(names(rivTables[[k]]) %in% drop)]
    ...
    assign(paste0('table', k), ft, envir = globalenv())
    }
  print(get(paste0('table', k)))
}
...
```{r Display Rivseg Table, echo=FALSE}
ft #view flextable
```
