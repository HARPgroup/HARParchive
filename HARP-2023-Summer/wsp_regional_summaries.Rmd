...
```{r Labeling & Mapping Prep, warning=FALSE, echo=FALSE, message=FALSE, eval=FALSE}
...
#skip metric pull if scenario pid = NA, meaning runid wasn't found
  ...
  segs$basin[i,paste0(runid_list_riversegs[j],'_', rivseg_metric[k])] <- model_metric$propvalue
  } else { #if the scenario/runid wasn't found
    segs$basin[i,paste0(runid_list_riversegs[j],'_', rivseg_metric[k])] <- NA
       }
      }
    }
  }
...
rivTables <- list()
for (k in 1:length(rivseg_metric)){
  rivTables[[k]] <- data.frame(segs$basin$name)
    for (j in 1:length(runid_list_riversegs)){
       for (i in 1:nrow(segs$basin)) {
  
  
   rivTables[[k]][,paste0('Name')] <-segs$basin$name
   rivTables[[k]][,paste0('Riverseg')] <- segs$basin$riverseg
   
  rivTables[[k]][i,paste0(runid_list_riversegs[j])] = segs$basin[i,paste0(runid_list_riversegs[j],'_',rivseg_metric[k])]
  
  rivTables[[k]][,paste0('Difference')] = round(segs$basin[,paste0('diff','_',rivseg_metric[k])],digits= 2)
  
  rivTables[[k]][,paste0('% Difference')]= round(segs$basin[,paste0('percentDiff','_',rivseg_metric[k])], digits= 2)

  rivTables[[k]]$metric <- rivseg_metric[k]
  
  
    }
    fn_tablegen(type = "riverseg", table = rivTables[[k]], alignment = "center") #flextable saved as var ft
  
  assign(paste0('ft'), ft, envir = globalenv())
