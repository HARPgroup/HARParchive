...
# set data source to pull from: prism, nldas2, or daymet
data_source = "prism"
...
comp_data$hydro_cfs <- 1.572 * (da * 640.0 * comp_data$precip_in / 12.0) / 3.07 
...
week_data <- sqldf(
  "select min(obs_date) as week_begin, yr, wk, min(dataset_day) as dataset_day_begin,
     avg(precip_in) as precip_in, avg(hydro_cfs) as hydro_cfs,
...
#function
mod_hydro <- lm(usgs_cfs ~ hydro_cfs, data=comp_data)
...
mod_week_hydro <- lm(usgs_cfs ~ hydro_cfs, data=week_data)
summary(mod_week_hydro)
#Looking at the results, we see some correlation but it's still not very strong
plot(mod_week_hydro$model$usgs_cfs ~ mod_week_hydro$model$hydro_cfs)
...
# PRISM
mod_hydro_jan <- lm(usgs_cfs ~ hydro_cfs, data=comp_data[which(comp_data$mo == 1),])
summary(mod_hydro_jan)
...
plot(mod_hydro_jan$model$usgs_cfs ~ mod_hydro_jan$model$hydro_cfs)

# January, next day flow todays precip
mod_hydro_jan_nd <- lm(nextday_usgs_cfs ~ hydro_cfs,
                       data=comp_data[which(comp_data$mo == 1),])
summary(mod_hydro_jan_nd)
...
# 
mod_hydro_jan_nz_ndd <- lm(nextday_d_cfs ~ hydro_cfs,
                            data = comp_data[which((comp_data$mo == 1) & (comp_data$hydro_cfs > 0)),])
summary(mod_hydro_jan_nz_ndd)
plot(mod_hydro_jan_nz_ndd$model$nextday_d_cfs ~ mod_hydro_jan_nz_ndd$model$hydro_cfs)
...
# flow
mod_hydro_jan_nz_cdd <- lm(usgs_cfs ~ hydro_cfs,
                            data = comp_data[which((comp_data$mo == 1) & (comp_data$nextday_d_cfs > 0)),])
summary(mod_hydro_jan_nz_cdd)
plot(mod_hydro_jan_nz_ndd$model$nextday_d_cfs ~ mod_hydro_jan_nz_ndd$model$hydro_cfs)
...
#month
mod_hydro_mon_nz_ndd <- lm(nextday_d_cfs ~ hydro_cfs,
                           data = comp_data[which((comp_data$mo == 2) & (comp_data$nextday_d_cfs > 0)),])
summary(mod_hydro_mon_nz_ndd)
plot(mod_hydro_mon_nz_ndd$model$nextday_d_cfs ~ mod_hydro_mon_nz_ndd$model$hydro_cfs)
...
hydro_lm <- mon_lm(week_data, "hydro_cfs", "usgs_cfs", "mo", data_source)
hydro_lm$atts
hydro_lm$plot
