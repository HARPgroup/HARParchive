...
model_comparison <- function(gage_id){
  ...
  percent_diff_abs <- j3|>
    ...
    mutate(residual = abs(precip_in-mean(precip_in)))|>
  ...
  p3 <- ggplot(percent_diff, mapping = aes(x = mo, y = var/3))+
    geom_bar(stat = "identity")+
    labs(title = gage_id)
    
  p4 <- ggplot(percent_diff, mapping = aes(x = mo, y = CV/3))+
    geom_bar(stat = "identity")+
    labs(title = gage_id)
  
  p5 <- ggplot(percent_diff, mapping = aes(x = mo, y = precip_in))+
    geom_bar(stat = "identity")+
    labs(title = gage_id)
